...
|---:|---|---:|---|
| 1 | `lib/components/ModalHost.svelte` | 6123 | `surface-audit-contract.test.ts` — **the last row in this table**, and it could never be one surface: this file is the union of two dozen reference modals, so it is audited as the SET of selectors it implements, each measured against its own const table on 2026-09-01. `app-av-settings-modal` (36 consts, 3 views), `app-play-youtube-modal` (19/3), `app-alerts-advanced-search` (50/17), `app-webrtc-troubleshooter` (85/43 — the largest clean surface pinned here), `app-alert-qa-modal` (38/15), `app-alert-filter-modal`, `app-mobile-app-info-modal`, `app-muted-users-modal`, `app-followed-users-modal`, `app-debug-log-modal`, `app-screenshare-preview` and `app-poll-modal` all measure **CLEAN**; `app-user-info-modal` (131/35) has nine values and they are two recorded reasons (four `#nav-*` composed at runtime, five `value="followChatStyle.*"` that Svelte refuses to write beside a binding), plus — since 2026-09-03 — exactly ONE text gap, `(test it)`, which is a REFUSAL and is named as one: the anchor built `?forcedStream=<host>`, a media host from a query parameter is an authority the client asserts, and `forced-stream-refusal-contract.test.ts` sweeps every shipped file for the parameter while asserting the diagnostic value beside it still renders; `app-session-control-modal`, `app-alert-send-report-modal`, `app-all-user-pmmodal`, `app-rec-preview` and `app-closed-session-page` each carry their own pin and their own recorded refusals. `app-av-settings-modal` reached 0/0 the same day and not before: the audit had reported `speakers-device` against markup that said `av-speakers-device`. |
| 2 | `routes/+page.svelte` | 1,959 | `surface-audit-contract.test.ts`, `brand-attribution-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-room`, **the largest surface in the capture** (229 consts, 108 embedded views), scoped to the page and the 84 files that compose it. Five const values and one literal remain and every one is on record: the sidebar's `ProTradingRoom.com` attribution (rebranded, and the LOGIN page still renders the captured one — pinned at both ends now, because the split had never been recorded), `helpLink` + the intercom URL (`RNB-01`, a FALSE gap — `hasSTHelpLink` is `!1` in `app-room`'s own constructor and never written), `cssSoundCloudIcon` (const 176 declares `id` twice and Angular keeps the second) and `/assets/images/playing.gif` (an ASSET the four-file capture cannot supply). **The pin records its own measured limitation**: with 84 files in scope a value moving to the wrong file still passes, proved by a control that stayed green. |
...
| 7 | `lib/components/PresentationArea.svelte` | 1,150 | `surface-audit-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-presentationarea` (292 consts, 125 embedded views), the page's second giant, scoped to the 94 files that compose it. Five const values and two literals remain and both groups are one thing each: the **Recordings tab** (`recordings`, `recordings-tab`, `#recordings`, `fa-file-video`, `"Recordings"`), blocked on an archive service neither database has a table for; `dropdownMenuNote` (`NTC-3`, a recorded divergence — the capture freezes the per-note gear's id, so two open notes would share one); and `"No room files found."` (the `#files` region's own refusal). Getting there took four corrections to the file list and two to the tool — the notes, swing-alert and day-trade-alert SUBDIRECTORIES were outside the glob, `ngForm` is a local reference found by POSITION now, and a 261-character tooltip is split across a `+` by prettier. §MTS — feeds `canEditNotes` to the tab strip (`MTS-02`). |
| 8 | `lib/components/RoomOverlays.svelte` | 1258 | `## RoomOverlays.svelte` in the v4 register — 7 gaps, read 2026-08-31. |
...
| 15 | `lib/components/PostAlertModal.svelte` | 677 | `post-alert-render.test.ts`, `send-later-contract.test.ts`, `post-alert-behavior.test.ts`, `sticky-non-trade-alert-contract.test.ts`, `alert-label-picker-contract.test.ts` — READ END TO END 2026-09-01 against `app-post-alert-modal`'s own const table (79 consts at byte 2,131,663, `decls:73 vars:24`) and all thirteen embedded views, across the four files that implement it (`PostAlertModal`, `ScheduledAlerts`, `ScheduledAlertFields`, `ScheduledAlertsTable`). Twelve const values and five text literals were absent: seven were built as `PAM-11` (the Repeat row's and select's classes, the weekends wrapper, the note's — the const SWEEP cannot see these, being a substring search over the whole app), and five are recorded refusals (`alert-text-label`/"Text this out?" on Twilio, `alert-dont-cross-post-label` on linked-room fan-out, `sendLaterAsEmail`/`sendLaterAsNick` as `PAM-10`). `PAM-12` records a divergence that was correct and unwritten: consts 8 and 9 both hardcode `aria-selected="true"` and nothing ever writes the attribute, so the reference announces two selected tabs — `MTS-06` one component over. `PAM-13` was MATCHED on 2026-09-02: the media guard now tests whether a file list EXISTS, as `fc` does, and the `filesTouched` latch mirrors its two assignment sites. The component's own four CSS rules ship in `captured-runtime-components.css` under the `<app-post-alert-modal>` host. |
...
| 49 | `lib/components/AlertSendReportModal.svelte` | 161 | `surface-audit-contract.test.ts`, `alert-report-modal-contract.test.ts` — `## AlertSendReportModal.svelte` in the v4 register, 3 rows (ASR-01…03) read end to end 2026-08-31: 2 measured refusals, 1 blocked on a one-line change in `Modal.svelte` that this pass did not own. RE-MEASURED 2026-09-01 by `gate/audit-surface.mjs` against `app-alert-send-report-modal` (39 consts, 11 embedded views): **41 const values and 5 text literals absent, and all 46 are ONE refusal** — the report body, the search bar, the status select, the flot pie and the per-recipient row all rest on a list of DELIVERY RECORDS this application has nowhere (24 tables searched; `alerts.dispatch` is five booleans naming what was TICKED; no mail transport in `lib/server`; `getAlertReport` has zero occurrences). Pinned as a SET, so building the queue turns the pin red rather than quietly satisfying it. |
...
| 53 | `lib/components/SessionHistoryPane.svelte` | 145 | `session-control-refusals-contract.test.ts`, `stream-player-blocked-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-session-control-modal` (135 consts, 32 embedded views). Fifteen const values and five text literals remain, and every one now has a reason on record: eleven are the Bootstrap-data-API `#`-target group, three are recorded residuals, `ngForm` is Angular, three texts are the WHIP pair and the Player Link readout (blocked at `ModalHost.svelte` and `stream-player-blocked-contract`), and the last two were unrecorded until now — **Swap Primary and Backup Media Servers** is gated on a `backupClusterID` that is `wired: false` and its action compares `deleteAlertPW` in the browser, and **Admin Dashboard Login** is gated on `modAdminLoginList`, one of the seven credentials that never cross to the room, so the gate itself is an oracle this room may not evaluate. Both premises are re-read from the bundle and the settings schema on every run. Audited with the session-control modal. |
...
| 63 | `lib/components/RestreamPane.svelte` | 105 | `session-control-refusals-contract.test.ts`, `stream-player-blocked-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-session-control-modal` (135 consts, 32 embedded views). Fifteen const values and five text literals remain, and every one now has a reason on record: eleven are the Bootstrap-data-API `#`-target group, three are recorded residuals, `ngForm` is Angular, three texts are the WHIP pair and the Player Link readout (blocked at `ModalHost.svelte` and `stream-player-blocked-contract`), and the last two were unrecorded until now — **Swap Primary and Backup Media Servers** is gated on a `backupClusterID` that is `wired: false` and its action compares `deleteAlertPW` in the browser, and **Admin Dashboard Login** is gated on `modAdminLoginList`, one of the seven credentials that never cross to the room, so the gate itself is an oracle this room may not evaluate. Both premises are re-read from the bundle and the settings schema on every run. Audited with the session-control modal. |
...
| 65 | `lib/components/CloseSessionPane.svelte` | 104 | `session-control-refusals-contract.test.ts`, `stream-player-blocked-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-session-control-modal` (135 consts, 32 embedded views). Fifteen const values and five text literals remain, and every one now has a reason on record: eleven are the Bootstrap-data-API `#`-target group, three are recorded residuals, `ngForm` is Angular, three texts are the WHIP pair and the Player Link readout (blocked at `ModalHost.svelte` and `stream-player-blocked-contract`), and the last two were unrecorded until now — **Swap Primary and Backup Media Servers** is gated on a `backupClusterID` that is `wired: false` and its action compares `deleteAlertPW` in the browser, and **Admin Dashboard Login** is gated on `modAdminLoginList`, one of the seven credentials that never cross to the room, so the gate itself is an oracle this room may not evaluate. Both premises are re-read from the bundle and the settings schema on every run. Audited with the session-control modal. |
...
**95 of 95 surfaces audited · 41,223 of 41,223 lines · 100.0%.**
...
>
> 1. **A surface is audited, not perfect.** Many rows end in a recorded refusal or a measured
>    blocker. `app-alert-send-report-modal` is forty-six absent values and one decision;
>    `app-presentationarea` is missing the whole Recordings tab, blocked on an archive service.
>    Audited means the gap is *known and argued*, never that there is none.
...
`app-rec-preview` — **CLOSED 2026-09-01, and the way it closed is the point.** It read: *"the id and
class of the `<video>` in the reference's IN-PAGE preview card. Ours is a separate WINDOW... upstream
points its card at a server-supplied `recPreviewLocation` and there is no such URL here."* Both facts
