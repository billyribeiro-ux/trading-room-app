...
| 7 | `lib/components/PresentationArea.svelte` | 1,108 | `surface-audit-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-presentationarea` (292 consts, 125 embedded views), the page's second giant, scoped to the 94 files that compose it. Five const values and two literals remain and both groups are one thing each: the **Recordings tab** (`recordings`, `recordings-tab`, `#recordings`, `fa-file-video`, `"Recordings"`), blocked on an archive service neither database has a table for; `dropdownMenuNote` (`NTC-3`, a recorded divergence — the capture freezes the per-note gear's id, so two open notes would share one); and `"No room files found."` (the `#files` region's own refusal). Getting there took four corrections to the file list and two to the tool — the notes, swing-alert and day-trade-alert SUBDIRECTORIES were outside the glob, `ngForm` is a local reference found by POSITION now, and a 261-character tooltip is split across a `+` by prettier. §MTS — feeds `canEditNotes` to the tab strip (`MTS-02`). |
| 8 | `lib/components/RoomOverlays.svelte` | 1259 | `## RoomOverlays.svelte` in the v4 register — 7 gaps, read 2026-08-31. |
...
**95 of 95 surfaces audited · 41,112 of 41,112 lines · 100.0%.**
