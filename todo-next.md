...
| 4 | `lib/components/AlertChatArea.svelte` | 1542 | `surface-audit-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` as the root of `app-chat` (93 consts, 27 embedded views), scoped to the seven files that implement it: **CLEAN — zero absent const values, zero absent text literals.** Pinned with both counts asserted. The animated typing dots joined it the same day (`TypingIndicatorDots.svelte`), which had been refused on a reason that turned out to be half wrong. |
...
| 6 | `lib/components/RoomNavbar.svelte` | 1,133 | §NAV — 11 rows against `U4e` read whole: 4 built, 1 fixed, 3 measured refusals, 1 deliberate divergence, 2 blocked. `docs/decoded/room-surface-audit-2026-08-30.md`. |
...
**93 of 93 surfaces audited · 39,711 of 39,711 lines · 100.0%.**
