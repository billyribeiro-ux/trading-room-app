...
| 82 | `routes/+layout.svelte` | 27 | OURS — the SvelteKit shell, with no reference counterpart: upstream's equivalent is `deployed-index.html`, whose Font Awesome and animate.css come from CDNs. What is here is the font-loading decision and its measurement — `font-display: optional` was tried and reverted because it lets a browser skip a face for a whole page view, and the three woff2 files are preloaded instead (fonts at ~26ms, `loadingdone` at ~72ms, first contentful paint at ~84ms on the prod build). Read end to end 2026-09-01. |
...
**93 of 93 surfaces audited · 40,195 of 40,195 lines · 100.0%.**
