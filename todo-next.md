...
Added 2026-08-16 14:21 EDT, in answer to the owner's question _"todo-next.md should have everything
our app is missing and needs to be implemented in detail. did you get that right?"_
...
measured pixels is therefore _not auditable in this checkout_, and must say so rather than be
guessed.

## The inventory — all 101 surfaces, measured

|   # | surface                                                     | lines | audited against the reference?                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --: | ----------------------------------------------------------- | ----: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|   1 | `lib/components/ModalHost.svelte`                           |  6097 | `surface-audit-contract.test.ts` — audited as the set of reference modal selectors it composes. The Discord and public-player state machines were extracted on 2026-09-03 so this host stayed below its existing size ceiling.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|   2 | `routes/+page.svelte`                                       | 1,958 | `surface-audit-contract.test.ts`, `brand-attribution-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-room` (229 consts, 108 embedded views). Five const values and one literal remain and every one is on record.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
|   3 | `lib/components/notes/NoteEditor.svelte`                    | 1,656 | `surface-audit-contract.test.ts`, `note-editor-modal-labelling-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-note` (94 consts, 17 embedded views): **zero absent const values**, and the one absent text literal is a recorded refusal — `"Loading images..."` cannot arise here because `sessionImages` is a page-load prop where upstream POSTs `getSessionFiles` when the browser opens (`session-image-files.ts` carries the measurement). GIF-07 closed the last const: the Giphy picker is wrapped in `Modal.svelte` with the captured `modal-basic-title` id, `h4` title, `modal-lg` body at `max-height: 77vh` and the `btn btn-outline-dark` Close. That id had been recorded as unreproducible because `GiphyPicker` is mounted at four sites — a right count about the wrong element, since the other three mounts are popovers with no `modal-title` at all.                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
|   4 | `lib/components/AlertChatArea.svelte`                       |  1594 | `surface-audit-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` as the root of `app-chat` (93 consts, 27 embedded views), scoped to the seven files that implement it: **CLEAN — zero absent const values, zero absent text literals.** Pinned with both counts asserted. The animated typing dots joined it the same day (`TypingIndicatorDots.svelte`), which had been refused on a reason that turned out to be half wrong.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|   5 | `lib/components/RoomMessage.svelte`                         |  1271 | `surface-audit-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-st-message` (74 consts, 53 embedded views): **CLEAN — zero absent const values, zero absent text literals.** Pinned as a ratchet with both counts asserted. The largest clean surface in that contract.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|   6 | `lib/components/RoomNavbar.svelte`                          | 1,155 | §NAV — 11 rows against `U4e` read whole: 6 built, 1 fixed, 1 measured refusal, 1 deliberate divergence, 2 blocked. NAV-05 and NAV-11 were refusals until 2026-09-02: both are values the reference ships that paint nothing, and both are inert UPSTREAM for the same reason, which is a defect reproduced rather than an escape. `docs/decoded/room-surface-audit-2026-08-30.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
|   7 | `lib/components/PresentationArea.svelte`                    | 1,150 | `surface-audit-contract.test.ts` — READ END TO END against `app-presentationarea` (292 consts, 125 embedded views). The authenticated Recordings tab closed its former five-value gap on 2026-09-03; the only text residual is the recorded never-fetched files-message refusal.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|   8 | `lib/components/RoomOverlays.svelte`                        |  1258 | `## RoomOverlays.svelte` in the v4 register; rechecked after preserving the recording-preview host order on 2026-09-03.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
|  14 | `lib/components/ScreenPane.svelte`                          |   808 | `## ScreenPane.svelte` in the v4 register — Seven rows, read end to end on 2026-08-31 against the same pinned bundle, with                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
|  15 | `lib/components/PostAlertModal.svelte`                      |   670 | `post-alert-render.test.ts`, `send-later-contract.test.ts`, `post-alert-behavior.test.ts`, `sticky-non-trade-alert-contract.test.ts`, `alert-label-picker-contract.test.ts` — READ END TO END 2026-09-01 against `app-post-alert-modal`'s own const table (79 consts at byte 2,131,663, `decls:73 vars:24`) and all fifteen embedded views, across the four files that implement it (`PostAlertModal`, `ScheduledAlerts`, `ScheduledAlertFields`, `ScheduledAlertsTable`). Three const values and three text literals remain, all recorded refusals: Twilio SMS and the two client-asserted sender fields. The linked-room suppression left the residual set on 2026-09-03 when it gained a conditional composer control, validated payload, durable immediate/scheduled state and controller consumer. `PAM-12` records a divergence that was correct and unwritten: consts 8 and 9 both hardcode `aria-selected="true"` and nothing ever writes the attribute, so the reference announces two selected tabs — `MTS-06` one component over. `PAM-13` was MATCHED on 2026-09-02: the media guard now tests whether a file list EXISTS, as `fc` does, and the `filesTouched` latch mirrors its two assignment sites. The component's own four CSS rules ship in `captured-runtime-components.css` under the `<app-post-alert-modal>` host.                                   |
|  16 | `lib/components/ExtraChatPane.svelte`                       |   809 | §XCP — 9 rows against `app-extra-chat` read whole: **all nine answered as of 2026-09-02**, 4 built, 3 fixed, 2 re-dispositioned. The last two both had blockers that named the wrong missing thing — XCP-07 refused on a rule that governs invented classes rather than transcribed ones, XCP-09 blocked on a re-capture when the rules were in the pinned bundle. `docs/decoded/room-surface-audit-2026-08-30.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|  17 | `lib/components/day-trade-alerts/DayTradeAlertsPane.svelte` |   638 | `## DayTradeAlertsPane.svelte` in the v4 register — **5 rows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
|  18 | `lib/components/StreamingView.svelte`                       |   758 | `## StreamingView.svelte` in the v4 register — **10 rows, from one end-to-end reading of class `xCe` — bytes 1,901,122 to 1,914,468 of the pinned                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
|  21 | `lib/components/notes/NotesPane.svelte`                     |   537 | `## NotesPane.svelte` in the v4 register — 4 rows, read against `zSe` (byte 1,930,173) — the notes pane inside `app-presentationarea` — and its                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
|  24 | `lib/components/AlertQaModal.svelte`                        |   421 | §19.4, §19.3 — 10 items, 6 defects, 1 false comment, against the reference's 159-line `<app-alert-qa-modal>`. Plus §QAM, 13 rows from a whole re-read on 2026-08-31: 4 built, 2 fixed, 1 already built, 1 measured refusal, 4 blocked.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
|  30 | `lib/components/ScheduledAlerts.svelte`                     |   309 | `## ScheduledAlerts.svelte` in the v4 register — Read end to end on 2026-08-31 against the v4 bundle; `XTe`'s "See Scheduled Alerts" control transcribed 2026-09-01, and the three send-later fields moved out to `ScheduledAlertFields.svelte` (row 89) when that took the file past its ceiling.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
|  31 | `lib/components/PrivateChatComposer.svelte`                 |   374 | `## PrivateChatComposer.svelte` in the v4 register — Nine rows, read on 2026-08-31 against the pinned v4 bundle by bracket-walking `consts:[[` at byte                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
|  32 | `lib/components/StreamTabs.svelte`                          |   319 | `## StreamTabs.svelte` in the v4 register — Six rows, appended 2026-08-31.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
|  34 | `lib/components/MessageMenu.svelte`                         |   262 | §MSM — 6 rows against all four captured kebab menus: 1 fixed, 1 already built, 2 measured refusals, 2 deliberate divergences. `docs/decoded/room-surface-audit-2026-08-30.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
|  46 | `lib/components/ScheduledAlertsTable.svelte`                |   172 | `surface-audit-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-scheduled-alerts-modal` (17 consts, 2 embedded views): **CLEAN — zero absent const values, zero absent text literals.** Pinned as a ratchet, with the const and view counts asserted alongside so a parse that goes quiet fails rather than reporting green.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|  47 | `lib/components/Modal.svelte`                               |   190 | `alert-report-modal-contract.test.ts` — the shared Bootstrap dialog shell, and it has no reference component of its own: upstream repeats `modal fade` / `modal-dialog` / `modal-content` / `modal-header` / `modal-body` / `modal-footer` inline in every captured dialog, so this file is audited by every pin that names it. Twenty-three call sites, counted from the markup with a brace-aware scan; `ASR-2` and `ASR-3` are recorded here (the self-referential `aria-labelledby` upstream ships, and the focus-on-open Bootstrap gave the reference for free and this room ships no Bootstrap JavaScript to supply).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
|  48 | `lib/components/TabGearMenu.svelte`                         |   156 | `surface-audit-contract.test.ts` — the main tab strip's two cogs, consts 53/54/55 (`dropdownMenuNotes`) and 66/67 (`dropdownMenuFiles`) of `app-room`, which is pinned CLEAN with this file in its scope as of 2026-09-01. The component's own docblock decodes both const groups by value and records the one divergence: const 55's `aria-labelledby` is `dropdownMenuButton` where the cog's id is `dropdownMenuNotes`, so upstream's notes menu points at an element that does not exist, and this room passes the pointer as a prop rather than reproducing the mismatch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|  49 | `lib/components/AlertSendReportModal.svelte`                |    76 | `surface-audit-contract.test.ts`, `alert-report-modal-contract.test.ts` — authenticated request/cancellation and modal states. The durable report body moved to row 97 after the former no-ledger refusal was closed end to end.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|  50 | `lib/components/notes/NoteTabContent.svelte`                |   151 | `## notes/NoteTabContent.svelte` in the v4 register — 3 rows (NTC-01…03) read end to end 2026-08-31. NTC-01 removed an INVENTED value: `title="Welcome Mat"` occurs 0 times in the 2,891,205-byte bundle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
|  51 | `lib/components/LogArchiveModals.svelte`                    |   206 | `surface-audit-contract.test.ts`, `chat-archive-log-contract.test.ts`, `chat-archive-row-render.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against BOTH `app-chat-logs-modal` (38 consts, 11 embedded views) and `app-alert-logs-modal` (38/11), scoped to the five files that implement them: **CLEAN — zero absent const values, zero absent text literals.** Two findings got it there, both reported 0/0 by the whole-app sweep: `vxe`'s `By:&nbsp;<i>{createdBy}</i>` line, missing outright, and `Fxe`'s loading arm, drifted to `mt-2` and a `…` where the capture has `text-center my-4` and `" Loading..."`. The alerts half is still a PLACEHOLDER (its sweep is gated on `deleteAlertPW`); what is pinned is the chrome, which is one surface drawn twice. 3 divergences recorded (a duplicate `id`, the `btn-ligth` typo kept, the compact row standing in for `app-st-message`), plus the loading arm being a branch INSIDE the viewer where upstream's is a sibling of it.                                                                                                                                                                                                                                                                                                                                                             |
|  52 | `lib/components/BootboxDialog.svelte`                       |   212 | `bootbox-markup-contract.test.ts` — READ END TO END 2026-09-01 against the ONE capture of bootbox's rendered DOM this repository holds (`docs/reference/evidence-dumps-full-read.md`, `account-page/upload-image-badge-prompt.html`), plus the `bootbox.*` call sites in the bundle. **bootbox builds its markup in JavaScript, so none of it is in the Angular bundle** — `bootbox-close-button`, `bootbox-accept`, `bootbox-cancel` and `bootbox-input` all return zero there — and the one DOM capture is from the AngularJS ACCOUNT page on Bootstrap 3, not the Angular-17 room on Bootstrap 5. Three captured values are therefore REFUSED and the refusal is recorded: `data-dismiss` (BS3's spelling), `data-bb-handler` (bootbox's own dispatch hooks, which nothing here reads) and `btn-default` alone (kept BESIDE `btn-secondary`, not instead of it). The class names and the Cancel-then-OK order are transcribed.                                                                                                                                                                                                                                                                                                                                                                                                                                           |
|  53 | `lib/components/SessionHistoryPane.svelte`                  |   145 | Audited with `app-session-control-modal`. Public-player authorization and primary/backup media selection closed on 2026-09-03; remaining reference-only values are composed Bootstrap targets, Angular binding keys, WHIP reference literals, or the intentionally controller-only admin-login credential.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
|  54 | `lib/components/ChatArchivePane.svelte`                     |   205 | `chat-archive-row-render.test.ts`, `chat-archive-read.test.ts`, `chat-archive-log-contract.test.ts` — the archives BROWSER, read end to end 2026-08-31 against `jxe` (byte 2,309,873) and again 2026-09-01 against `vxe` (byte 2,301,700), the row itself. `vxe` is THREE labelled lines — a date, `By:&nbsp;<i>{createdBy}</i>` and `Channel:&nbsp;<i>{channel}</i>`, all three `<strong class="fw-bold">` — and this room drew one compressed line with **no `By:` at all**. Not a data gap: `chat_archives.archived_by_user_id` has been `notNull` since the table was added and no read path selected it, so the browser could not answer the question the column exists for. Transcribed, with the name joined from `users` in both reads and asserted against a real database. 3 divergences recorded (a duplicate `id`, the `btn-ligth` typo kept, the compact row standing in for `app-st-message`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
|  86 | `lib/components/ChatArchiveLogPane.svelte`                  |   259 | `surface-audit-contract.test.ts`, `chat-archive-log-contract.test.ts`, `chat-archive-row-render.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against BOTH `app-chat-logs-modal` (38 consts, 11 embedded views) and `app-alert-logs-modal` (38/11), scoped to the five files that implement them: **CLEAN — zero absent const values, zero absent text literals.** Two findings got it there, both reported 0/0 by the whole-app sweep: `vxe`'s `By:&nbsp;<i>{createdBy}</i>` line, missing outright, and `Fxe`'s loading arm, drifted to `mt-2` and a `…` where the capture has `text-center my-4` and `" Loading..."`. The alerts half is still a PLACEHOLDER (its sweep is gated on `deleteAlertPW`); what is pinned is the chrome, which is one surface drawn twice. 3 divergences recorded (a duplicate `id`, the `btn-ligth` typo kept, the compact row standing in for `app-st-message`), plus the loading arm being a branch INSIDE the viewer where upstream's is a sibling of it.                                                                                                                                                                                                                                                                                                                                                             |
...
|  92 | `lib/components/RecordingPreviewCard.svelte`                |   302 | `rec-preview-contract.test.ts` — `app-rec-preview` (byte 2,353,188) transcribed WHOLE on 2026-09-01: the arming test, the six subscriptions collapsed into three effects, the 1s frame timer, the drag, and both icons. It was eleven lines of markup with two inert icons until that day, on a recorded blocker that RE-MEASURING dissolved — `recPreviewLocation` is not a value this repository lacks, it is a value the SERVER sends, by one command (`setRecPreview`, byte 1,023,704) that had never been transcribed. With the receiver in place the component is transcribable end to end, and the fact that this room's server never sends the command is not a gap: `armed` stays false and the card stays dark, which is exactly what the reference does in a room whose server has not sent it. Mounted by `RoomOverlays` (not `ModalHost` — it is not a modal, and that host has neither `media` nor `prefs`). The contract MOUNTS and drives it: 26 cases, sixteen negative controls seen red, one of which found a real hole — the `reopenRecPreviewWindow` path's arming gate was untested.                                                                                                                                                                                                                                                                  |
|  93 | `lib/components/TypingIndicatorDots.svelte`                 |   110 | `surface-audit-contract.test.ts`, `typing-indicator-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-typing-indicator-dots` (1 const, 0 embedded views): **CLEAN.** Born the same day out of a recorded reason that was half wrong: both chat columns said the dots could not be drawn because `.typing-indicator` has no rule in any stylesheet here — true of the captured STYLESHEETS, and beside the point, because an Angular component's `styles:[…]` array is injected at runtime out of the BUNDLE. The array specifies the whole appearance (3px circles, `#9e9ea1`, resting opacity `.4`, a 1.5s blink staggered a third of a cycle apart) and is transcribed verbatim. One line is OURS: a `prefers-reduced-motion` block, the first in this application.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
|  94 | `routes/session-transcript/+page.svelte`                    |   471 | `transcript-authority-contract.test.ts`, `server/session-transcript.test.ts` — `app-session-transcript`, the transcript WINDOW, transcribed whole on 2026-09-02 from the component at bundle byte **2,607,394**: the date picker and its `yyyy-MM-dd` round trip, `formatDate`'s 12-hour clock, the client-side search over `text` and `speaker`, 300-row server paging, the five navigation buttons rendered twice, and the three-way loading/error/list state machine. It is the **50th and last** reference component `reference-component-inventory-contract.test.ts` was answerable for, and it moved from that file's `NOT_RENDERED` map to `BUILT_AS` rather than having its reason re-worded. The recorded blocker was _"building the STORE would be inventing a data source"_ — false: the data is this room's own relayed captions, and the reference's server holds a transcript too (`getSessionTranscripts`, byte 1,151,135). **One divergence, and it is a refusal**: upstream's URL carries `globals.sesionToken`, so ours carries no query string at all and the server re-derives the room, the caller and the room's NAME from the session cookie. Four negative controls seen red on the authority contract, one of which found a vacuous assertion — the archives gate's 403 was being satisfied by the fail-closed branch's identical message.         |
|  95 | `lib/components/SessionInfoModal.svelte`                    |   167 | `session-info-modal-contract.test.ts` — "Session Information", `getMyToken()` at bundle byte **2,255,348**, built 2026-09-02. Two `mb-3` blocks, each a `form-label` over an `input-group` holding a readonly `form-control` and a `btn-outline-secondary` with `fas fa-copy`; all of that chrome is transcribed by value. **HALF THE DIALOG IS REFUSED, on screen.** The Session Token field renders disabled and empty with its reason beside it: `globals.sesionToken` is the session credential, this room's cookie is `httpOnly`, and reproducing the field would mean the server writing that value into the DOM — turning a cookie an XSS cannot read into a string it can. The Session ID field IS real, and the reason it had been refused with it did not survive re-measurement: _"already visible in the address bar"_ is true of the reference (`globals.sessionID` = the `?id=` parameter, one assignment, byte 1,148,131) and **false of this room**, whose address is `/`. Two more divergences, both forced: upstream copies with an inline `onclick` inside its interpolated message — a stored-XSS primitive — and ends in `alert()`, which `CLAUDE.md` forbids by name. Four negative controls seen red, one of which caught a vacuous assertion of my own: the `disabled` check was being satisfied by the Copy button rather than the input it named. |
...
|  57 | `lib/components/ViewerAlertPrefsPane.svelte`                |   240 | `viewer-alert-prefs-contract.test.ts` — READ END TO END 2026-09-01 against the six reference templates this file implements: `tke` (byte 2,230,654), the four row views `GEe`/`KEe`/`XEe`/`eke` (2,228,857–2,230,278) and `HEe`, the positions row. **Three gaps, none of them recorded anywhere** — `d(3,"span",16),v(4,"Users join/leave:")`, the group's only visible name; `T(7,"hr")`, the rule dividing arrivals from departures; and a PER-ROW gate on each of the four (`O(5,…beepOnUserJoin…)`, `O(6,…userJoinAndLeavePopup…)`, `O(8,…)`, `O(9,…)`) on top of the group gate, which is an OR — so a room that enabled only the beep drew all four switches, two of them over a popup it had turned off. All three built, with three negative controls seen red. One divergence recorded: the `p-2 text-mode-box` around the positions row is OURS, because upstream draws it as the last of five siblings in a box the other four of which live in `ModalHost.svelte`.                                                                                                                                                                                                                                                                                                                                                                                             |
...
|  60 | `lib/components/ImageUploadDialog.svelte`                   |   185 | `image-upload-dialog-contract.test.ts` — READ END TO END 2026-09-01 against `imgUpload()`'s bootbox call (byte 1,442,225). **Two options never reproduced**: `backdrop:!0`, the `.modal-backdrop` element — this repo had already ruled that option rendered surface twice (`ROV-04`, `VID-01`) and this was the one dialog of the family without it, so nothing dimmed behind it (`app.css:1556` sets `.modal{background:transparent}` and `modal-open` occurs nowhere) — and `onEscape:!0`, which needed focus-on-open to work at all, the same `ASR-3` finding `Modal.svelte` records. Bound to the ELEMENT with `stopPropagation`, not to the window, because the room's global Escape ladder ends in `dialogs.alert` and a failed upload raises one.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
|  62 | `lib/components/UserNotesPane.svelte`                       |   124 | `user-notes-contract.test.ts` — READ END TO END 2026-09-01 against `mTe` (byte 2,065,327) and `fTe` (2,064,890), the two templates this file implements. **Two const values absent, neither recorded anywhere**: const 38 `[1,"row"]`, the row `mTe` wraps the notes LIST in — a bare `.col` carries half a gutter of padding that only `.row`'s negative margin cancels, so the list sat inset while the Add Note row below it, which HAS its row, did not — and const 96 `[1,"icon","fa","fa-plus-circle"]`, whose `icon` had been dropped. Four separate places here describe `mTe` as "a `col` scrolling at max-height 300px", so the row around it had never been read. Both built, two negative controls seen red.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|  63 | `lib/components/RestreamPane.svelte`                        |   105 | Audited with `app-session-control-modal`; production restream credentials remain presenter-only and server-projected.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
|  64 | `lib/components/ChatTabStrip.svelte`                        |   110 | `surface-audit-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` against `app-alerts` (54 consts, 13 embedded views): **CLEAN — zero absent const values, zero absent text literals.** Pinned as a ratchet with both counts asserted. Audited with the alerts column; the tab strip's consts are in that table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|  65 | `lib/components/CloseSessionPane.svelte`                    |   104 | Audited with `app-session-control-modal`; close/lock authority remains server-owned.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
|  66 | `lib/components/ReactionPrefsPane.svelte`                   |   125 | `reaction-notice-contract.test.ts`, `font-awesome-contract.test.ts` — READ END TO END 2026-09-01 against `REe` (byte 2,226,939) and `NEe` (2,227,411). **The INVERSE finding: markup this room INVENTED.** Both rows carried a header block — `<div id="appReactionsPopup" title="Reactions Response" class="pb-2"><i class="fas fa-face-smile"></i><span class="pl-2">Reactions Response:</span></div>` and its `-Qa` twin — where `REe`/`NEe` are `d(0,"div",17)` and nothing else. Bundle occurrences: `appReactionsPopup` 0, `fa-face-smile` 0, `"Reactions Response:"` 0, against `appDisableVideo` (the group immediately beside these two) at 1. And the icon drew NOTHING — `fa-face-smile` is Font Awesome 6 and this project ships 5.8.1. Deleted; the sweep that catches the icon half now runs over every component.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|  67 | `lib/components/ImageLightbox.svelte`                       |   168 | §ROV — `ROV-04`, the backdrop this dialog opened without.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
|  69 | `lib/components/RoomBranding.svelte`                        |    91 | `room-branding.ts` + its tests — READ END TO END 2026-09-01 against `changeFavicon` (byte 2,602,147), `addCustomCSS` (2,602,431) and the `globalsLoaded` caller (2,595,021), all three imperative rather than templated, so there is no const table to sweep. Every step is reproduced: the cache-bust, `id="dynamic-favicon"`, removing `link[type="image/x-icon"]`, and `createTextNode` for the inline stylesheet. ONE deliberate correction, argued in full: upstream's `e.indexOf("https") >= 0` is broken in both directions — an ordinary stylesheet containing `url(https://…)` is set as a `link.href`, and `http://…` is injected as CSS — so the value is PARSED here instead.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
|  71 | `lib/components/ImagePasteConfirm.svelte`                   |    76 | `bootbox-markup-contract.test.ts` — the `onImagePaste` confirm, which exists TWICE upstream (bytes 1,992,250 and 1,445,719) and builds the same body both times, differing only in the textarea the chat one appends. Read end to end when this file was born out of three copies in `RoomOverlays.svelte`, two of which had shipped WITHOUT `<h4>Upload this image?</h4>` (`dta-04`). The bootbox chrome around it is measured by the contract above, whose caveats about Bootstrap 3 apply here too.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
|  74 | `lib/components/GifConfirmDialog.svelte`                    |    84 | `bootbox-markup-contract.test.ts` — READ END TO END 2026-09-01 against the ONE capture of bootbox's rendered DOM this repository holds (`docs/reference/evidence-dumps-full-read.md`, `account-page/upload-image-badge-prompt.html`), plus the `bootbox.*` call sites in the bundle. **bootbox builds its markup in JavaScript, so none of it is in the Angular bundle** — `bootbox-close-button`, `bootbox-accept`, `bootbox-cancel` and `bootbox-input` all return zero there — and the one DOM capture is from the AngularJS ACCOUNT page on Bootstrap 3, not the Angular-17 room on Bootstrap 5. Three captured values are therefore REFUSED and the refusal is recorded: `data-dismiss` (BS3's spelling), `data-bb-handler` (bootbox's own dispatch hooks, which nothing here reads) and `btn-default` alone (kept BESIDE `btn-secondary`, not instead of it). The class names and the Cancel-then-OK order are transcribed. **One gap found and fixed:** this was the only bootbox dialog in the room with no `.modal-backdrop`, while `BootboxDialog` — serving the SAME `bootbox.confirm` — `ImageLightbox` (`ROV-04`) and `VideoPlayer` (`VID-01`) all emit one. An internal inconsistency rather than a transcription, and argued as such at the code.                                                                                                            |
|  75 | `lib/components/ToastHost.svelte`                           |    85 | `toast-host-contract.test.ts` — READ END TO END 2026-09-01 against ngx-toastr's toast component (byte 883,657) and the two containers its service builds imperatively (878,628 and 879,514). **One real divergence found: `@flyInOut` is named for a slide and defines none** — three opacity states and two `{{easeTime}}ms {{easing}}` transitions, which the defaults resolve to `300ms ease-in`, while this room's toasts flew in from 300px to the right. Corrected. The captured class ORDER (`${toastType} ${toastClass}`) transcribed with it; `closeButton` and `progressBar` both default OFF and are correctly absent; the keyboard reachability on the dismiss is OURS and is recorded as such.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
|  79 | `lib/components/RemoteAudioSinks.svelte`                    |    50 | `surface-audit-contract.test.ts` — the hidden `<audio>` per remote microphone, covered by the `app-room` pin (2026-09-01) which includes this file in its scope. `msRemAudio-{userID}` is the capture's own id and load-bearing: `adjustVol` does `$("[id^=msRemAudio-]")` at byte 2,517,022, `adjustVolPres` targets one peer's element, and `reconnectAudio` removes them all before re-subscribing — this room already queried that prefix against elements that had no id at all.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
|  81 | `routes/logout/+page.svelte`                                |    31 | OURS — no reference counterpart, and the absence is the point: the capture has no logout control anywhere, so the room's shell markup would stop matching if one were added to the navbar. Its own comment records that, and why the control is a POST form rather than a link (no prefetch and no stray GET can end a session). Read end to end 2026-09-01; nothing here is transcribed and nothing here should be.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
|  82 | `routes/+layout.svelte`                                     |    26 | OURS — the SvelteKit shell, with no reference counterpart: upstream's equivalent is `deployed-index.html`, whose Font Awesome and animate.css come from CDNs. It now holds only the stylesheet import and the record of where font preloading went: the three hand-written `<link rel=preload as=font>` tags were a SECOND mechanism beside `hooks.server.ts`'s `preload` filter, so each face was preloaded twice per page and the two lists had drifted (three here against the filter's four). `font-display: optional` was tried and reverted because it lets a browser skip a face for a whole page view. Re-read 2026-09-02.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
|  96 | `routes/player/[room]/+page.svelte`                         |    66 | `stream-player-blocked-contract.test.ts` — screenshare-only HLS player backed by an expiring, revocable capability; no chat, notes, files or private-room credential reaches it. Route CSS is isolated in `player.css` so the component stays below its source ceiling.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
|  97 | `lib/components/AlertDeliveryReportBody.svelte`             |   198 | `surface-audit-contract.test.ts`, `alert-report-modal-contract.test.ts` — all 39 report consts, search/status filtering, delivery summary and authoritative recipient outcomes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
|  98 | `lib/components/DiscordSettingsPane.svelte`                 |   102 | Presenter-only OAuth status/connect/unlink surface. The controller stores provider identity only and never persists an access token.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
|  99 | `lib/components/PublicPlayerPane.svelte`                    |   119 | `stream-player-blocked-contract.test.ts` — issues and revokes one-time bearer links and never exposes the short-lived media token in room state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 100 | `lib/components/RecordingArchivePane.svelte`                |   330 | `main-tab-strip-gates.svelte.test.ts` plus recording storage tests — paginated catalog, authenticated range playback, immutable chat/alert log snapshot, transcript download and presenter deletion.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 101 | `lib/components/RecordingTab.svelte`                        |    31 | `main-tab-strip-gates.svelte.test.ts` — exact reference tab chrome, archive entitlement gate and roving tabindex behavior.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |

**101 of 101 surfaces audited · 41,950 of 41,950 lines · 100.0%.**
...
> contract that keeps it read. That is the claim, and it is worth stating the four ways it is
> narrower than it sounds — because a coverage number nobody bounds is the next thing to go stale.
>
> 1. **A surface is audited, not perfect.** Some rows end in a recorded refusal or a deployment-only
>    acceptance step. The former alert-report and recordings blockers closed on 2026-09-03.
>    Audited means every remaining difference is known and argued, never that none exist.
> 2. **Two rows are pinned by a GLOB and are weaker for it.** `app-room` and `app-presentationarea`
>    scope to every component on the page, and the audit counts a value present if it occurs in ANY
>    listed file — a control that moved a label out of `RoomNavbar.svelte` left `app-room` green.
>    Both pins record that measurement themselves.
> 3. **Four rows have no Angular selector to point the tool at.** `RoomBranding`, the three bootbox
>    dialogs, `ToastHost` and the two `routes/` files were read against imperative code, a
>    Bootstrap-3 DOM capture from a different application, or nothing at all where the surface is
...
1. **It is bounded by evidence, not only by effort.** With the DOM captures absent, a surface can be
   audited against the bundle's _logic and literals_ and no further. Every gap recorded from here on
...
The method in the paragraph above — _transcribe every const by value, then measure ours_ — was
...
| real gaps on built surfaces             | `app-alert-send-report-modal` (15), `app-post-alert-modal` (7), the two log modals (6 each), `app-room` (6), `app-user-settings-modal` (5), `app-presentationarea` (5), `app-chat` + `app-extra-chat` (1 each), `app-screenshare-view` (2), `app-poll-modal` (1) | the work this sweep found                                                                                                                     |

### A residual is not the same thing as work, and the file measures the difference

The fourth group was first written as _"the work this sweep found"_. Checking that before saying it is
...
Stream Player tab's legacy readonly-link id; the replacement is the revocable, capability-backed
`PublicPlayerPane`) and `audioID`/`videoID` (Angular `ngModel` binding keys; ours carry the ids and
...
`app-rec-preview` — **CLOSED 2026-09-01, and the way it closed is the point.** It read: _"the id and
class of the `<video>` in the reference's IN-PAGE preview card. Ours is a separate WINDOW... upstream
points its card at a server-supplied `recPreviewLocation` and there is no such URL here."_ Both facts
were true; the conclusion was not. `recPreviewLocation` is not a value this repository lacks — it is a
value the SERVER sends, by one command (`setRecPreview`, byte 1,023,704) nobody had transcribed. With
the receiver in place the whole card is transcribable, and this room's server never sending the command
makes the capture's own arming test fail, which is what a reference room does in the same state.
...
> _"we're not building anything yet. and we wont until all the documentation is properly done and in
...
**Owner clarification, 2026-08-16:** _"we're not removing anything right now. That will only get
done after we finish the implementation phases."_
...
1. **Do NOT prioritise the reading queue by deletion risk.** §16.10's register (11 evidence dirs,
   ...
   _pre-deletion checklist for later_, **not** a reason to read `account-page` before something that
...
**Owner plan, 2026-08-16: _"as soon as we close all the gaps and implementation i will get rid of
every single node dump, file reference and everything else and then get a fresh pull to then audit
against ours. That will avoid conflicts and make errors easy to pick on if any."_**

**That is the right plan, and it changes the standard for every line written from here on.**

|                       |                                                                                               |
| --------------------- | --------------------------------------------------------------------------------------------- |
| ❌ **not sufficient** | _"`restoreMobileAppTokens` at byte 2,444,920"_ — the file it points into will not exist       |
| ✅ **required**       | the **verbatim string, value, class list, payload and gate**, transcribed into the doc itself |

**These documents must stand alone after every capture is deleted.** A byte offset is a _provenance
note_ — keep it, it proves the claim was read rather than guessed — but **it can never be the only
...
ordered by _what unblocks implementation_, not by what is at risk of deletion — deletion happens
...
1. **Evidence is READ, never searched.** Locating with a tool is fine. **Concluding** from a tool's
   ...
   rebuilding. _This session alone: grepping the manage page for `FCM|Notif` found two settings;
   READING the same region found three alert-delivery channels and a JWT revocation list, none of
   which contain those letters._
...
3. **Rule out your own tooling BEFORE reporting a failure.** _This session: a bundle comparison
   ...
   opposite of the truth._ If a check fails, first prove the check is right.
...
5. **Nothing exists without a consumer.** No class with no CSS, no control whose only effect is
   ...
   6b. **⭐ A ZERO FROM A RENDERED CAPTURE IS A STATEMENT ABOUT STATE, NOT EXISTENCE.** _Owner
   instruction 2026-08-16: "sometimes it may show 0 but its not because it doesn't exist, because
   there was a member or an extra admin added."_ **A count of 0 is only evidence when the
   precondition that would produce a row is known to have been met.** State the precondition or the
   zero says nothing. _Earned: I recorded `adminUserRemove` count 0 as an unknown when
   `removeAdminUser` is demonstrably in `app.min.js` with its confirm string and payload — the
   account simply had no extra admin users to render a row for._ See §15.8.
   6c. **⭐ AUDIT OUR IMPLEMENTATION AS YOU READ, NOT AFTER.** _Owner instruction 2026-08-16._ Every
   reference control gets checked against `apps/controller/src` and `apps/room/src` **in the same
   pass that decodes it**. _Earned: §15.8's eleven Account-page controls read as a gap list until
   audited — **all eleven are already built**, one of them (`listAdminUsers`) under another name._
...
6. **⭐ NOTHING IS MISSING WITHOUT A CHECK.** A gap is _present in the reference_ **AND** _absent from
   our source_ — **both halves need hard evidence.** Before writing anything down as a gap or a
   finding, grep our own tree for it. _Owner instruction 2026-08-16, and it caught a live failure:
   ...
   over parts of it. See the red block at the head of §10._ Rule 5 says nothing exists without a
...
- **P-1's root cause — EXHAUSTIVE, not sampled.** The push decision chain has **eight gates** and
  **not one reads subscription state** (§8.2). The entitlement data _exists_ on the member row —
...
| the three `ptrMobileAppCaseByCaseEnabled` branches           | _"App for Some Members?"_ → `manageMobileApp` `enable`/`disable` + `user.hasMobileApp`                                                                                                | §8.1, §8.4 |
| what `customMobileAppLaunchWord` does                        | the **custom URL scheme** for a white-labelled app                                                                                                                                    | §6.9       |
| what `customMobileAppV3Name` does                            | _"Custom app String"_                                                                                                                                                                 | §8.4       |
| what `sendFcmAlertsNew` switches between                     | _"Use pub/sub for notifications"_ — **the design risk §0 named is REAL**; if it means FCM topics, per-member revocation is structurally impossible. Server behaviour still uncaptured | §8.4       |
...
looked for `dt({`, the helper name in the _decoded_ files. The raw v4 bundle uses `ut({` — the
...
- [x] **Re-decode the 51 components from `source-v4-2026-08-15/main.d1d09071be31f1ba.js`** into a v4
      ...
      inside `pnpm test`; add alongside. **Closed 2026-09-03:** 51 components / 194 generated
      artifacts now live under `docs/source-v4-2026-08-15/components/`; the archived corpus is
      untouched.
- [x] **Re-point `pull-everything-contract.test.ts`** at the v4 bundle, keeping the "finds every
      `selectors:[` rather than a hardcoded list" property.
- [x] **Read `app-presentationarea`'s +299 bytes.** Closed by the current v4 register and the
      end-to-end `PresentationArea.svelte` row in the 101-surface inventory above.
- [x] **Promote the register's nine `MATCH` verdicts to v4.** Closed by the v4 register and its
      executable per-surface contracts; the current inventory is 101/101 surfaces.
...
<div class="mobile-app-container">
  <p class="text-muted mb-4">
    Use this to restore your mobile app connectivity and get a test notification
    on your device. Only do this if you are not getting notifications
  </p>
  <button
    type="button"
    class="btn btn-primary"
    (click)="restoreMobileAppTokens()"
  >
...
- [x] Build the third tab against the v4 bundle, including the ungated behaviour **or** a recorded
      decision to gate it (`mobile-app-decoded.md` §3 row 26 calls it "the one inconsistency in an
      otherwise uniformly-gated feature"). Closed by `MobileRestorePane.svelte`; §2.5 records the
      deliberate availability gate.
- [x] **Resolve and build the server half.** The live empty acknowledgement is captured in §3a;
      `mobile-pin.remote.ts`, `room-config-client.ts`, and controller `internal/mobile-restore`
      implement an authenticated result-bearing equivalent rather than reproducing the false
...
> below is still the only _automatic_ stop — that part holds — but it is one gate of eight, not the
...
| ---------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------ | ------- | --------- |
| `mobileAppExpireNotificationsDays` | **Push expire days** | _"If user does not log in this many days, we'll stop sending push notifications"_                | **14**  | **false** |
| `ptrMobileAppExpirePairCodeDays`   | **PTR app exp days** | _"If user does not log in from regular site, mobile app token will expire after this many days"_ | **7**   | **false** |
...
**External legacy production operations — not repository implementation:**

- Unsubscribe already-lapsed members in the legacy system (`pauseUserNotifs('unsub')` /
  `resetFCMForuser`). This checkout has neither authority nor credentials to mutate that system.
- Choose and set a temporary legacy **Push expire days** value below 14. The numeric policy is an
  owner decision; this rebuild now enforces its configured value at send time.

**This rebuild — get it right before the fan-out exists:**

- [x] **Check `notificationsState` at SEND time.** `deliverySuppressionReason` reads it before any
      FCM call; paused, unsubscribed, or `alerterAppFcmUserOff` members are persisted as suppressed.
- [x] **Gate on entitlement at SEND time, not entry.** Every dispatch re-reads membership, room-app,
      free-trial, case-by-case app, marketplace billing status, and paid-period facts.
- [x] **Consume billing revocation at the send boundary.** Marketplace states other than `active`
      and `trialing`, including `past_due`, `canceled`, `unpaid`, and `incomplete_expired`, fail
      closed. The provider event that refreshes those stored facts is deployment integration, not a
      reason to leak while it is present.
- [x] **Wire login-decay LAST, as a backstop only.** The configured captured rule defaults to 14
      days and runs after explicit membership, preference, app, and billing gates.
- [x] **Never revoke by deleting tokens** — only a registration FCM itself disowns is deleted.
      Suspension belongs in `notificationsState`.

**Row Q in `TODO.md` is adjacent and is NOT this.** Q proves the _web entry door_ closes after
...
~39 ms _before_ the real reply arrives and would say the same thing if the server had refused.
...
_(This continues §2. It sits here because it was captured after §3a, and the sections are kept in
the order the evidence arrived rather than reshuffled — a renumbered document is one where the
citations stop matching the session that produced them.)_
...
```html
<li role="presentation" class="nav-item">
  <button type="button" role="tab" class="nav-link">
...
<li role="presentation" class="nav-item">
  <button type="button" role="tab" class="nav-link active">
...
| ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| **`twillioApiToken`, `twilioPhone`** (labels _Twillio Token_, _Twillio Phone_)             | **A SECOND ALERT DELIVERY CHANNEL — SMS.**                                                          |
| **`protextingSecretTok`, `protextingGroupIDs`** (_Protexting Token_, _Protexting GroupID_) | **A THIRD one.** Protexting is a bulk-SMS provider, keyed by group.                                 |
| **`invalidTokens`** — _"Comma separated list of invalid JWT tokens."_                      | **A JWT REVOCATION LIST.** Directly a P-1 mechanism, and its name contains no push/mobile/FCM word. |
| `useV5` — _"Use v5? (DON'T!)"_, `useV3` — _"Yes!"_                                         | **There is a v5.** The captured room runs v3 on this flag.                                          |
| `superClusterID`, `superClusterExpectedServerCount`                                        | supercluster scaling — _"scale the session across the super cluster"_                               |
| `useFFmpegRecording` (BETA), `useLessBusyVsRoundRobin`                                     | recording + load-balancing, both relevant to `TODO.md` rows R and X                                 |
| `customPlayerURL`                                                                          | _"always show an iframe with this url in the screens section"_                                      |
...
- [x] **Superseded as an intermediate task:** later §8 read the complete settings surface and row
      menu, the current extractor enumerates all 269 settings, and the v4 101-surface register pins
      the client corpus. The partial 7% statement above is retained as historical context, not the
      current audit state.
- [x] **Superseded by the same complete evidence pipeline** for
      `evidence-dumps/NEXT-STEP/gaps/rawHtml.html` and the `login-page/*` siblings; those live
      captures remain outside the public repository, while their sanitized/extracted facts are
...
> _"by elimination the FCM settings must be in lines 2–8."_ They were never in the file at all — the
...
>
> ```js
> $scope.makeReqTokenForCmd = function (cmd) {
>   var tok = $localstorage.get("tokenSite"),
>     args = {};
>   return (
>     (args.token = tok),
>     (args.cmd = cmd),
...
1. **`showAlerterAppTokens` calls no server.** `docs/MOBILE-APP.md` lists it among the row-menu push
   ...
   the roster payload contains them and `getFCMTokens` exists to fetch something the row does _not_
   have. Those are two different token sets, or two different freshnesses. Not yet resolved.
2. **`restoreMobileAppTokens` appears 0 times in this bundle.** It is the v4 _room_'s command and has
...
$scope.manageMobileApp = function (xrefid, name, $index, mode) {
  ...
  args &&
    ...
    (args.appcmd = mode),
    $http
      .post(appVars.globals.APIURL + "/users/v1/sessions", args)
      .success(function (data) {
        data.success
          ? (bootbox.alert(
              "Mobile App command OK for: " + name + ". (" + mode + ")",
            ),
            $scope.loadUsers())
          : bootbox.alert(
              "ERROR: Mobile App command failed. Error:" +
...
| ----- | --------------------------------------------------------------------------------------------- |
| **0** | _never assignable_ — every bulk loop skips `0 === user.role`, and the select-all skips it too |
...
`xrefId`/`sessionId`, _different from the `xrefID`/`sessionID` used everywhere else_. **So there are
...
`$scope.userPermissions.userName` — a variable set by a _different_ button — so **the reference
...
- **Four password endpoints, two identity systems:** `/users/v1/change-password` and
  `/users/v1/forgot-password` for the _site user_, `/users/v1/user-change-password` and
  `/users/v1/user-forgot-password` for the _xref user_. All take `recaptcha` and `source:"webApp"`;
...
- **No `lastLogin` / entitlement gate anywhere in it.** Every push action here is an _admin acting on
  one member by hand_. Nothing in this slice ties any of them to subscription state — consistent with
...
`makeTempPresenter(perms)` sends `"a"` (Presenter/Admin — _"see the roster, view user info, open/close
the room, as well as present"_) or `"p"` (_"share screens and talk on the mic"_). The broadcast
...
> second one — _"a regular user with no presenter controls?"_ — is unreachable dead code; it was
...
key _suffixes_, not key names. It also means **nothing in localStorage is shared across rooms**,
...
this.send = function (type, data) {
  ...
  service.socket.send(JSON.stringify(m));
};
```
...
**Consequence for everything in §6.2:** the ~26 `chatModel.send(...)` command _names_ are evidence,
the _envelope_ is not. Do not build a v4 room command from this file's envelope — §3a's capture is
...
u = {
  ...
  jwtToken,
  jwtTokenSite,
  cver: appVars.globals.ver,
  isM: appVars.globals.isM,
  apiV: service._apiV,
};
```
...
**P-2 cannot be built on it.** This repository's standard is explicit: _every authority decision is
made on the server from data the server owns — never asserted by the client, ever_. A device limit
...
if (bnd == service.roomID)
  return void appVars.doAlert("You have been banned from this room…", true);
```
...
// FilesCtrl
$scope.makeReqTokenForCmd = function (cmd) {
  var tok = $localstorage.get("token"),
    args = {};
  return (
    (args.token = tok),
    (args.tokenSite = $localstorage.get("tokenSite")),
    (args.cmd = cmd),
...
- `appVars.globals.isM && isPresenter()` → **`nonPresenter = true`**, logged as _"forcing nonPresenter
  admin on mobile app"_. A presenter on the mobile app is demoted to a viewer-with-admin-rights.
...
is defeated by clearing site data. Recorded as the pattern to _replace_, not to copy.
...
after position 0. Note `> 0`, so a message that _starts_ with `?` is not a question.
...
- `getChatlog` → keep own messages, plus admin messages that contain **no** `@` at all, plus admin
  messages containing `"@all "` or `"@<my nick> "`. An admin message mentioning _someone else_ is
...
- **Temp presenter:** `makeTempPresenterOn` / `makeTempPresenterOff` move the user between rosters by
  ...
  acts on _me_: `"a"` or `"p"` promotes and sets `isTempPres`, with **`isLimitedP = (perms == "p")`**;
...
- **Debug:** `enableRLog` · `getRLog` → the client replies `getRLogResp {uid, log: appVars.getLog()}`
  ...
  sent it).
- **Auto-record:** `sessData.autoRecord` starts recording when _I_ start talking while screensharing
...
service.roomState && service.roomState.isRecording
  ? ((service.isRecording = !0),
    lg(
      "getRoomState setting is recording to true. recUser:" +
        service.roomState.recUser.nick,
    ),
...
this.hangUpDialOutPhone = function (origID) {
  this.send("hangUpDialOutPhone", { orig_id: origID });
};
```
...
this.isPresenter = function () {
  return "a" == this.perms || "o" == this.perms;
};
this.isViewer = function () {
  return "r" == this.perms || "l" == this.perms;
};
```
...
- **Bitrate:** default `512000`; a requested rate above `sessData.media_max_bitrate` is clamped
  **unless `sessData.hqVideo`** is set. `toggleScreenShare(1)` means "alt constraints" and pins 512000. Webcam publishes at a fixed `1024000`.
...
- Auto-unmute after a disconnect is **present but deliberately disabled** — the branch logs
  _"auto starting audio unmuted after disconnect...DISABLED..."_ and does nothing. Auto-restart of
...
- Firefox instead gets a _"Share whole screen or a window?"_ dialog setting `capture` to `"screen"`
...
pin: globals.audioPin}` — the audio room is **PIN-protected**, and the pin arrives on `setPorts`
  ...
  `everUnmuted`.
- **Audio autoplay is handled properly:** a rejected `play()` promise raises _"Your browser needs your
  OK to play the room's audio"_ and replays on the click. Worth keeping — it is a real browser
...
- **`sessData.newServersideRec` disables the whole client-side forwarder path** — the recording
  `getOpenPortResponse` and `recStopped` handlers return early with _"doing this serverside"_. So
...
data.soundCloudURL &&
  (wireUpSoundCloud(encodeURIComponent(data.soundCloudURL)),
  ($scope.scClass = "#FFFFFF"));
```
...
mechanism; the room-switching _feature_ is fine, the transport is not.
...
roster. The alert text — _"if you get a Voicemail vs a person"_ — makes the intent explicit: a
...
- `custLogoutURL` is where `doHangup` sends you after clearing `token`, `tokenSite` and the storage
  namespace; without it the user is told _"You can now close this browser window / tab"_.
...
Notification API, so the _desktop_ half of "stop sending alerts" may have a second delivery path
...
_"Sorry, `<name>` is too large!"_; the resulting chat message is literally
...
_"Upload this image?"_ before sending), and the picker.
...
msgTxt = $($.parseHTML(msg.text))
  .text()
  .replace(/\bhttp[^ ]+/gi, urlwrap | urlwrapImg);
```
...
- `startLoadedMoreTimer` then polls **every 5 s** and, as soon as the user is no longer scrolled up,
  re-renders the whole log to _"delete chat bloat"_ — i.e. the expansion is temporary by design.
...
$scope.launchPTRApp = function () {
  ...
  appVars.sessData.customMobileAppLaunchWord &&
    (launchKey = appVars.sessData.customMobileAppLaunchWord + "://");
  var href =
    launchKey +
    "?t=" +
    $scope.appObj.al +
...
- **`customMobileAppLaunchWord` is the white-label override**: a tenant with their own app in the
  stores gets their own scheme, and the setting holds the _word_, with `://` appended by the client.
...
```js
chatModel.send("typing", {
  c: chan,
  pm: $scope.isPrivateChat,
  pu: $scope.privChatUID,
});
chatModel.send("notyping", {
  c: chan,
  pm: $scope.isPrivateChat,
  pu: $scope.privChatUID,
});
...
$scope.usersTyping = str;
$scope.usersTypingCnt = len;
```
...
| _anything else_        | —                        | `$scope[type] = val` and persisted under the same name                                     |
...
```js
var iosURL =
    "https://itunes.apple.com/us/app/pro-trading-room/id1147825433?ls=1&mt=8",
  androidURL =
    "https://play.google.com/store/apps/details?id=com.bellesoft.protradingroom",
  iosAlerterURL =
    "https://apps.apple.com/us/app/pro-trading-room-alerts/id1542468993",
  androidAlerterURL =
    "https://play.google.com/store/apps/details?id=com.bellesoft.ptrAlerter";
```
...
**This reframes P-1.** The owner's problem — _"members continue to receive alerts on their app"_ — is
...
```js
$scope.appObj = {
  pairCode: chatModel.pairCode,
  roomID: chatModel.roomID,
  myemail: chatModel.myemail,
  al: chatModel.jwtToken,
  mobileAppURL: iosURL,
  mobileAppURLAndroid: androidURL,
  alerterAppURL: iosAlerterURL,
  alerterAppURLAndroid: androidAlerterURL,
};
```
...
_"Here are your mobile app credentials:\n\nRoomID: …\nPair Code:…"_ plus both store links —
...
| ---------------- | ------------------------------------------------- | ------------------------------ |
| _(always)_       | the room's alert pane                             | —                              |
| _(default on)_   | **push to the mobile app(s)**                     | suppressed by **`dontPush`**   |
...
- Guard: refuses a date in the past; confirms with _"Send this alert on: … send as: …"_.
...
- **`changeSessionLock`**: _"all regular users will be kicked out of the room and will not be able to
  login till you unlock the session."_ `sessData.isLocked` raises a 15-second warning toast on join.
...
- Members are blocked from admin channels client-side:
  `extraAdminChannels.indexOf(chatChannel) != -1` → _"Sorry, regular users can't post here..."_
...
- **`doUpV {uid, time, idx}`** — you cannot upvote your own message, each index only once, and
  **one upvote per 30 seconds** globally: _"Take it easy, only 1 upvote every 30 seconds ;)"_.
...
- **`sessData.privMessageHugePopup`** — for **non-presenters**, an incoming PM triggers a `$.blockUI`
  full-screen takeover: _"Private message from: `<nick>`"_, the message body, and a flashing
...
> **Reference bug:** `addWebcamStreamError` offers _"Do you want to take over the webcam?"_ and calls
...
**This sharpens P-1's root cause decisively.** §3 concluded _"the reference stops push on `lastLogin`
decay, not subscription state"_ — true, and now the reason is visible. It is **not** that the
...
```js
var baseUrl = "/ptr_app/mp/v2",
  api_url = appVars.globals.APIURL + baseUrl;
$http.get(api_url + "/marketplaces/" + sessID, {
  headers: { Authorization: "Bearer " + chatModel.jwtTokenSite },
});
...
| --------------- | -------------------------------------------------------------------------------------------------------- |
| `jwt`           | _"SSO (JWT) - Single Sign On using Wordpress/PHP/Other JWT method"_                                      |
| `open`          | _"Anyone with the room link can join with their email & name"_                                           |
| `registrationA` | _"WEBINAR STYLE - Users get sent a unique link to join automatically after they register"_               |
| `registrationM` | _"Registraion with MANUAL approval. You approve each registration before users get a unique link"_ (sic) |
| `webinarRoom`   | _"Unique link & password (optional)"_                                                                    |
| `unamePW`       | _"Unique email/pw for each user"_                                                                        |
| `closed`        | _"Closed - Nobody can use this room while in closed mode"_                                               |
...
`list` · **`changeCustomURL`** (`cname`, stripped to `[a-z0-9]` — _"Only letters and numbers allowed…
as this is a URL"_) · **`uniqueLinkLogin`** · `statsForSession` · **`updateClusterID`** (applies the
cluster to **all** sessions) · **`cloneSession`** (`name` → redirects to
`/ptrApp#/page/manageSession/<id>`) · **`deleteSession`** (type `yes`; _"This is not undoable, all
data will be lost"_) · `sessionPubData` · **`clearUserList`** (_"delete all non-presenter users"_) ·
`loadTextList`.
...
`DetectRTC`-based, run on `sessionPubData`. **Blocked:** Edge (all versions — _"Edge broken for
now"_), old Edge, **Firefox < 52**, **Chrome < 50**, IE without the plugin, and anything reporting
`isWebRTCSupported === false`. **Bypassed entirely** when `pubSessionData.ptrMobileAppEnabled` and the
device is iOS or Android. Message: _"WARNING Unsupported/Outdated Browser. Please use a more recent
version of Chrome, Firefox, or Opera"_ — with a `strictBrowserMode` session setting alongside it.
...
§6's read-log said, repeatedly, _"by elimination `sendFcmAlertsNew`, `invalidTokens`,
`diasableFCMAlerts`, Twilio and Protexting must be in lines 3–8."_ **Lines 2–8 have now been read end
to end and none of them is there.**

**Verified both ways, 2026-08-16 09:50** — counts taken _after_ the full read, as corroboration of it
...
> ⚠ **A second correction, to this correction.** An earlier version of this paragraph blamed _"the
> pull script's census, which only records non-zero probes."_ **That explanation is false and has
...
$scope.saveSessField = function (fieldName, fieldVal, loadingSettings) {
  ...
  if (args)
    return (
      ...
      (args.fieldVal = loadingSettings
        ? fieldVal
        : fieldVal || $scope.sess[fieldName]),
...
fieldVal}`. So a setting's _name_ never appears in the JavaScript — it appears in the **manage page's
...
> slices to the first 10,000 and then filters for users _with_ tokens** — returning exactly the
...
- `clearUserListFT` follows up with _"Don't forget to change the Free Trial Password in the settings
  tab"_ — so free-trial access is a shared password.
- **`updateUserXref` gains two more modes here:** `inviteStatus` (an approval workflow — "Approve" or
  _"pause / set to Pending"_), and `newBadges[]` + **`allRooms`**.

> ⚠ **Command/label mismatch worth checking before porting.** The button labelled
> _"remove ALL badges from ALL users in the room?"_ sends **`clearTokensForUsers`**. The name says
...
restriction dialog is two scrollable checkbox lists headed _"Restrict to Sessions (leave none selected
for no restriction)"_ and _"Restrict to Endpoints (…)"_ — **so an unrestricted key is all-access by
...
can nominate a _different badge id_ to render in the dark theme.** All return `data.badges`.
...
sessionID}` — the confirm text spells out the semantics: an empty `pw` means _"system will assign a
  random pw"_, and `sendEmail` controls _"Email Credentials: Yes/No"_.
...
> _load_ settings serialises `$scope.sess` and **downloads it**. It has never imported anything. A menu
...
`tokcmd` modes (`pause` / `resume` / the one labelled _Remove_, i.e. `unsub`), but the _argument
strings_ are in the `ng-click` attributes, which this summary quotes labels from rather than markup.
...
stopped getting alerts" is _an admin bumping the timestamp_, which also silently re-arms the
14-day leak.
...
| ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _"`makeReqTokenForCmd` — the permission gate every request depends on. Not in the slices read."_ | **Found — §6.12.** `LoginCtrl`'s version: `{token: localStorage["<roomID>.tokenSite"], cmd, source:"webApp"}`                                                                                                                                                                                                                      |
| _"whose falsy return is the client-side permission gate"_                                        | **❌ That claim is wrong.** All five definitions found (§6.2, §6.3, §6.11, §6.12) **build and return an object unconditionally.** It can never be falsy. Every `args && …` guard in the codebase is dead code, and **there is no client-side permission gate here at all**                                                         |
| _"`__al` — the page global holding the JWT. Not defined in the slices."_                         | **Found — §6.12.** Injected server-side into the page; `jwt_decode(__al)` yields `{email, name, permissions, sessions, type, perms}`. Assigned to `chatModel._al` and sent on socket login                                                                                                                                         |
| _"`otherJWTSessions` — built server-side; whether it can span tenants is unknown."_              | **Narrowed — §6.12.** It is `alObj.sessions`, i.e. **carried inside the JWT**, not fetched. So the question is not "what does the server build" but "**who signs that token and what do they put in `sessions`**"                                                                                                                  |
| _"The Marketplace pane. Never opened."_                                                          | **API shape found — §6.12.** `GET/POST {APIURL}/ptr_app/mp/v2/marketplaces/<sessID>` with a bearer token; entry shape `{sessionID, title, logoURL, published, locked, featured, hidden, tagline, description, images[], tags[], memberships[]}`; dashboard at `/mp/room-dashboard/<mpId>`. The **pane's markup** is still unopened |
...
- **Three deny-by-default allow-lists:** `allowedMemberships`, `sess.allowedProducts`,
  ...
  it already exists for _entry_.** The question P-1 turns into: why does it gate the door and not the
  alert fan-out.
- **🟢 `sess.linkedStreamsAPIKey` — this is how `sendLinked` works.** _"For pushing alerts and streams
  to other rooms, you can use the following settings. You need the other rooms ID and the API Secret
  of the other room to do this."_ **Shared-secret federation between two rooms.** §6.10 flagged
...
- **`modAdminLoginList`** — _"put any emails here of admins you want to allow access to the admin
  panel section"_. **An admin panel exists beyond the manage page**, gated by an email allow-list.
  With §6.13's `/users/v1/adminusers` table, that is two separate admin-access mechanisms.
- **Seat quota `1 / 2`** in the account page's Users column, and _"these counts are just for
  information purposes, does not affect your room's **limits**"_ — **an authority above this page
...
- **v3/v5 provisioning is product copy with no control:** _"(DON'T TURN THIS ON, If PTR did not clear
  you for v3!! it will not work….)"_ — an operator→tenant grant that the tenant UI describes but
  cannot perform.

**Its verdict on the operator console is a measurement worth preserving:** there is no SaaS operator
surface in the tenant's view or in the tenant's bundle, and _"absence from a tenant's view is not
absence from the product."_ It also correctly refuses a false positive — the account page's
...
2. **`.stripe-mini` is the CSS class of the inline Stripe block on a user row**, and it renders only
   ...
   says exactly how to get it: _open a room that HAS a marketplace member (User List Actions →
   Marketplace Users) and re-run._
...
The capture's role note is also useful: _"presenter or admin on the manage page — bulk actions
visible, **no Extra Admin Users**"_. So the **Extra Admin Users** panel (§6.13's `/users/v1/adminusers`)
...
```html
<a ng-click="pauseUserNotifs(user._id,user.userName,$index,'pause')">
  PAUSE Mobile Notifs</a
>
<a ng-click="pauseUserNotifs(user._id,user.userName,$index,'resume')">
  RESUME Mobile Notifs</a
>
<a ng-click="pauseUserNotifs(user._id,user.userName,$index,'unsub')">
  Remove Mobile Notifs</a
>

<li ng-if="sess.ptrMobileAppCaseByCaseEnabled">
  <a ng-click="manageMobileApp(user._id,user.userName,$index,'enable')">
    Enable Mobile App</a
  >
</li>

<li ng-if="sess.ptrMobileAppCaseByCaseEnabled">
  <a ng-click="manageMobileApp(user._id,user.userName,$index,'disable')">
    Disable Mobile App</a
  >
</li>

<li ng-if="sess.fileAccessCaseByCase">
  <a
    ng-click="manageFileAccess(user._id,user.userName,$index,'enable'|'disable')"
    >Enable/Disable Files</a
  >
</li>
```
...
| #   | gate                                                      | scope            | where                                                                              |
| --- | --------------------------------------------------------- | ---------------- | ---------------------------------------------------------------------------------- |
| 1   | `ptrMobileAppEnabled` / `customMobileAppEnabled`          | room             | _"Enable PTR app?"_                                                                |
| 2   | **`freeTrialsGetApp`**                                    | room             | _"Also enable the app for free trials?"_                                           |
| 3   | `ptrMobileAppCaseByCaseEnabled` + **`user.hasMobileApp`** | **per member**   | set by `manageMobileApp`                                                           |
| 4   | **`diasableFCMAlerts`**                                   | room             | **_"Disable PUSH Alerts?"_ — a room-wide push kill switch**                        |
| 5   | **`user.alerterAppFCMUserOff`**                           | **per member**   | set by `pauseUserNotifs`; renders a **red** phone icon on the row                  |
| 6   | `mobileAppExpireNotificationsDays`                        | room, time-based | _"If user does not log in this many days, we'll stop sending push notifications"_  |
| 7   | `dontPush`                                                | **per alert**    | §6.10                                                                              |
| 8   | `isAlertOnly`                                             | room type        | _"Alerts only rooms are just rooms to receve push notifications and nothing else"_ |
...
push · SMS (Twilio _or_ Protexting) · email · Twitter/X · linked rooms · **an external webhook** ·
...
originated in room A, and the swing/day-trade variants pull _from_ other rooms as well as pushing
_to_ them.
...
| ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| **`diasableFCMAlerts`**                                                         | _"Disable PUSH Alerts?"_                                                          |
| **`sendFcmAlertsNew`**                                                          | _"New FCM Method?"_ — **"Use pub/sub for notifications"**                         |
| **`invalidTokens`**                                                             | _"Comma separated list of invalid JWT tokens."_                                   |
| **`ptrMobileAppCaseByCaseEnabled`**                                             | _"App for Some Members?_ Note above needs to ALSO be on"                          |
| **`customMobileAppV3Name`**                                                     | _"Custom app String"_                                                             |
...
| **`useV3` / `useV5`**                                                           | _"(DON'T TURN THIS ON, If PTR did not clear you for v3/v5!! it will not work….)"_ |
...
<div ng-show="sess.hasAppPairLink && sess.pairSecretKey">
  Sample link you would need to use to add each user: (replace email/name with
  the real user email/name
...
line, the whole of P-1: the reference has a documented way to _add_ a member on subscribe and no
documented way to _remove_ them on cancel.
...
1. **`/public/html/POST_ROUTE_API_DOCUMENTATION.md`**, linked from the Settings tab as
   _"API POST Routes Docs"_ via `/public/html/api-docs.html?src=…`.
...
`superClusterID` + `superClusterExpectedServerCount` — _"new supercluster scaling logic to scale the
session across the super cluster"_.
...
- **`positionsIframe` + `positionsIframeUrl`** — _"Enable positions iframe?"_. **This is R-2's
...
- **Auto open/close:** `autoOpenTime` / `autoCloseTime` (_"Time in Military EST"_),
  `ignoreAutoOpenCloseOnWeekend`, `autoResetSession`, `doNotAutoSoftReset`.
- **Auto clear:** `chatAutoClear` / `alertsAutoClear` (_"11:45PM EST / 10:45PM Central"_),
...
- **Newer chat features not in the register:** `enableReactions`, `enableQAReactions`,
  ...
  `hasQAOnAlerts`, `advancedSearchAlerts`, `claimNickName`, `enableTokenBadges` (_"Badges will come
  from JWT token"_), `copyTrades`, `tipMeBtnEnabled` / `tipMeBtnTxt` / `tipMeBtnUrl`, `salesBanner`.
...
**Owner correction, 2026-08-16 10:40: _"if you read all the css files and folders i created you will
have a lot of answers that once again you're assuming. either on new-room-control or new-room folder
i created an entire css folder with every css stylesheet."_** **Correct. Located, and it invalidates
...
> _"The product runs **TWO Bootstrap generations** on two different surfaces… **The room** is
...
> has no `.panel` at all, so that markup cannot be 4 or 5."_
...
| ----------------------------------------------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`~/Desktop/new-room-control/css-modals/styes.css`** _(sic)_     | **218,719** | **PTR's own `styles.css`** — the single first-party manage stylesheet. Same byte count as `gaps-closed.md:56`'s `TIER1-fetched/styles.css`                                                                                                                                                                                                                                                                                 |
| `~/Desktop/new-room-control/css-modals/bootstrap.css`             | 144,638     | **full Bootstrap 3 source**                                                                                                                                                                                                                                                                                                                                                                                                |
| `~/Desktop/new-room-control/css-modals/bottstrap-min.css` _(sic)_ | 147,852     | minified twin                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| §15.8: _"`.btn-secondary` — the class is in the markup but nothing readable defines it"_, with a hedge that a cross-origin sheet _"could in principle"_ carry it | **PROVEN DEAD. `btn-secondary` = 0 in `buttons.less`, 0 in `bootstrap.css`, 0 in `bottstrap-min.css`, 0 in `styes.css`.** It is a Bootstrap **4** class name in a Bootstrap **3** build. The hedge is gone — _"Close Add Admin User" renders as a bare user-agent button and copying it faithfully copies a bug_ |
| §15.8: `.btn-inverse` _"exists only in styles.css; Bootstrap 3 has no such class"_                                                                               | **CONFIRMED, and quantified: 58 occurrences in `styes.css`, 0 in Bootstrap.** Not a one-off — a load-bearing custom class                                                                                                                                                                                        |
| §15.9: `label-orange` _"is not stock Bootstrap 3"_                                                                                                               | **CONFIRMED: 6 in `styes.css`, 0 in `labels.less`, 0 in `bootstrap.css`**                                                                                                                                                                                                                                        |
| §15.9/§15.12: _"heading CSS is uncaptured — 'this heading has no rule' is unprovable"_                                                                           | **FALSE.** `type.less` carries the heading block at lines 9–26 and `h3, .h3 { font-size: @font-size-h3; }` at line 49. **The h3's 24px / 500 / 26.4px is derivable from source, not merely observed**                                                                                                            |
| §15.8: _"No :hover, :focus, :active, :visited or :disabled rules appear anywhere in this slice"_                                                                 | **Those states are all in `buttons.less` and `bootstrap.css`.** They were never missing — the _capture_ only collected rules matching each element's current state                                                                                                                                               |
| §15.12: _"2 cross-origin stylesheets are unreadable, so a zero in the css column is weaker"_                                                                     | **Only true of the capture.** The sheets themselves are on disk                                                                                                                                                                                                                                                  |
...
> _"The AngularJS admin theme's NAME and vendor are not in the capture… I grouped 16 states as theme
...
§15.8 recorded this as _"`styles.css .btn { outline: none !important }` and `a { outline: none }`"_.
...
| `.btn-default` border **`#e6e9ee`** — _"not Bootstrap's #ccc"_ | `#e6e9ee` is a theme-wide token: also `.page-header{border-bottom-color:#e6e9ee}`, `.nav-tabs-alerts>li>a{border:1px solid #e6e9ee}`, `.tab-content{border-color:#e6e9ee}`, `.popover{border-bottom:2px solid #e6e9ee}`                                                                                                                  |
...
§15.9 recorded: _"The parent element that constrains `.app` to 1140px is not captured. The `.app`
rule declares `width:100%`, so the 1140px measured width comes from an **uncaptured ancestor** (its
selector, class and rules are unknown)."_

```css
.layout-boxed .app-container > section {
  max-width: 1140px;
  margin: 0 auto;
} /* lines 556-559 */
```
...
§15.9 noted the computed `padding-bottom: 0` _"proves an ancestor carried `.footer-hidden`"_ but had
...
.app {
  ...
  padding-bottom: 80px;
  width: 100%;
} /* lines 471-475 — exactly as captured */
```
...
> ⚠ **One attribution to correct.** `account.css:743` says _"It sits INSIDE the **1170** container in
> the reference"_. **Bootstrap 3's `.container` is 1170px at the `lg` breakpoint, but
...
> ⛔ **Owner correction, 2026-08-16 10:22: _"stop assuming, and only work based off of hard evidence
> and you will only have the entire picture once you finish reading all the files."_**
>
...
>
> **The capture's own author hedged where I did not:** _"a statement about THIS BUILD as served to
> THIS ACCOUNT — not proof that PTR operates without one."_ I turned that into "does not exist".
>
...
>
> The document's own words, verbatim: _"A reader skimming twenty zeros would conclude there is no
> admin concept in this build, **which is flatly wrong**."_ The same capture holds
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **`stateControllers`** | **0 for all 23 entries, including the positive controls.** The whole haystack is **three values, two distinct strings** — `LoginCtrl` and `KickedCtrl`. _"Do not cite it as evidence of anything."_                                              |
| **`modules`**          | **0 for all 23, including the controls.** It searched ~216 **vendor module names**, not application source — no controller, service, directive or factory name is in it. _"Its zeros must never be quoted as 'no operator code in the bundle'."_ |
...
correctly said the control plane _"cannot be matched, only designed"_ but could not distinguish
_there is no console_ from _this account cannot see one_. The registry can.
...
**Just over half the legacy route table is dead theme.** `app.users` and `app.chat` _look_ alarming
...
```html
<a type="button" ng-click="createNew()" class="btn btn btn-warning mb btn-block"
  >New Room</a
>
```
...
The metric is _"non-abstract state with no **ui-sref** on this page"_. It ignores raw `href`s,
...
> _"The verdict is scoped by its own wording to **THIS BUILD** (`app.min.js v=1786262947549`) as
...
It contains **eight live HS256 JWTs** — full header, payload _and_ signature — because the Launch
...
- **29 of 31 states were never rendered** — everything about their content is _not captured_, which
  is different from _does not exist_.
- **Resolve function bodies are not captured.** If any authorisation happens in a resolve, this
  capture cannot see it. `data: null` proves only that no _declarative_ route metadata exists.
...
wording, _"because the four account panes carry no id, their headings carry no class, and the panes
use `panel pane-default` — not Bootstrap's `panel-default`."_

> **None of them was clicked.** _"`page.welcome.html` is a template and contains no function bodies,
> so for every handler here the question 'does invoking this mutate anything?' is genuinely
> unanswered."_ An adversarial review cleared six as read-only from their labels and **every one of
...
| `sessionManageLink`     | _(none — pure `href`)_                        | `<a href="#/page/manageSession/«hex 24»" class="btn btn-sm btn-inverse"><i class="icon fa fa-cogs"></i> Manage</a>`                                                       |
...
populates it.** §4's row is closed as _unanswerable from the client_ rather than _not yet found_ —
...
| **`.btn-default` border** | **`#e6e9ee`** — _not_ Bootstrap's `#ccc`. **Using #ccc would be visibly wrong**                                                                                                                                        |
...
> ordered by **stylesheet source order, not specificity** — _"Do not read cascade winners off the
> array order; read the computed block."_
...
1. **Focus outline suppressed site-wide** — `styles.css .btn { outline: none !important }` and
   `a { outline: none !important }`. _"An accessibility regression we should NOT copy."_
...
5. **`.btn-secondary` is a DEAD CLASS** in this Bootstrap-3 build. The "Close Add Admin User" button
   renders as the **UA-default grey** (`#efefef`, transparent border). _"Reproducing it faithfully
   means reproducing a bug."_ Give it a real style.
...
**Owner correction, 2026-08-16 10:28:** _"sometimes it may show 0 but its not because it doesn't
exist, because there was a member or an extra admin added."_ **Correct, and I have the hard evidence
that proves it on the very control I got wrong.**

I recorded `adminUserRemove` — **count 0** — as _"not captured, not proven absent."_ That was too
...
**Rule, now standing:** _a zero from a rendered capture is only evidence when the precondition that
would produce a row is known to have been met._ State the precondition, or the zero says nothing.
...
```html
<h4 class="ng-binding">
  Total <span ng-click="showNewRoom=showNewRoom+1;">Sessions</span>: 1
</h4>
<!-- <button class="btn btn-sm btn-default" ng-click="getSessions()">Refresh</button> -->
← COMMENTED OUT
<div class="row">
...
**verification spec**, not a gap list — the remaining question is whether the _values_ match
...
- **State labels are `<div>`, not `<span>`.** _"A Bootstrap `.label` on a block-level div renders
  differently from the usual inline span."_ And **`label-orange` is not stock Bootstrap 3.**
- **Action labels carry a leading space:** `" Launch"`, `" Manage"`, `" Marketplace"`.
- **Marketplace has no `href`** — pure `ng-click`.
- Sort icons are **`<div>`, not `<i>`**, which is _why_ the header row is 60.5px tall — the block
...
> _"`ng-hide="s.isArchivedRoom && !showArchivedRooms"` on every row, plus `sortByUUID`/`sortByName`"_
> _"The reference hides **two things** behind clicks on the word 'Sessions'"_
...
> _"the API-key restrictions editor — ours; the reference opens a **bootbox** for it"_

and says: _"Nothing in this capture supports or refutes the bootbox claim… `manageApiKeyRestrictions`
is **NOT** among [the four known bootbox handlers]."_
...
> **Do NOT document "the global sets the scope flag" as fact.** The capture is explicit: _"Correlation
> of two truthy values on one page is not a causal chain, and the coercion function, if any, is not in
> the evidence."_ It is equally consistent with two independent sources that happen to agree.
...
⚠ **The four absent globals are not evidence of anything.** _"Those names came from the reference's
own templates. Do not build a feature-flag surface on this absence."_

### `lifecycleControls = []` — and why it is weaker than it looks

Recorded, with the capture's own caveat: _"it does not say what the probe looked FOR — the search
terms, selectors or handler names the collector used are not recorded anywhere in the capture, so a
false negative from a too-narrow probe cannot be ruled out."_
...
_"The failure mode for an exhaustive decode is silent omission — six agents each cover their slice and
nobody notices a whole key was never opened."_ **This is the model for the twice-run audit the owner
...
3. **The live JWT — 8 copies — that no agent mentioned**, carrying the owner's name, email, user id
   and `exp 2027-08-09`, _"while the redactor masked the same 24-hex id about 900 characters earlier
   in the same string."_
...
`adminUserAddToggle[1]`) _are_ self-hidden by `ng-hide`; two are not. **The mechanism differs and
nobody had distinguished them.**

**And the five-click threshold is the COLLECTOR's assumption, not an observation** — _"refusedClicks
says 'click N of 5', which is the collector's assumed threshold… the actual number of clicks the
build requires is not in the capture."_ **§15.5 should not be read as "five clicks".**

### ✅ §H item 6 vindicates §15.11 — our `account.css` comment is backed

§H line 1146 leaves our bootbox comment as _"either backed by evidence outside this slice or an
unevidenced claim in our own source."_ **§15.11 resolved it: backed.** `manageApiKeyRestrictions`
...
| 2   | **Silent truncation** — `html` at exactly 4,000, `text` at exactly 300, **no ellipsis, no marker**                          | **open.** _"the same defect `collect-manage-gaps.js` was written to fix in its predecessor, whose header calls silent truncation 'the worst kind' — reintroduced here"_ |
...
> _"Neither `ptrMobileAppEnabled` nor `customMobileAppEnabled` nor `freeTrialsGetApp` appears anywhere
...
> disabled still sees a Mobile App tab and a working Restore Connectivity button.**"_
...
- Its gate is **re-checked inside the method**, not only in the template: _"belt-and-braces and it is
  worth keeping."_
...
| ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **`customMobileAppV3Name` and `customMobileAppLaunchWord`** — _"Lookup: the manage-page bundle `/public/dist/app.min.js`"_                     | ✅ **CLOSED.** That bundle was read in full (§6). **`customMobileAppLaunchWord` = the custom URL SCHEME** — `<word>://?t=<jwt>&s=<roomID>&pc=<pairCode>`, default `protradingroomapp://` (§6.9). **`customMobileAppV3Name` = the manage label "Custom app String"** (§8.4) — still no consumer found in any client                                                                                           |
...
| **`ptrMobileAppCaseByCaseEnabled`** (row 8, "0 occurrences", NOT IN BUNDLE)                                                                    | ✅ **CLOSED by §8.1.** It is a manage setting _"App for Some Members?"_ that reveals two row-menu items calling **`manageMobileApp(…,'enable'\|'disable')`**, writing **`user.hasMobileApp`**                                                                                                                                                                                                                |
...
> ⚠ **That document also flags its own unverified claims:** _"The three `MOBILE-APP.md`
> contradictions above were NOT re-verified by the main agent and remain claims, not findings."_
...
> ⛔ **Owner correction, 2026-08-16 10:22.** This section was headed _"P-1 IS ANSWERED"_. **Withdrawn.**
...
2026-08-13.** I named this file as queue item #1 and called it _"one fetch away"_. **It was already in
...
**This overturns §8.6.** That section concluded _"there is no corresponding remove endpoint anywhere
on the page"_ — true of the **manage page markup**, and wrong as a statement about the system. The
...
| authenticate     | `secret` = the room's **`apiSecret`** (§6.13 `generateNewApiSecret`; §8.10 the settings row). **Rate-limited** — the doc lists _"Protection against brute force attacks on API secrets with configurable limits"_ |
...
2. **`/session/deletedlogs`** — _"When a mod deletes or edits a message/alert, these are stored
   here."_ So the reference **retains an audit trail of moderation**, which §6.10's client-side-only
...
> **User Lists:** _"Complete user information with roles **and FCM status**"_

**`POST /session/users` returns each member's FCM status.** So the API can both _read_ who is
subscribed and _change_ it — a reconciliation loop between billing and push is possible without any
new endpoint.
...
- **`emailHash()` is a server-side function**, applied to inbound emails, and **"Email addresses are
  ...
  should use.
- **`addUsers` is an UPSERT** — _"add new users, update existing ones."_ Safe to call repeatedly.
- **`delUsers`** — _"Removes users and unsubscribes from FCM notifications."_ Confirmed twice more in
  the Implementation Details and Notes sections.
- **QA mode exists:** _"FCM operations can be skipped in QA environment"_ / _"QA mode support allows
  testing without affecting FCM subscriptions."_ **There is a server flag that suppresses FCM side
  effects** — which is how P-1's work can be tested without pushing to real devices.
- **Trial operations auto-create users** — _"Trial status operations automatically create users if
  they don't exist."_
...
- **`useV3/V4/V5` was already answered there**, better than §8.8 put it: all three are **0 occurrences
  in the v4 bundle** — _"The v4 client never reads them; they select which client is served."_ And
  **`useV4`'s control is HTML-commented out** in the template (A.2, bytes 223,400–226,400).
- **A.4 is the finding §8 should have led with:** _"Settable on manage, never read by the v4
  client"_ — measured **0** in the bundle: `downloadRecordingsDisabled`, `linkedRoomSwingAlerts`,
...
| **The eight push gates as a synthesis** (§8.2)                                                                                             | a _conclusion_ across settings + row fields + alert model, not an enumeration                     |
...
| **Stripe on the member row** — 14 fields + status vocabulary (§6.12)                                                                       | corroborated by `stripe-details-*.json`; the _synthesis_ with push is new                         |
...
**A second rule, earned here:** rule 6 says _nothing is missing without a check_. Its twin is
...
| --------------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **`apps/room/docs/website-ptr1-prt2-full-read.md`** | **1,180** | _**"FULL END-TO-END READ: ptr1.json + prt2.json. Every cap and every node was walked. No sampling of caps."**_ ptr1 = 23 caps (incl. 18 opened modals/dropdowns, and forced dark/light theme full-DOM captures); prt2 = 5 caps |
...
>
> An earlier version of this table said `restoreMobileAppTokens` _"was decoded **and built**"_, citing
...
>    _test_ files, never in a component.)
>
...
>
> **The lesson is the mirror of §11's:** the rule _"nothing is missing without a check"_ has a twin —
...
`enterprise-and-control-plane.md` reads _parts_ of `app.min.js` (§7 lists its own gaps, three of
...
> _"Counts come from python `.count()`, **never `grep -c`** — the bundle is one line, so a line-based
> count returns 1 or 0 and destroys every real number. That mistake was made here on 2026-08-15 and
> nearly caused a correct report to be dismissed."_

**§9's counts are safe** — they use `grep -o … | wc -l`, which emits one line per _match_, not per
...
**Owner instruction, 2026-08-16: _"You also have to make sure you're not reporting something that we
already have based on hard evidence."_** That instruction was given **while §10.2 was being written,
...
_present in the reference AND absent from our source_ — **both halves need hard evidence**, and I
supplied only the first for an entire section. `~/CLAUDE.md` rule 5 already says _"nothing exists
without a consumer"_; this is its mirror — **nothing is missing without a check.**
...
_"Make sure every single thing missing and gap is closed on the file based on hard evidence… double
check over and over and ensure your docs are correct. Hard evidence only."_
...
**Both directions agree with the full reads.** §6.13's conclusion stands; its _explanation_ did not
and was corrected.
...
chatModel.send("alertLater", {
  txt: task.txt,
  sendTxt: task.sendTxt,
  sendEmail: task.sendEmail,
  sendTweet: task.sentTweet,
  sendLinked: task.sendLinked,
  sendLaterDate: task.sendLaterDate,
  sendLaterAsEmail: task.sendLaterAsEmail,
  sendLaterAsNick: task.sendLaterAsNick,
  sendAt: task.sendLaterDate,
  nonTradeAlert: task.nonTradeAlert,
});
```

**Ten keys. No `dontPush`.** Confirmed against the exact bytes. The immediate `alert` send _does_
...
| §6.13            | its _explanation_ of my lines-2–8 error corrected; the blamed artefact does not exist                            |
...
| `ptrMobileAppCaseByCaseEnabled`    | §8.1, §8.4  | _"App for Some Members?"_; gates the two `manageMobileApp` menu items                          |
| `customMobileAppLaunchWord`        | §6.9        | the custom **URL scheme**; deep link `?t=<jwt>&s=<roomID>&pc=<pairCode>`                       |
| `customMobileAppV3Name`            | §8.4        | _"Custom app String"_                                                                          |
| `sendFcmAlertsNew`                 | §8.4        | _"New FCM Method? — Use pub/sub for notifications"_ (server behaviour still unknown)           |
| `invalidTokens`                    | §8.4        | a **manual** comma-separated JWT denylist field                                                |
| `diasableFCMAlerts`                | §8.4        | _"Disable PUSH Alerts?"_ — room-wide push kill switch                                          |
...
| --------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------------------- |
| ⭐ **`/public/html/POST_ROUTE_API_DOCUMENTATION.md`**                                                     | **which of §8.2's eight push gates the server evaluates and in what order**; what `sendFcmAlertsNew` switches to; whether `closeRoomAndRevoke` writes `invalidTokens`                                                                                                                                                            | fetch it — the manage page links to it as _"API POST Routes Docs"_                | **no** — one fetch     |
...
1. ⭐⭐ **Fetch `/public/html/POST_ROUTE_API_DOCUMENTATION.md`** — §8.7. The manage page links to it
   as _"API POST Routes Docs"_. **This is the reference's own server-side API documentation**, and it
...
_(Written after all 17 lines were read. The earlier version of this table, written at six slices, was
more pessimistic on P-3 and had P-5's payload as unknown — both are now settled.)_
...
| **P-2** | a green field                                                    | **Four pieces already exist** — `isM`, the md5-email identity, `kickDuplicates` (§6.2), and `alerterAppTokens` deduplicated _by email_ in the admin UI (§6.13), which is the reference admitting the problem. **None can be built on as-is:** `isM` is client-asserted and the ban is a localStorage key (§6.3, §6.4). Enforcement must be server-side                                                                                                         |
...
artefacts as _"on the Desktop, never read, must be transcribed before the dumps are deleted"_ when
...
1. **Neither file dies with the dump deletion.** The rule at the top of this file — _"write values,
   not locations"_ — was written because a byte offset into a deleted capture is worthless. **It does
...
_both_ halves are checked, and "this evidence is at risk" is a claim about our repository that has to
...
**2026-08-16.** §16.7 opened item **0b′** — _"the shipped stylesheet is 265 bytes smaller than the
captured v4 build sheet; deliberate patch or stale copy, not recorded as either."_ **It is closed,
...
canonical search-vs-read failure** (_"I searched for `st-fileSortBar`, got nothing, and reported 'not
in the capture'"_). So the first instinct is that something is missing again. **Check our source
...
- `apps/room/src/lib/file-sort.ts:13-16` already records the reason in prose: _"our older capture
  ...
  Evidence has a date, and the live application moves.**"_
...
stylesheet.** The queue called it _"the complete v4 ROOM CSS"_ and put it at the top of the reading
order. **Reading 8,086 lines of it as the v4 reference would have imported pre-v4 values wholesale —
a direct rule 6d violation** (_"THE CORPUS IS v4"_), and the sort bar proves the two builds genuinely
disagree.
...
list is not evidence of anything.** The presence of each of the 26 _is_ hard evidence.
...
_"the dumps are being deleted — write values, not locations."_ **It has been enforced against the
...
| ------------------------------ | ----- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ⭐ **`more-fucking-evidence`** | 3     | **16 K** | **Cited by name in the global `~/CLAUDE.md`** as the source that revealed the **30 empty `<tr>` elements** around 2 populated ones driving `nth-of-type` striping — _"no search for a class name would ever have surfaced an empty element."_ **Tiny, authoritative, at risk.** |
...
**Bootstrap 3 LESS sources** — the _custom-vs-stock oracle_. `apps/controller/evidence-bootstrap-3.3.7.css`
...
```html
<div>
  <span class="st-fileName">Melissa's Premarket Checklist.pdf </span
  ><span class="st-fileSize ml-2">68Kb </span>
  <div class="st-fileName"><i>Nov 24, 2025, 7:27:14 AM</i></div>
</div>
```
...
<div class="d-flex flex-column">
  <div>
    <span class="st-fileName">TR3NDY ROOM SCHEDULE.png </span
    ><span class="st-fileSize ml-2">44Kb </span>
    <div class="st-fileName"><i>Oct 13, 2023, 11:27:49 PM</i></div>
  </div>
  <a
    target="_blank"
    href="/var/www/uploads/4a500d1f358c6eefbf70295bdb7796d0"
    type="image/png"
    download="TR3NDY ROOM SCHEDULE.png"
    class="ng-star-inserted"
    ><img
      alt="Image"
      class="fileDriveImg"
      style="background-color: #000;"
      src="/var/www/uploads/4a500d1f358c6eefbf70295bdb7796d0" /></a
  ><!---->
</div>
```
...
```html
<table
  class="table table-striped m-auto w-100 mt-3 st-fileTable ng-star-inserted"
>
  <tbody id="filesDriveList">
    <td class="ng-star-inserted">
      <div class="d-flex justify-content-center align-items-center flex-wrap">
        <a
          class="fileDowload ng-star-inserted"
          href="/var/www/uploads/66b8e8bdc0470b24bf74e4fe0fa2bf86"
          type="audio/mpeg"
          download="PMZ mp3.mp3"
        ></a
        ><!----><a
          title="Download File"
          target="_blank"
        ...
        ><!----><button
          type="button"
          title="Play"
          class="btn ml-2 st-fileDownload btn-success ng-star-inserted"
        >
          <span class="ng-star-inserted"
            ><i class="fa fa-play-circle mr-2"></i>Play
          </span>
        </button>
      </div>
    </td>
...
instructions, and the _only_ record of the 30-empty-row striping behaviour in its original form.
...
<div class="row login-row">
  <div
    class="col-md-6 offset-md-3 col-sm-6 offset-sm-3 col-xs-12 login-form-container animated fadeInRight faster"
  >
    <h1 class="room-title">Welcome to the Room 3625</h1>
    <!---->
    <p class="text-center authenticate-info">Please complete this form:</p>
    <form novalidate class="mb-3 login-form"></form>
  </div>
</div>
```
...
<div class="loginGravatar">
  <div class="text-center user-avatar">
    <img
      src="https://www.gravatar.com/avatar/6ee71e550a8c162767abf7d2dc8eea84?d=mm"
    />
    <span title="Setup Avatar" class="setup-avatar"
...
<div class="mt-1 text-right">
  <a class="session-login-link">Not you? clear form</a>
</div>
<div class="mt-3 t text-center">
  <a class="session-login-link">Have a password?<br />Click here</a>
</div>

<div class="login-footer">
  <p class="text-center">
    Powered by:
    <a href="https://protradingroom.com" target="_blank">ProTradingRoom.com</a>
  </p>
...
bundle and this capture is one render, so the honest reading is _there are probably two layouts and
we implemented the other one._ **Resolve by reading bundle consts 33–36 before touching it.**
...
`O(2,…)` / `O(3,…)` / `O(4,…)` selectors deciding which of the three renders. It sits _after_ all of
...
§16.13 shipped `let rememberMe = $state(false)` with the comment _"`pue`'s `ngModelChange` writes
`rememberMe` and nothing else does, so it starts unchecked."_ **The second half is false.**
...
correct _for this path_. The comment now says that instead. **This is precisely the failure
...
| `this.email&&e&&(this.readOnlyEmail=!0)`                                                                                                                                                  | `readOnlyEmail` is derived from _email AND token both present_, not configured                                                                                   |
...
§16.13b recorded a single failure path with the fallback string _"There was an error login in,
please try again or contact support"_. **That is the missing-token branch.** The `loginFailed`
...
its `room-message`column carrying`h1.room-title`**and**`div.room-description`(const:`[1,"room-description",2,"height","100%","overflow-x","hidden",3,"innerHtml"]` — bound with
...
**`this.rememberMe=!0`.** §16.13c asserted _"`rememberMe` is NOT in this list, so it initialises
`undefined` — falsy"_ and called that independent confirmation. **It was neither.** I had started
...
§16.13b listed _"`disableEditingUsername` … ours has no perms term"_ as a gap. **It is not one.**
...
<div [const 6]>
  <h3 [23]>
    WARNING Unsupported/Outdated Browser. Please use a more recent version of
  ...
  <div [24]><img [25] /></div>
  <p [26]>
    For best results, We suggest using <span [27]>CURRENT VERSIONS of</span>
    <a [28]>Google Chrome</a>, <a [29]>Firefox (v52 and up)</a>, or
    <a [30]>Opera</a>
  </p>
  <br />
  <!-- only when NOT strictBrowserMode: O(21, e.strictBrowserMode ? -1 : 21) -->
  <button [32] (click)="browserOKDismissed=true">
    Let me try anyhow (NOT RECOMENDED)
  </button>
</div>
```
...
<div
  ...
  tabindex="-1"
  role="dialog"
  aria-labelledby="avatar-from-facebook-modal"
  aria-hidden="true"
  class="modal fade"
>
...
> _"The two modals are NOT symmetrical, and the difference is a real accessibility bug in the
> reference: the Facebook dialog carries `tabIndex="-1"`, `role="dialog"` and `aria-labelledby`; the
> Gmail one carries none of the three. Copying both verbatim would reproduce the defect."_
...
declared a field absent). Both times I had _read_ rather than grepped, and read the wrong extent —
...
2. **condition** — `!(browserOK || browserOKDismissed)`, dismiss button hidden when
...
§16.19 recorded the lesson that produced two false "absent" claims: _"'I opened the file' is not the
safeguard; 'I opened it at a boundary I verified' is."_ This read starts **before** the first view
...
.login-wrapper div.room-description {
  max-width: 80%;
  margin: 5% auto;
  font-size: 20px !important;
}
.login-wrapper .room-description img {
  width: 100%;
  height: auto;
}
.login-wrapper p {
...
.login-wrapper p.authenticate-info {
  padding: 15px 0;
}
...
.login-wrapper .input-group-append {
  border: 1px solid var(--lighter-gray);
  border-left: none;
}
...
appEventBus.subscribe("loginFailed", (e) => {
  globals.logginIn = !1;
  bootbox.alert(sessData.loginErrorMsg || e.message, () => {
    try {
...
```js
globals.isPlayer &&
  "a" == globals.decodedPassedToken.perms &&
  ((this.readOnlyEmail = !0),
  (this.disableLoginForm = !0),
  (this.pw = ""),
  (this.usernameInstructions =
    "You can adjust your name here, it would be used to show the stream name"),
  (sessData.hideNotes = !0),
  (sessData.hideFiles = !0),
  (sessData.hideStreams = !0),
  (sessData.hideChatAlerts = !0),
  (sessData.hideRoster = !0),
  (sessData.hideVideoPlayer = !0));
```
...
1. **`/public/images/supported_browsers.jpeg`** — not in `apps/room/static` (which holds only
   `assets/` and `favicon.ico`). Blocks G-2's image. _(standing from §16.19)_
...
--dark-black: #222 /* .login-wrapper color                                  */
  ...
  --lighter-gray: #eee
  /* .login-wrapper background, .form-control border        */ --white: #fff
  /* .login-form background, .setup-avatar background       */
  --avatar-gear-icon-padding: 5px 5.5px;
```
...
providers: [
  {
    provide: ZN,
    useValue: { siteKey: "6LcDyB4TAAAAAEajRvbeLyW2Lj_2TmXV5YSjAixx" },
  },
];
```
...
transcribed is the _secret_ key, which is server-side and is not in this bundle. Nothing is leaked
...
guiEventBus.subscribe("openAlertQAModal", ({ msg: e, openModal: i }) => {
  e &&
    (i &&
      ($("#alertQAModal").modal("show"),
      (this.modalId = e._id),
      $("#textAreaQATxt").val("")),
    this.modalId === e._id &&
      ($(`.${e._id}`).on("hidden.bs.modal", () => {
        delete e.unreadQA;
      }),
      (this.qaMsg = e),
      (this.msgs = e.qa),
...
sheet _also_ carries a correct `#textAreaQATxt{max-height:300px;width:100%}` near its end, so the
...
`app-alert-qa-modal` opens with _"KNOWN CSS GAPS - HONEST: these match NOTHING now the scope attrs
are gone"_ and lists those four selectors. **All four DO exist in the reference stylesheet** — I read
...
```css
#alertQAModal .modal-dialog {
  max-width: 600px !important;
  overflow-y: initial !important;
}
#alertQAModal .modal-body {
  min-height: 330px;
  max-height: 70vh;
  height: 100%;
  overflow-y: auto;
...
.textAreaBtns {
  color: var(--textarea-holder-btns-color) !important;
}
.textAreaBtns:hover {
  color: var(--textarea-holder-btns-hover-color) !important;
  cursor: pointer;
}
.txt-area {
  border-radius: 0;
...
.txt-area:focus {
  ...
  box-shadow: 1px 1px 1px var(--darker-gray);
}
#form-upload-img .input-group-text,
#form-upload-img .form-control {
  border-radius: 0;
} /* DEAD — no such element */
.white {
  color: #fff;
}
.textAreaBtnSelected {
...
.bs-popover-auto[x-placement^="top"] > .arrow:after {
  border-top-color: var(--modal-content-bg-color);
} /* Q-D5 — cannot match */
.giphy-search {
  width: 400px;
  height: 700px;
  border: 2px solid var(--modal-content-bg-color);
  background-color: #fff;
  overflow: hidden;
}
.giphy-search .input-group-text {
  border: none;
  background-color: var(--modal-input-group-bg);
}
.giphy-search .fa-times {
  font-size: 16.5px;
  padding: 10px;
}
.giphy-search .fa-times:hover {
  cursor: pointer;
  opacity: 0.85;
}
.giphy-header {
...
.giphy-search li:hover {
  background-color: var(--modal-upload-files-color);
}
.giphy-search h4 {
...
2026-08-16: _a gap counts only if the implementation needs it._ This section is therefore driven
...
`:1759` is wrong in a second way: it says _"Shift+Enter is ignored"_ while the code two lines below
