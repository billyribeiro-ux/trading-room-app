...
| 4 | `lib/components/AlertChatArea.svelte` | 1558 | `surface-audit-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` as the root of `app-chat` (93 consts, 27 embedded views), scoped to the seven files that implement it: **CLEAN — zero absent const values, zero absent text literals.** Pinned with both counts asserted. The animated typing dots joined it the same day (`TypingIndicatorDots.svelte`), which had been refused on a reason that turned out to be half wrong. |
...
| 8 | `lib/components/RoomOverlays.svelte` | 1240 | `## RoomOverlays.svelte` in the v4 register — 7 gaps, read 2026-08-31. |
...
| 16 | `lib/components/ExtraChatPane.svelte` | 773 | §XCP — 9 rows against `app-extra-chat` read whole: 3 built, 3 fixed, 1 measured refusal, 2 blocked. `docs/decoded/room-surface-audit-2026-08-30.md`. |
...
**93 of 93 surfaces audited · 40,285 of 40,285 lines · 100.0%.**
