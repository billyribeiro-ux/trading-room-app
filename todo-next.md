...
| 4 | `lib/components/AlertChatArea.svelte` | 1586 | `surface-audit-contract.test.ts` — READ END TO END 2026-09-01 by `gate/audit-surface.mjs` as the root of `app-chat` (93 consts, 27 embedded views), scoped to the seven files that implement it: **CLEAN — zero absent const values, zero absent text literals.** Pinned with both counts asserted. The animated typing dots joined it the same day (`TypingIndicatorDots.svelte`), which had been refused on a reason that turned out to be half wrong. |
...
| 16 | `lib/components/ExtraChatPane.svelte` | 801 | §XCP — 9 rows against `app-extra-chat` read whole: **all nine answered as of 2026-09-02**, 4 built, 3 fixed, 2 re-dispositioned. The last two both had blockers that named the wrong missing thing — XCP-07 refused on a rule that governs invented classes rather than transcribed ones, XCP-09 blocked on a re-capture when the rules were in the pinned bundle. `docs/decoded/room-surface-audit-2026-08-30.md`. |
...
**95 of 95 surfaces audited · 41,090 of 41,090 lines · 100.0%.**
