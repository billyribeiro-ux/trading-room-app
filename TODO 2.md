...
2026-08-14 from the bundle: the reference publishes to **MediaMTX** over **WHIP** and lets that server
record. `useMediaMTX`, `mediaMTXClusterID` and `backupMediaMTXClustterID` are all real manage-page
settings, all inside the reference's own `dont-touch` group. Client-side `MediaRecorder` is NOT a
divergence — upstream takes that same branch whenever no MTX stream exists.
