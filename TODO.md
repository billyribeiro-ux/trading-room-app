...
| 6 | **`apps/room` has no Playwright harness at all.** Half (a) is DONE — the controller suite runs in CI as the `controller end-to-end` job in `quality.yml`, 9 specs green, and the six stale locators it had rotted into are repaired (see the CHANGELOG entry of 2026-08-23 14:0x EDT). Half (b) is the real build and is untouched: the room has no `@playwright/test`, no config and no specs, and it is the application where a browser check is worth most — media transport, the SSE roster, and the presenter gate are all things no SSR render test can see. **What (a) proved is the argument for (b):** every one of the six defects was the SPEC being stale against a deliberate, correctly-made application change, and each would have been caught the same day by a job that ran | a room harness, and a decision on what it drives — the room needs a signalling peer, which the controller suite never did |
