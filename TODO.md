...
| 7 | **TWO controls say it worked and do nothing — down from five.** Fixed 2026-08-23: `kick-duplicates` (row 1), **both `"Command send OK."` liars** (`session-send-sales-image` / `session-send-users-url` → `sessionSendUrl`, presenter-gated by `presenterRoom()`, URL-parsed by `broadcastableUrl`, on `cmds` under the capture's names, bytes 1015180 / 1015357), and **`mute-chat-24`** — the third entry ever to leave `EXACT_ALERTS`. Its wire is `sendServerAdminCommand("muteChat", {user, time})` at byte 2080089 and a working mute already existed (`mute24`); both doors now call `applyChatMute`, which also **announces** the mute the room had only ever enforced. Still lying: `admin-notes-password` (row 2) and `session-save-close-message` (`"Message Saved"`, nowhere to save it). `mute-chat-indefinitely` is INERT, not lying — `muteChat("0")` upstream, and the indefinite mute already exists as the controller's opcode 3 (`role = 3, muted = true`), already enforced by `refuseIfChatMuted` via `member.muted`; what is missing is a room→controller door, the equivalent of `internal/room-ban`. The `"Command send OK."` RECEIVERS are still unbuilt | rows 2 and 7's two receivers; somewhere to save the close message; the opcode-3 door |
...
| 9 | **The `privCmdsIn` switch: ELEVEN cases, FOUR now built.** Read whole at bytes 995950-996500, not searched. Built: `forceReload`, `kickUser`, `unmuteChat`, and **`muteChat` (2026-08-23)** — `subscribe("muteChat", e => { chatEnabled = !1, bootbox.alert(e) })` beside `subscribe("unmuteChat", () => { chatEnabled = !0, alertService.success("Chat enabled") })` at byte 1430423, an asymmetry reproduced rather than normalised. `getRoster` is built in `RoomRoster`. **Left: six.** `remoteRestartAudio` — `sendServerAdminCommand("remoteRestartAudio", user)` + `bootbox.alert("Audio restart request sent OK")` at 2080461, receiver `subscribe("remoteRestartAudio", () => this.reconnectAudio())` at 1119299, and `reconnectAudio()` at 1133537 is *"remove every `[id^='msRemAudio-']` element, then `startListeningToPresenter` for each talking user"* — needs a faithful equivalent on `RoomMediaTransport`, which has `attachRemoteAudio`/`removeRemoteAudio` but no talking-user re-subscribe. `getDebugLog`/`debugLogResp` are a three-hop exchange (2080323). `remotePresCommand` is blocked with row 4. `userInfo` and `updateProfilePic` remain. **A SECOND switch exists** — `handleServerCmd` at byte 1010700 carries `kickUser` and `muteChat` on the BROADCAST `/cmds/` channel filtered by `xrefID` in the browser; this room addresses both instead, which is the 633f828 fix. It also does `i.banned && emit("logout")`, which this room does not: a banned member is disconnected but not logged out | five receivers; the logout-on-ban |
