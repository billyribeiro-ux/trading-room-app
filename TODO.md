...
| ~~AH~~ | **CLOSED 2026-08-15 21:26 — bounded by the ALERT PAGE, and moved to `alert-log.ts` with its reasoning.** A `LIMIT` of its own would have been wrong the way `chat-log.ts` describes a bare `.limit(300)`: it would drop questions belonging to an alert that IS on screen. Questions exist only for alerts and the load already ships one page of them, so `loadQuestionsForAlerts(roomShortCode, alertIds)` reads exactly those — bounded by that page, no second cursor to keep in step, and captured alerts (negative ids, no rows) correctly absent. The 2026-08-14 tenancy join stays IN the statement beside the id list. `+page.server.ts` 1,659 -> 1,583, below its ceiling rather than raising it. Guarded in `chat-paging-contract.test.ts` beside the other two reads, three controls seen red. **Three of those controls were no-ops first** — mutations targeting a `.where(...)` prettier had wrapped across lines — and a fourth guard matched `loadAlertPage`'s room filter instead of the new one. Both classes are recorded in the CHANGELOG entry. | CLOSED | `CHANGELOG.md` 2026-08-15 21:26 |
## Scope — what is NOT being matched

| AK  | **`AlertChatArea` — the extraction is PROVEN to work and is blocked on 24 test assertions, not on the component.** Attempted end to end 2026-08-15 21:16 and REVERTED deliberately rather than left half-done. Everything below was measured, so the next pass starts here instead of rediscovering it. **IT WORKS:** the pane is `+page.svelte` lines 9035-9732 (696 lines of body), out cleanly, `+page.svelte` **11,550 -> 10,870 (-680)**, `AlertChatArea.svelte` 842 lines, `svelte-check` **0 errors across 1,138 files**. **IT GOES INSIDE THE SNIPPET, not instead of it** — `chatAlertsPane` is rendered at TWO sites (the horizontal and vertical room layouts), so a component at each would spell the prop list twice. **THE PROP LIST IS 45 AND THE COMPILER PRODUCES IT.** Write the file with an empty `<script>`, run `svelte-check --output machine`, and take every `Cannot find name`. Do NOT scan the region by hand: `data` appears TWELVE times in that markup and not once as the variable — every hit is a `data-bs-toggle` or `data-bs-target` attribute, which is exactly the false positive that has already produced four invented props this session. **The compiler has ONE blind spot here:** `alertLabels` is a shorthand prop on `<RoomMessage>`, and while `RoomMessage` is itself unresolved its props go unchecked, so it never appears. Add it by reading, then confirm once the import lands. **`showMessageOptions` is the only `$bindable()`** — it is assigned by the composer's "+" button. As a plain prop the four composer buttons would appear in the component and nowhere else, and `svelte-check` is silent on it. **Two types must move to `$lib/types` first:** `MessageActionItem` is declared inside `+page.svelte` and exported to nothing. Doing that also finds a duplicate — the page's local `MessageReactionPayload` is character-identical to the one `$lib/types` already exports. **WHAT ACTUALLY BLOCKS IT: 24 assertions across 13 files** (alerts-advanced-search, alerts-toolbar, chat-badge-supply, chat-gif-muted, chat-mode, chat-rte-gate, day-separator, dump, extra-chat-column, message-gates, presenter-messages-right, privchat-toolbar, source-size). **A blanket re-point of their `+page.svelte` read to the component DOES NOT WORK, and the reason is specific:** several of them assert on BOTH the page's SCRIPT (`const presenterMessagesOnTheRight = $derived(data.sessData?...)`, `let chatGif = $state(loadedSettings...)`) and the pane's MARKUP (`kind="chat"`, `{...messageChrome}`). Those files need TWO source constants and each assertion pointed at the right one. That is per-file judgement, roughly a dozen files, and it is the whole remaining job. **Do it in one sitting with context to spare** — a half-migrated set of contract files is worse than an unextracted page, which is why this was reverted rather than committed. | MEDIUM — the last big line reduction, fully scoped | this row; `CHANGELOG.md` 2026-08-15 21:16 |

| ~~AJ~~ | **DONE 2026-08-15 21:07 — migration `0013` adds `dark_theme_badge_id`, and the control is the captured dialog.** `dark_theme` stays as a superseded column (forward-only, `0002` shipped); `true` backfills to NULL because the boolean never recorded WHICH badge and there is nothing honest to derive. The action is ONE statement — the target is resolved by a subquery carrying the account, because SELECT-then-UPDATE is what `account-actions-contract.test.ts` refuses. Three refusals: empty clears, a badge cannot nominate itself, the target must be in this account. **A control on the cross-tenant case came back GREEN** — the executable coverage is in `account-row-scope.db.test.ts` and `*.db.test.ts` is EXCLUDED from the default vitest run, so nothing in CI was watching; closed with a source-level guard plus three db cases. Two of my own: a `Map` of form elements that never needed to exist (a button knows its own form), and a self-referencing FK that typed the whole `badges` table `any` until annotated `(): AnyPgColumn =>`. **And I broke an unrelated test by formatting** — `api-key-session-restrictions` asserted on a line break that prettier removed; relaxing it exposed that the file has TWO `.from(rooms)` reads and the guard passed with the scope deleted from one. Anchored to `ownRooms` now. **DB CASES NOW EXECUTED, 2026-08-15 21:30: 55 tests across 9 files, green.** They stand up their own throwaway PostgreSQL — `initdb` into a temp directory, a random high port bound to 127.0.0.1 only, then `pg_ctl stop` and delete — so no real database is touched and migration `0013` is applied to a fresh cluster on every run. That is also why a `psql \d badges` against the local dev database shows NO `dark_theme_badge_id`: the tests never went near it, and that dev database simply has not been migrated. Checking it and nearly reporting "the migration did not apply" was mine, caught by asking which database the tests actually use. REMAINING and honest: the room-side dark-theme chip is still unbuilt, because no capture describes what one looks like. | DONE | `CHANGELOG.md` 2026-08-15 21:07 |
