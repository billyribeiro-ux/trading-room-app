...
| U | **No longer an evidence gap — the trigger is fully specified in the bundle, and no script is needed.** The row used to say it "has never rendered in any capture" and was one of the collapsed `<!---->` placeholders. That was true of the CAPTURES and false of the evidence: `main.d6d3c112b59b7d0d.js` carries it, its icons and its conditions. There are **two** `#dropdownVolume` triggers, which is why this looked missing — `["id","dropdownVolume","data-bs-toggle","dropdown",1,"nav-link","d-flex","align-items-center"]` is the main-nav one and **is already built** in `+page.svelte:6824` (matching `fa-2x`, `fa-volume-mute` and the `mainNavItem` label, each of which appears exactly once in the bundle). The one we do NOT have is the zoom-controls overlay variant: `["id","dropdownVolume","data-bs-toggle","dropdown",1,"btn","btn-sm","btn-dark"]`. **Its template is `hSe`:** a `button` holding one of three icons chosen by `audioVolume` — `>50` → `fas fa-volume-up`, `<50 && >4` → `fas fa-volume-down`, `<4` → `fas fa-volume-off`. **Reproduce the boundaries exactly:** at precisely 50 or precisely 4 NO icon renders, because every branch is a strict inequality. Its parent `CSe` puts three children in `div.zoom-controls-container.position-relative` — the zoom buttons (const 89), this button (const 90), then `div.dropdown-menu.volumeControl` (const 91) with `h4 " Volume "`, a `span.float-right.mr-2[data-bs-toggle=dropdown] > i.fas.fa-times` closer, `input[audioVolSlider][type=range][min=0][max=100][title=Volume].mx-auto.py-2.volCtrl`, `br`, Mute/Unmute (`title="Mute Audio"`/`"Unmute Audio"`, `btn btn-primary btn-sm`), `hr`, and `div.room-sound-options`. `ScreenTabs.svelte:249` renders the container and `{@render controls()}` only, so the button and menu are what is missing. **Remaining: build it.** | LOW — evidence complete, implementation outstanding | this row |
