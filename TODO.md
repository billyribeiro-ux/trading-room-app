...
| 6 | **BOTH HALVES DONE — and half (b)'s premise was false, exactly like half (a)'s.** This row said *"the room has no `@playwright/test`, no config and no specs"*. Wrong. `apps/room/scripts/verify-{tooltip-placements,screen-volume,viewer-only-layout,mobile-layout}.mjs` launch **real Chromium** through `@playwright/test`'s `chromium`, resolved from `apps/controller` deliberately — the scripts say so at their top, so that adding a browser gate is not a dependency change. They are not smoke tests: they measure tooltip arrow boxes and the 6px gap `k_([0, 6])` specifies, the volume icon per level, the navbar dropdown's full child order, split layouts and mobile widths, and exit non-zero on a mismatch. jsdom reports every rect as zero, so no vitest run can make those claims. **Measured 2026-08-23: 6/6 placements, 8/8 volume states, 4/4 viewer-only layouts, 4/4 mobile widths — 22 assertions, all green, and NOTHING had ever run them**: not CI, and not the room's own `test` script, which is `privacy:verify && schema:verify && vitest run`. They now run in the `room` leg of `quality.yml`, with their PNGs and JSON uploaded on failure. Two keep an honest NOTE rather than overclaiming: viewer-only measures the reference's CSS taking effect on our elements rather than a pixel diff (no capture of this product in that mode exists), and mobile renders the ARRANGEMENT rather than the Svelte component (the room needs a controller). **The lesson, twice over: "we have no browser coverage" was never checked, in either app** | a spec-style harness driving the LIVE room is still absent, and that one genuinely needs a controller and a signalling peer |
