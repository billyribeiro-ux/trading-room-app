...
| `docs/RETIRE-AWS-SFU.md` | **shutting down the OLD SFU on AWS** — every step and command, browser console and CLI. It is EC2 in us-east-1, not Lightsail |
...
| --- | --- | --- | --- |
| O | **The old SFU on AWS is still running.** An **EC2** instance in **us-east-1** at `34.195.170.147`, serving this project's own SFU behind Caddy on 443 — identified 2026-08-10 04:56 by `whois` (Amazon Technologies Inc.) and reverse DNS (`ec2-34-195-170-147.compute-1.amazonaws.com`), which is why looking in Lightsail found nothing: **it was never Lightsail.** Idle (`rooms:0, peers:0`) and fully orphaned — nothing in `apps/`, `services/`, `ops/`, `scripts/`, any `.env` or the live Caddyfile refers to it, so stopping it breaks nothing. It bills by the hour regardless, and it has SSH and HTTPS open to the internet on a build frozen 2026-08-02. **Blocked on an expired AWS session, not on permission:** `aws sts get-caller-identity` → *"Your session has expired"* (account `255248181057`, user `trading-app-admin`), and `ssh root@…` is `Permission denied (publickey)`. `aws login` is an interactive browser sign-in and is the owner's to run. | cost + exposure, not urgent | **`docs/RETIRE-AWS-SFU.md`** — every step and command, console and CLI |
