...
| ~~I~~ | ~~**Four public pages render unstyled, and the class list is SEVEN not six.**~~ MEASURED in Chromium at 1280px, each element compared against a bare sibling of the same tag: `.pub-auth`, `.pub-container`, `.pub-form-card`, `.pub-field` and `.pub-hint` all compute **identically to an unstyled element** — no rule reaches them. `privacy` and `terms` use **`pub-container`**, not `pub-auth`; an earlier version of this item listed six classes and missed it, because it was written from reading the pages rather than from rendering them. `pub-error`/`pub-success` render only on states not reached in that run and are an honest gap. ~~Separately: `contact`'s submit computes `background: rgba(0,0,0,0)` with white text, so check it is not white-on-white.~~ **RETRACTED — that was my measurement, not a defect.** `.pub-root .button` sets `background-color: #4589e3` and then a `background:` shorthand, which resets `background-color` to transparent while setting `background-image` to the gradient that actually paints. My probe read `backgroundColor` alone and omitted `backgroundImage`. It renders blue under white text. **DECIDED by the owner 2026-08-09: use the `pub-*` classes and write the missing rules — do NOT restyle these onto `acc-*`.** So they STAY in the marketing shell; do not add them to `CONTROLLER_PATHS`. Every new rule must be scoped `.pub-root …` like every existing rule in `public.css`, because unscoped these sheets fight the controller bundle over `.container`, `.row`, `.col-md-*`, `hr`, `img`, `.navbar`, `.caret` and `.dropdown-menu` — that is how the bulk-actions menu once came out 376px instead of 238px. And do not repeat the mistake this item was rewritten for: the shell is decided by `$lib/chrome.ts`, not by the `(public)/` folder, and a page can carry perfect markup and still lay out wrong under the wrong ancestor — see item J. **CLOSED 2026-08-09.** `public.css` now defines all seven — `pub-auth`, `pub-container`, `pub-form-card`, `pub-field`, `pub-hint`, `pub-error`, `pub-success` — every one scoped `.pub-root …`, in a section headed NOT TRANSCRIBED because those pages were never captured (`/contact` exists in the capture only as five links TO it: `home-page/file` 380, 397, 457, 487, 506). Owner's decision followed exactly: `pub-*` rules written, nothing restyled onto `acc-*`, and nothing added to `CONTROLLER_PATHS`. Values reuse what the app already has rather than being picked fresh — Bootstrap 3.1.1 panel and `.form-control` geometry, `#777` from `.acc-text-muted`, and the two message colours literal-copied from `--btn-danger-bg` / `--btn-success-bg` so this sheet does not depend on `account.css` loading. Deliberately fluid with NO `@media` block: `verify-breakpoints.mjs` asserts this file carries exactly 767/768/991/992/1200. **The white-on-white check this item asked for: it is fine, and the reading was an artifact.** `.pub-root .button` (`public.css:421-439`) sets `background-color: #4589e3` and then `background: linear-gradient(#5da4ff, #417bff)`; the shorthand resets `background-color` to transparent — which is the `rgba(0,0,0,0)` that was measured — while setting `background-image` to the gradient, so it paints blue under white text. Reading `background-color` alone misses the image. `contact` was rebuilt with labelled fields and an honest not-delivered state pointing at the real `support@` mailbox; `privacy` and `terms` were written from the source code, each naming the third parties a template would miss (Gravatar's MD5-of-email lookup, reCAPTCHA, Resend via SES `us-east-1`) and each carrying an explicit "not reviewed by a lawyer" notice plus a list of what is still undecided. **Verified:** breakpoint contract passes; `svelte-check` 0 errors, 3 warnings — unchanged, so none were added; 27 tests pass; autofixer clean on all three pages. **Honest gap:** `verify-home-fidelity.mjs` could NOT be run — it reads `evidence-dumps/`, which lives outside this repo — so its two `public.css` reject patterns were checked by hand instead (0 matches). | ~~cosmetic~~ **done** | `public.css`, "Public APPLICATION pages" |
| ~~J~~ | ~~**`/forgot-password` and `/reset-password` rendered in the WRONG shell.**~~ — **CLOSED 2026-08-09, same day, and recorded because the lesson outlived it.** Both were added under `(public)/` and fell through to the `marketing` chrome, so they rendered inside `.pub-root` while their markup was written against the controller's `acc-*` classes. `account.css` scopes its grid under `.acc-body`, so none of it matched. Measured in Chromium at 1280px: `.acc-panel .col-md-6` was **538px and unfloated** instead of 254px floated, and every input and button **508px instead of 254px** — every field twice its intended width. **Nothing caught it**: svelte-check clean, 522 unit tests green, build fine, and the SSR HTML contained every expected class. The defect lived only in the cascade. Fixed by moving the decision into `$lib/chrome.ts` with `chrome.test.ts` covering it; re-measured, all four metrics now match `/login` exactly. | — | in-code comment in `$lib/chrome.ts` |
