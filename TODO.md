...
| AJ | **THE ROW IS CLOSED, 2026-08-29 — `app.css` styled 29 classes nothing wears, and ONE of them was a missing feature.** Measured across 200 class selectors in our own `app.css`, with every `.svelte` and non-test `.ts` in `src/` as the corpus of possible wearers. Fifteen were INVENTED here (pre-decomposition panel names) and are gone — 26 rules, 185 lines, deleted in their own commit, every selector in every rule dead, so `postcss` could refuse by construction to touch a rule mixing live and dead selectors. Fourteen were CAPTURED, and this row read that flag as *"the rule is waiting for markup"*. **Ten have now left the catalog, in four different ways, and three of the four are this gate reporting work that did not exist.** (1) BUILT — `remove-profile-picture-btn`, then `edit-user-avatar-options` from `K2e` @ 2,058,852, whose const 23 IS that button: it belongs INSIDE the dropdown, not floating on the avatar, and the menu gate `O(6, o.user.userXrefID === o.appService.globals.user.userXrefID ? 6 : -1)` has **no role term**, which widened the server to `roomForAvatarChange` answering two authorities. Two of the four affordances were one control. (2) WORN BY AN INTERPOLATION — the five `mic-status-*`, a surface this row called *"missing with no counterpart here at all"* while `ModalHost.svelte` rendered `class="mic-status mic-status-{micStatus} mb-3"` over all five states. (3) WORN AS A CLSX OBJECT KEY — `mid`, eleven lines from those five, on the mic level bar: `class={['volume-bar-fill', { low: micLevel <= 30, mid: micLevel > 30 && micLevel <= 70, high: micLevel > 70 }]}`, the reference's own three thresholds. The literal matcher missed it for ONE CHARACTER — a key is followed by a colon, which was in its leading boundary set and not its trailing one. `isWornAsClassKey` reads `class={…}` VALUES only, because admitting `x:` across the whole corpus would let every options bag in the room vouch for a class named `title` or `id`; a control proved it load-bearing, `.refreshAll` failing as shipped and passing once widened. (4) **NEVER A SURFACE AT ALL — `chat-stars` and `tagline`, and the premise itself was false.** A captured rule does not imply captured markup: measured across BOTH pinned bundles, `chat-stars` occurs 12 times and `tagline` once and **all 26 occurrences are immediately followed by `[_ngcontent-%COMP%]`** — compiled CSS selectors, zero strings in any const table. The full 131-entry const table of `app-user-info-modal` (parsed at byte 2,087,741) holds neither name. The star rating IS real and wears three OTHER classes — consts 60/61/62 `stars-container`/`stars-icon`/`stars-num`, which this room already renders in `ModalHost.svelte` and twice in `RoomMessage.svelte`. `chat-stars` is the PREDECESSOR's name for it: `ptr1-DECODE.md:5289` has `.chatStars { … vertical-align: text-top }` and `todo-next.md:1145` has `<img class="chatStars" src="/public/images/<years>.png">` wearing it. v4 kept the rule, kebab-cased, and re-implemented the badge; **this room copied in a rule that had been dead upstream for a whole major version and then catalogued it as a feature to build.** Both rules are gone from `app.css`. **ONE ENTRY IS A REAL SURFACE, and reading the TEMPLATE rather than the neighbouring consts is what named it.** `smallAvatarImg`, const 98 `[1,"smallAvatarImg",3,"src","alt"]`. Its neighbours read as a followed-users list — 93 a `col` scrolling at `max-height:300px`, 96 `fa-plus-circle`, 99 `fa-minus-circle`, with the follow-chat colour inputs a few entries later — and that guess was **wrong**, which is the fourth wrong guess this row has made from a const table alone. `fTe` @ 2,064,959 settles it: the list is the **PER-USER ADMIN NOTES** list on `#user-modal`'s notes tab. Each row is that avatar, then `" [" date|short "] " name ": " note`, then a `fa-minus-circle` calling `deleteNode(note, index)`; `mTe` wraps them and puts " Add Note " below. **This room renders that tab and only its FALSE branch** — `{#if !canManageNotes}`, upstream's `pTe` password prompt, with no `{:else}` — so a presenter who clears the password gets an empty panel. The follow feature it was mistaken for is largely BUILT here (`managed-users.svelte.ts`, `followChatStyle`, the five colour inputs and the sound gate in `ModalHost.svelte`, `#followedUsersModal` worn), and its own avatar is sized by a descendant rule instead of that class. Filed as row AK. `renderedUpstream` is measured against the pinned v4 bundle on every run, so "captured" can never again be read as "pending"; the predicate requires the class to be a string INSIDE AN ARRAY, because bare containment matched `{name:"mid"}` in the bundled sdp parser. Five negative controls seen red. **The catalog is FOUR entries: one surface to build and three carried rules.** | **CLOSED.** Nothing here is a pending reference surface except `smallAvatarImg`, which is filed in `CHANGELOG.md`, and it too is now BUILT: the per-member admin notes list, which the const-table guess had filed as a follow-a-user list until the template was read. |
| PC | **The private-chat Load More has no SCROLL RESTORATION.** The reference's scroller holds a fourth field beside the three built on 2026-08-30: `loadMoreLastID = "pcm-" + this.msgs[0]._id` at bundle byte 2,191,427, read by its `getPCLog` subscriber as `document.getElementById(this.loadMoreLastID).scrollIntoView(!0)` followed by `parent.scrollTop = parent.scrollTop - 20`. Without it the reader's view jumps by however many rows the page prepended. It was modelled in `#lib/chat-paging.ts` and REMOVED before commit, because nothing could read it: our rows render through `CompactMessageRow`, which emits no `id` attribute, so the lookup would find nothing — and `load-more-paging.test.ts` now asserts the field's absence so re-adding it without its reader fails. **Buildable now; two pieces.** (a) an `id` on the row — `CompactMessageRow` is shared with the all-user modal, so it takes an optional prop rather than hard-coding the `pcm-` prefix; (b) a `scrollToAnchor` beside `RoomPrivateChat.scrollToBottom`, which already does this kind of DOM work. NOT verifiable in the browser job as it stands: `.pc-messages` needs an open conversation, which needs a second member in the roster. | scroll restoration |
| 9 | **The `privCmdsIn` switch: ELEVEN cases, EIGHT built here, TWO built elsewhere, ONE left — and **`badges` was wrongly grouped with it, corrected 2026-08-30.** This row filed `badges` beside `notesArr` and `privData` as needing a server-side user-detail lookup this room has no endpoint for. True of the other two; false of that one. The controller builds `badges: {definitions, byEmailHash}` for the WHOLE room in `internal/room-config`, `+page.server.ts` puts it on the page load, and `RoomFeeds.badgesFor` has resolved it for the chat the entire time — including the dark-theme variant swap. What was missing was the CELL: `<td><div class="d-inline-block align-baseline mr-1"></div>` with nothing bound to it, while the same member's badges rendered on every chat message they sent. Built; upstream binds `innerHTML` there and this does not, because every value in a badge is controller data. See `user-badges-contract.test.ts`.** Re-enumerated 2026-08-29 from bytes 995,800-997,400, and the offsets are quoted in `private-commands.ts`'s own docblock so the count can be re-run rather than trusted: `995896 forceReload`, `995969 remoteRestartAudio`, `996041 getDebugLog`, `996120 debugLogResp`, `996187 kickUser`, `996261 muteChat`, `996320 unmuteChat`, `996376 remotePresCommand`, `996456 userInfo`, `996699 updateProfilePic`, `996894 getRoster`. Built HERE: all but the three that are not addressed frames — `remotePresCommand` rides the `cmds` channel and `getRoster` lives in `RoomRoster`, both built. `forceStopScreen` is a NINTH branch with no upstream case at all (2026-08-29). The whole channel sits behind **ONE addressing gate**, deny-by-default, which every branch written after it inherits without its author knowing the rule exists. **LEFT: the SOCKET half of `userInfo`, and only that.** Sender `invoke("invokeCmd", {cmd:"userInfo", uid, rid, socketID, serverID, liveOnly})` at byte 1,026,474. The reference has TWO paths behind one method and this row used to describe them as one: `a ? socketService.getUserInfo(...) : getUserInfoDB(s, r)` at byte 1,159,275, where `a` is the socketID. **The DB half is BUILT (2026-08-30)** — `routes/user-detail.remote.ts` over `server/user-detail.ts`, presenter-only and room-scoped, filling the Last Login cell that had no producer at all and the Email cell that only ever filled from a roster row. Every reference caller except the roster takes that branch (chat message 1,352,046; followed-users 2,356,520; Random User 2,516,476). `badges` was corrected out of this row above; `notesArr` is the Admin Notes tab, built on its own query rather than riding this frame. What remains is the branch the roster takes, which asks the MEMBER'S OWN BROWSER for live telemetry — `privData.ip`, `privData.uaStr`, `data.cver`, `data.streamServer`, `serverID`. That is a round trip to a peer, not a database read, and it is the same shape as `getDebugLog`/`debugLogResp`: a request frame addressed to one member and a reply frame back. **Blocked on nothing; not built.** **This row said FIVE built and THREE left until 2026-08-29** — written before `getDebugLog`, `debugLogResp` and `updateProfilePic` landed, and the same staleness the row's own prose warns about. The logout-on-ban is DONE and pinned by `ban-ends-the-session-contract.test.ts`. | one receiver: `userInfo` |
...
| **E** | **DONE 2026-08-29 — RUN in a browser, and now run on every push.** The row's ask was *"no browser has watched a column leave the DOM when an owner ticks the box"*. One has: `apps/room/e2e/room-config-seam.spec.ts` enters a room whose owner has hidden the chat/alerts column and asserts `.alert-chat-box` has count **0**, then enters a default room and asserts count **1**. Both passed on first run. **It is not the probe this row described, and it is more than that probe could have been.** The instrument was `room-config-seam-e2e.mjs` — untracked, absent from a fresh checkout, and pointing at port 5180 where a different project answered its one attempted run with a 404. This is a TRACKED spec in the suite that already boots both halves, so it runs in CI rather than by hand; and because `stub-controller.mjs` now keys settings on the SHORT CODE — *"any short code is a room"* was already true of it — the same build answers BOTH ways in one run, which is the only version of the assertion that can tell *"the column is hidden"* from *"the column was never built"*. **Three negative controls seen RED**, and the third is the one that matters: removing `hideChatAlerts` from the controller's `ROOM_VISIBLE_SETTINGS` makes the column stay, so the spec observes the SEAM and not just the component. Honest limit, stated at the file: the stub is not the controller — it verifies the `config-read` HMAC and refuses a write token on the read route, so the capability half is real traffic, but the settings it returns are the spec's. That the controller puts the right value on the wire stays `room-config-boundary.test.ts`'s subject | NOTHING. Done |
| **R** | **Screenshare quality and the MP4 question — rows 6, 8 and 10 of `apps/room/docs/streaming-choices.md`.** Rows 2 and 4 shipped: the recorder picks VP9 at 8 Mbps (`recording-codec.ts`) and `contentHint = 'detail'` is set on the captured screen track. **STILL OPEN:** row 6 raising the 1920 cap for Retina (every member pays the bandwidth) and row 8 an explicit `maxBitrate` (a floor is exactly what hurts the member on the worst connection). Both need the same measurement, and it needs a human because `getDisplayMedia` requires an OS screen-picker dialog automation cannot click — the procedure is `apps/room/docs/MEASURE-SHARE-QUALITY.md`, ~5 minutes. Headless returns Chrome's synthetic gradient, which compresses too easily to show any difference. Row 10 (server-side remux, so MP4 is universal rather than Safari-only) needs the same cluster as X and AC. **`streaming-choices.md`'s TWO STALE LINES ARE CORRECTED, 2026-08-29** — the header no longer claims nothing but row 1 is implemented, and the "What is already true" table records `contentHint` as `'detail'` rather than unset. **This row's own citation for it was ALSO stale and is fixed here:** it named `media-transport.svelte.ts`, where `contentHint` has zero occurrences. The line is in `local-capture.svelte.ts`'s `startScreenSharing`, which took the produce paths when it was extracted — a third stale pointer of the same kind as the 44 in `missing-commands-triage.md`, found by verifying the row before acting on it | the owner, then a MediaMTX cluster |
...
| **AL** | **`deleteAlertPW` is not enforced, and the whole door is measured — 2026-08-30.** The setting's own help text is *"If set, Presenters will need to enter the password to delete an alert"*, and nothing in this room checks it. `message-actions.remote.ts`'s delete branch asks `usersCanDeleteOwnMsgs` of a MEMBER and lets a presenter through unconditionally — correct for chat, and for alerts it means an owner can configure this password and watch every presenter delete alerts unchallenged. The setting is `wired: false`, so nothing LIES about it; it does nothing, which is the quieter half of the same problem. **Everything needed is measured.** The capture is `archiveChatDate(e)` at bundle byte 2,048,903: `sessData.deleteAlertPW ? bootbox.prompt({title:"Please enter the password to delete this alert:", …, callback: i => i.trim() === sessData.deleteAlertPW ? send : bootbox.alert("Wrong password!")}) : send`. The controller half is a second route modelled on `internal/room-notes-auth` — **a second ROUTE and never a credential name on the first, because a name on the wire is an oracle over all seven** — and the comparison it would use already exists and is tested: `answerCredentialPrompt` in `room-credential-prompt.ts`, shipped 2026-08-30 with four negative controls. The room half is a `sessions.alert_delete_access_at` grant beside `notes_access_at` (its OWN column: clearing one password must not open the other), a `requireAlertDeleteAccess` mirroring `requireNotesAccess` including its TTL, and a prompt before the send. **It was built end to end and reverted**, for one reason: the client prompt costs about thirty lines in `message-actions.svelte.ts`, which is AT its ceiling, and shipping the server gate without the prompt is a regression — a presenter in a room with the password set would get a 403 and no way to satisfy it. **The blocker is therefore an extraction, not a design question.** The candidates in that file are the delete branch's optimistic hide, its Q&A special case (a question has no captured-fixture twin, so there is nothing to patch back) and its confirm-copy ternary; picking one is a deliberate piece of work, not a side effect of another change | **MEDIUM — a configured password that does nothing; the door is designed, measured and blocked on one extraction** |
| **AN** | **`+page.server.ts` still exports a `logout` action that NOTHING posts to — found 2026-08-30 while converting row AG's seventeen, and measured rather than assumed.** `src/routes/logout/+page.svelte` renders `<form method="POST">` with no `action` attribute, so it posts to its OWN route, and `src/routes/logout/+page.server.ts` carries a `default` action with a byte-identical body — `logout(cookies)`, clear `locals`, `redirectSignedOut()`. A sweep of `src/` for `?/logout` in any quote style finds nothing, and `remote-call-sites-contract.test.ts` now asserts BOTH halves: that the room page exports exactly `['logout']`, and that no file posts to it. So there are two implementations of signing a member out and one of them is unreachable. It was left alone deliberately during that conversion — it is a progressive-enhancement form POST rather than a JS dispatcher, and the row that governed the work said to leave it — but a duplicate nobody has written down is how the wrong one gets fixed. **The work is to decide which one survives**, delete the other, and drop the corresponding half of that assertion. | **LOW — dead code, not a defect; the reachable path works** |
