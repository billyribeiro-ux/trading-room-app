...
| 6 | **No automated BROWSER check runs in CI.** `RoomNavbar`'s half of this row CLOSED on 2026-08-23: `room-navbar-render.test.ts` serves the 904-line bar through `svelte/server` and pins the presenter gate in the markup a member actually receives — seven cases, negative-controlled by forcing `{#if isPresenter}` open and watching the member assertion go red. One draft assertion of mine was wrong and the component right (`media.screenSharing` correctly withholds "Stop Sharing All" from a presenter who is not sharing), kept two-sided rather than deleted. **What remains is the larger claim:** no Playwright and no real browser, so every render assertion in this repository is SSR or jsdom | a browser in CI, which is work rather than a blocker |
| 7 | **The `privCmdsIn` channel carries SIX commands this room does not handle.** Found by READING bytes 995300-996700 end to end while confirming `forceReload`, rather than by searching for it — the whole switch was in the same region. Upstream: `forceReload` ✓, `unmuteChat` ✓, and then `remoteRestartAudio`, `getDebugLog` / `debugLogResp` (a pair — a presenter asks, the member replies with `V1`, its rolling client log), `kickUser` (`emit("kickPage", xe.msg), e.disconnect()` — note the order is the OPPOSITE of `forceReload`'s), `muteChat` (the receiver half of a mute this room enforces server-side instead), and `userInfo`. Each was checked against `apps/room/src` before being written down: `remoteRestartAudio` appears nowhere, and the others appear only in unrelated contexts. **Not invented into existence** — what each SHOULD do beyond the emit is not in the room bundle | a capture per command, then the wire |
| 8 | **Three more `bootbox.alert(msg, callback)` receivers exist upstream and none is wired here.** Same region, bytes 2596600-2597200, read end to end: the room reset — *"The room is being reset by an administrator. Click OK to continue..."* → reload; `openSession` — *"The session is now open, click here to reload the page and enter"* → reload; and `permsChangeReload` — *"An admin has changed your room permissions, you need to reload this page to continue"* → navigates to `${apiROOT}/sessions/v2/reAuthSessionTok?sessionID=…&tok=…&r=1` rather than reloading. **The PRIMITIVE they need now exists** (`RoomDialogs.alertThen`, shipped with `forceReload`), so each is a receiver plus a sender, not new plumbing. `permsChangeReload` is the odd one — it re-mints a session token against an endpoint this room does not have | the sender for each; the dialog half is built |
| 9 | **79 macOS duplicate files (`* 2.*`) sit in the working tree, and 25 of the room app's local lint errors come from them.** None is tracked by git and 78 of the 79 are gitignored, so **CI never sees any of this** — `eslint .` reports 28 locally against 1 in CI. Classified rather than assumed: 56 are byte-identical to their original; 22 differ and in **all 22 the original is newer**; the three largest were read line by line and are strictly older superseded versions — `room-config-seam-e2e 2.mjs` still carries the backtick-in-template-literal bug the live file documents as fixed. `apps/room/.data/proroom 2.sqlite-wal` is a **0-byte orphaned WAL** with no matching database. `CHANGELOG 2.md`, the only one git could see, has been removed. **The other 78 were NOT deleted unilaterally** — they are untracked, so deletion is unrecoverable, and that is the owner's call. **The root cause matters more than the clutter:** something is duplicating files in the working tree, and `apps/room/scripts/` holds scripts that are untracked and absent from a fresh checkout (see row E), so a sync conflict landing on the wrong copy could lose real work | an owner decision to delete; then find what is duplicating them |
