...
| # | open item | needs |
| --- | --- | --- |
| 1 | **`kick-duplicates` reports a hardcoded negative — AND IT IS BUILDABLE TODAY.** I wrote that it needed `emailHash` on the roster. **Wrong, audited 2026-08-23:** `apps/room/src/lib/types.ts:58` declares `emailHash: string` on `User`, and `+page.server.ts:318` fills it with `hashEmail(account.email)`. `RoomUserActions` already reads `this.#session().connectedUsers` (`user-actions.svelte.ts:265`), which is `readonly User[]` — so every roster entry carries `emailHash` and `id` right now. `RosterAuthority` is a NARROW local interface for `mute-all-non-admins` only; it was never the roster. **`kickUser` exists (built today).** Nothing is missing | NOTHING. Iterate `connectedUsers`, match `emailHash` with a different `id`, send one `kickUser` each, count, alert one of the two captured strings |
| 2 | **`admin-notes-password` alerts `"Wrong password!"` unconditionally.** I wrote that `deleteAlertPW` appears nowhere. **Wrong — I only grepped `apps/room/src`.** `apps/controller/src/lib/room-settings-schema.ts:88` defines it: `{ name: "deleteAlertPW", section: "settings", type: "textarea", label: "Delete Alert Password", help: "If set, Presenters will need to enter the password to delete an alert", wired: false }`, typed at `:385` as `deleteAlertPW: string`, with a fixture at `room-config-boundary.test.ts:28`. **It is `wired: false`** — the setting exists and is deliberately gated from crossing to the room by `room-config-boundary.test.ts` | flip `wired` to `true` for `deleteAlertPW` and let it cross — a governed change, not a missing field |
...
| 6 | **`kick` sends; `kick-ban` IS BUILDABLE and I said it was not.** Built 2026-08-23. I wrote that a ban "needs durable storage this room does not have". **Wrong, audited 2026-08-23:** `apps/controller/src/lib/server/db/schema.ts:335` is `banned: boolean('banned').notNull().default(false)` on `room_users`, and the role model directly above it at `:322` reads *"0 owner · 1 presenter · 2 participant · 3 chat muted · 4 banned"* — **two** durable representations of a ban, both already migrated. `banIPList` and `isBannedIp()` (`room-entry.ts:197`, used at `:227`) are a third, on the entry path. Still genuinely missing: upstream's `app-kicked-page` (`currPage="kicked"`) | `kick-ban` needs a controller write to `room_users.banned`, like `savePermissions` does — NOT a new store. A kicked page is still absent |
| 7 | **FIVE controls say it worked and do nothing — now GATED, 2026-08-23, so a sixth cannot join them quietly.** `kick-duplicates` and `admin-notes-password` (rows 1-2), `session-save-close-message` (`"Message Saved"`), `session-send-sales-image` and `session-send-users-url` (both `"Command send OK."`, sharing a branch with the GENUINE `session-send-video`). **The contract now asserts the fourth disposition it used to deny**: a `handled` branch must reach a command or write state, or be declared in `DIALOG_ONLY_ACTIONS` with what is missing — with a staleness half that fails if a declared entry is later wired, so the list cannot become a suppression file. Notices are a SEPARATE list (`copied-to-clipboard`, `invalid-restream-link`): a message about something the caller already did is not a lie. Negative control seen RED. **Building the five still needs what rows 1-3 need** | the roster's `emailHash`; `deleteAlertPW`; somewhere to save the close message; two send commands |
