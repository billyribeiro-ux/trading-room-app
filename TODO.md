...
| 2 | **`admin-notes-password` alerts `"Wrong password!"` unconditionally. DO NOT FIX IT BY FLIPPING `wired`.** The target exists — `apps/controller/src/lib/room-settings-schema.ts:88` defines `deleteAlertPW` ("Delete Alert Password", *"If set, Presenters will need to enter the password to delete an alert"*), typed at `:385`, fixtured at `room-config-boundary.test.ts:28`, carrying `wired: false`. **I recommended flipping it to `true` and that recommendation was WRONG, corrected 2026-08-23 before anything was built.** `room-config-boundary.test.ts:116-138` shows `wired` means the setting CROSSES TO THE ROOM, so flipping it ships the password to every client and makes the check `typed === globals.sessData.deleteAlertPW` in the browser — a client-side authority decision, readable in devtools. That is the 2026-08-07 privilege-escalation shape, and the repository forbids it "ever, for any reason". **The reference does exactly that and we must not copy it here**; this is the same category as the six media controls, where the wire is captured and the SERVER gate is what is missing | a server command that takes the typed password, compares it against the room's `deleteAlertPW` server-side, and returns allow/deny. `deleteAlertPW` STAYS `wired: false` |
| 3 | **`session-refresh-roster` and `session-soft-reset` alert a captured string and send nothing.** Both server behaviours ARE captured — the reference states them on screen: refresh roster *"clears the user list and forces all 'stale' connections out"*, soft reset *"Resets the media state of the room, Makes all users reconnect to the media servers gently"*. The method is `softReset()`, not `softResetSession()`. Audited: neither name appears in `apps/controller/src` or `services/**` | two server commands, gated like `kickUser`. This one really is absent |
| 4 | **Ten inert controls.** Nine have their wire captured (see the CHANGELOG of 2026-08-23 15:20 and 16:10). Six are `remotePresCommand` sub-commands — `mutemic`, `mutecam`, `mutescreens`, `restartScreen`, `startRec`, `stopRec`. `disable-private-chat` is a MATCH: upstream renders it with no click binding either. `get-my-token` is fully evidenced and merely unbuilt | a SERVER-side presenter check in `services/**` for the six media controls — the 2026-08-07 rule, not evidence. `get-my-token` needs only building |
...
| 6 | **`kick-ban` — every part exists and the route is mapped; it is a cross-app authorization change, so it is written out rather than half-started.** `kick` sends (2026-08-23). I twice claimed a ban needed storage this room lacks; **both claims were false**: `apps/controller/src/lib/server/db/schema.ts:335` is `banned: boolean('banned').notNull().default(false)` on `room_users`, the role model at `:322` reads *"0 owner · 1 presenter · 2 participant · 3 chat muted · **4 banned**"*, and `banIPList` + `isBannedIp()` (`room-entry.ts:197`, used `:227`) is a third representation on the entry path. **THE PRECEDENT IS `savePermissions`, read end to end 2026-08-23** — copy its shape exactly: (1) a controller endpoint beside `src/routes/internal/room-permissions/`, deny-by-default, that re-checks the CALLER is owner/presenter **of that room** and the TARGET is a member of it — because room role and room membership are two facts and only the controller owns the second; (2) a `writeRoomBan(shortCode, callerEmail, targetEmail, banned)` in `apps/room/src/lib/server/room-config-client.ts:620`'s shape, signed with `ROOM_JWT_SECRET`, target named by EMAIL not id (ids do not cross the seam); (3) widen `kickUser` in `presenter-commands.remote.ts` to take `ban`, matching the capture's `{user,msg,ban,kickAllInstances}` — one command, as upstream sends it; (4) `RoomKicks.handle` gains the `kick-ban` branch; (5) delete its `INERT_ACTIONS` entry, at which point the disposition contract demands a real handler. **AUTHORITY IS DECIDED TWICE, deliberately** — `presenterRoom()` refuses a member before a request leaves the room process, and the controller re-checks anyway; neither side may be the only check, which is the 2026-08-07 rule. Still absent and unrelated: upstream's `app-kicked-page` | the five steps above, in one commit with a negative control |
| 7 | **THREE controls say it worked and do nothing — down from five, gated so a sixth cannot join quietly.** Fixed 2026-08-23: `kick-duplicates` (row 1) and **both `"Command send OK."` liars** — `session-send-sales-image` and `session-send-users-url` now call `sessionSendUrl` in `for-all-broadcast.remote.ts`, presenter-gated by `presenterRoom()`, URL-parsed by `broadcastableUrl`, broadcast on `cmds` under the capture's own names `sendSalesImageToChat` / `sendUsersToURL` (bytes 1015180 / 1015357). **Their RECEIVER half is NOT built** — the reference emits an event and what its app does with it was not read, and upstream also excludes the presenter from their own broadcast (`globals.isPresenter ||`). So the send is real and members do not yet act on it. Still lying: `admin-notes-password` (row 2), `session-save-close-message` (`"Message Saved"`, nowhere to save it), and `kick-ban` — which is now INERT rather than lying. The contract asserts the fourth disposition and its staleness half has now correctly evicted three entries as they were built | the two receivers; somewhere to save the close message; rows 2 and 6 |
...
| 9 | **The `privCmdsIn` switch carries ELEVEN cases and both halves of every one are captured** — including `remotePresCommand`, `updateProfilePic` and `getRoster`, which were never recorded. `getRoster` sorts presenters first then ascending `loggedIn`. Senders are all in the user-info modal with their exact alert text | building each receiver |
| 10 | **Three `bootbox.alert(msg, callback)` receivers, senders found.** `hardReset` calls `disconnectAll()` BEFORE its alert; the sender is `case"changeUserPerms"` on `/cmds/`. Only `permsChangeReload`'s target is genuinely absent — it navigates to `${apiROOT}/sessions/v2/reAuthSessionTok`, an endpoint outside the room bundle | building two of them; the third needs that endpoint |
