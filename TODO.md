...
`room-settings-schema.ts` declares **269** settings and marks **181** of them `wired: false` —
...
**37 of the 181 are read by the reference's OWN room client**, as `sessData.<name>`.

**Twenty-one have already left the list, on the day it was written.** It opened at 202 unwired and
58 questions; both numbers have moved seventeen times since.
...
`src/lib/setting-coverage-contract.test.ts` pins the 37 by NAME and asserts separately that the seven
