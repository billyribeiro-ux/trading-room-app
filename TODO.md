...
| --- | --- | --- | --- |
| M | **Neither app has a check that boots the built server and fetches a page.** Two total outages in one day got past `svelte-check`, 566 and 524 passing tests, and clean builds — because every one of those inspects source or bundles, and neither starts the artefact that ships. The room's Kit 3 env bug (`$env/dynamic/private` empty at runtime) and the controller's gsap externalisation (`Cannot use import statement outside a module`) are both invisible until a built server runs against a real environment. The room now has a one-line smoke test (`/session?…&jwtSite=bogus` → 403, documented in `DEPLOYMENT.md`); the controller has none, and `/` returned 500 for hours while every gate stayed green. A post-deploy probe of `/` and `/login` would have caught it in seconds. | **HIGH — this is how both outages reached production** | `CHANGELOG.md` 2026-08-10 04:24 and 04:35 |
