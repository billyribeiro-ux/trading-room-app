...
| AK  | **`FilesPane` — the one pane inside `PresentationArea` with no component of its own.** Measured 2026-08-15 22:48, not estimated: roughly **480 lines** of the component's 1,652 and about **25 identifiers** — the toolbar (Delete Selected / Refresh / Upload File / Stop Playing For All), the sort bar, the search input and the files table with its per-row Download / Delete / Play / Play For All / alert-sound controls. **Everything it needs is already named**, because the props are `PresentationArea`'s and the compiler produced them: `data.files`, `filesHidden`, `fileTab`, `fileSort`, `selectedFileIds`, `playingForMe`, `mp3Playing`, `isPresenter`, `mountUploadFileLink`, `countFiles`, `searchedFiles`, `matchesFileTab`, `applyFileSort`, `toggleFileSelection`, `deleteSelectedFiles`, `deleteFile`, `playMp3ForMe`, `playMp3ForAll`, `stopMp3ForAll`, `setAlertSound`, `openModal`, `onfilesearch`, plus the `alertSoundButtonFor` / `fileSizeInKb` / `fileSortTitle` / `mediumDate` / `invalidate` imports. **NOT done in the same pass as `PresentationArea` on purpose** — two extractions at once is how a mangle ships, and that pass already re-pointed 40 assertions across 11 files. **The contract file is already prepared for it:** `files-pane-contract.test.ts` now reads TWO sources, `pane` for markup and `page` for the handlers behind it, so this extraction re-points `pane` and nothing else. **Watch the same two traps that bit this session:** a mechanical source-constant replace must cover `lastIndexOf` as well as `indexOf`/`slice`/`match`, and it must not rename a LOCAL variable that happens to share the constant's name — both happened and both were caught by asserting the substitution count before writing. **And add `components/FilesPane.svelte` to `EXTRACTION_SOURCES` in `source-size-contract.test.ts` in the same commit**, or any file re-pointed at it drops silently out of the vacuity police, which has now happened twice. | MEDIUM — the last inline pane; `PresentationArea` 1,652 -> ~1,170 | this row; `CHANGELOG.md` 2026-08-15 22:48 |
