...
| Q | **The WordPress plugin has never been executed.** `integrations/wordpress/tradingroom-sso/tradingroom-sso.php` was written against the WordPress and WooCommerce APIs, and its contract with our verifier is covered by 12 tests — including the two PHP-specific encoding hazards (`wp_json_encode` escaping forward slashes, and `json_encode` emitting `{}` for a non-sequential array). **But the PHP itself has never been parsed or run:** PHP is not installed on this machine and Docker Hub was unreachable (`context deadline exceeded` pulling `php:8.3-cli`), so neither `php -l` nor a real mint was possible. Closing it is one command wherever PHP exists — `php -l integrations/wordpress/tradingroom-sso/tradingroom-sso.php` — followed by an install against a staging WooCommerce with a real subscription, cancelled mid-test to prove the door closes. **Do not ship it to a customer before that.** | blocks the first WordPress customer | `integrations/wordpress/README.md`, §Status |
