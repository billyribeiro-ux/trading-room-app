...
`room-settings-schema.ts` declares **269** settings and marks **193** of them `wired: false` —
...
**49 of the 193 are read by the reference's OWN room client**, as `sessData.<name>`.

**Nine have already left the list, on the day it was written.** It opened at 202 unwired and 58
questions; both numbers have moved seven times since.
...
`src/lib/setting-coverage-contract.test.ts` pins the 49 by NAME and asserts separately that the seven
