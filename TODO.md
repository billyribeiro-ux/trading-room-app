...
| X | **No `addUser` route exists, on either host.** Not an evidence gap — the shape is fully captured. The reference renders a read-only `<input class="form-control col-md-6">` holding `https://chat.protradingroom.com/ptr_app/sessions/v2/addUser/<publicId>/?sec=<pairSecretKey>&email=__userEmail__&name=__userName__`, on the ROOM host rather than the controller, with those two placeholders literal. `sec=` came back empty on the captured room because it has no `pairSecretKey` set, so a run against a room with "Pair Link For App?" enabled would show a populated key — worth doing before building, but not blocking. What is missing is the endpoint itself. | blocks self-serve mobile pairing | `CHANGELOG.md` 2026-08-11 14:55; `evidence-manage-gaps-2026-08-11.json` |
