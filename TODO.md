...
| AL  | **`apps/controller/.env` points local dev at `newroom_control_dev`, a database seven migrations behind.** Found 2026-08-15 21:38 by reading the cluster after connecting to the wrong database and drawing a wrong conclusion from it. `applied_migrations`: **`tradingroom_dev` is at 12** (`stream_ingest_keys`), **`newroom_control_dev` is at 5** (`open_existing_rooms`). So anything run locally against `.env` — the app, a manual `psql` check, a seed — talks to a schema missing `room_sessions`, `recorded_max_capacity`, `stream_ingest_keys` and four others, and it answers like a working database rather than failing. That is how a local check can disagree with CI and with production while looking healthy. **The runtime role is `tradingroom_app` and the current database is `tradingroom_dev`**; `newroom_control_dev` is a legacy name in the same family as `ptr_clone` (`ops/naming-provenance.md`). **NOT repointed here:** `.env` is local configuration and not mine to edit, and which database a developer's env should name is the owner's call. The fix is one line if the answer is `tradingroom_dev`. | MEDIUM — a local database that lies by being seven migrations old | this row; `apps/controller/.env`; `ops/naming-provenance.md` |
