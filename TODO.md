...
| 7 | **`kick` AND `kick-ban` TELL THE PRESENTER "User kicked OK" AND SEND NOTHING — and no gate catches it, because they sit in the `handled` bucket.** Found 2026-08-23 while checking whether `kick-duplicates` could be built. `user-actions.svelte.ts:618-629` opens a prompt seeded with the captured default message, and on confirm closes the modal and sets `dialogs.alert = 'User kicked OK'`. There is no command in that branch, and there is no command to call: `UserActionCommands` has `presenter`, `editUsername`, `unmuteChat`, `forceReload`, `savePermissions` and nothing else, and the only `kickUser`/`kickPage` text in `apps/room/src` is inside a comment at `events.svelte.ts:662`. **This is worse than the eleven inert controls, which at least stay silent**: a dead control that says nothing is a gap, a control that reports success is a lie, and `user-action-disposition-contract.test.ts` counts these as `handled` because a branch exists. The three buckets are handled / alerted / inert, and this is a FOURTH thing the contract cannot see — a branch that dialogs and does not act. Upstream sends `sendServerAdminCommand("kickUser",{user,msg,ban,kickAllInstances})`, then `setPreference("kickMsg",msg)`, then `doCloseModal()`, then the alert — so our message text and ordering are faithful and the SEND is missing | a `kickUser` command, server-gated like `forceReload`; and a contract change so "has a branch" stops counting as handled |
