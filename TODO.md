...
| 6 | **No automated BROWSER check runs in CI — and the harness ALREADY EXISTS, which reframes this row entirely.** `RoomNavbar`'s render-test half closed 2026-08-23 (`room-navbar-render.test.ts`, SSR, presenter gate negative-controlled). **Corrected 2026-08-23 after reading rather than assuming: `apps/controller` has `@playwright/test` 1.62.1, a `playwright.config.ts` with `testDir: 'e2e'` and a `webServer`, a `test:e2e` script, and FOUR written specs** — `critical-journey`, `hydration-contract`, `manage-room`, `responsive-contract`. **Nothing in `.github/workflows/*` mentions `test:e2e` or `playwright`, so none of them has ever run in CI.** So this splits into two unequal halves: (a) invoke the EXISTING controller suite from a workflow — small, and the specs are already there to tell you whether they pass; (b) `apps/room` has no Playwright at all, which is the real build. Do (a) first and measure what it reports before writing (b) | (a) a workflow job; (b) a room harness |
