...
| AI  | **The second chat column renders the same messages WITHOUT reactions, edit or public reply.** Found 2026-08-15 20:30 by READING all four `RoomMessage` call sites while bundling the sixteen shared props into `RoomMessageChrome`. `ExtraChatPane.svelte`'s `Props` declares thirteen of the sixteen — `theme`, `chatStyle`, `chatGif`, `chatBadges`, `enableBadges`, `showBadgesToPresentersOnly`, `disableStarYears`, `presenterMessagesOnTheRight`, `currentUserId`, `currentUserEmailHash`, `currentUserName`, `viewerIsPresenter`, `followedUsers` — and **does not declare `usersPublicReply`, `enableReactions`, `enableEditMessage` or `enableEditAlerts` at all**, so its `RoomMessage` call cannot forward them and they fall to their `false` defaults. The two columns show the SAME channel, so one message can carry a reaction bar in the main column and none in the second. **`message-gates-contract.test.ts` was written against exactly this defect** — its header names it *"reactions work in chat but not on alerts"* — and it cannot see this one, because it reads the page's two call sites and this is a third list one component further out. That is the limit of a text guard scoped to one file, which is the same lesson `remote-call-sites-contract.test.ts` exists for. **NOT FIXED, because it is a behaviour change and the owner's call:** handing `ExtraChatPane` the `messageChrome` object closes it in one line, and would turn reactions and both edit entries on in the second column for every room that has them enabled. Whether the second column is meant to be a full peer of the first or a lighter read-only view is a product decision, not a refactor. | MEDIUM — a capability the owner enabled is silently absent in one of two columns | this row; `apps/room/src/lib/components/ExtraChatPane.svelte`; `apps/room/src/lib/room-message-chrome.ts` |
