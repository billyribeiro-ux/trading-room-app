...
| **AK** | **The per-user ADMIN NOTES list is not built — `#user-modal`'s notes tab renders its password prompt and nothing else. Found 2026-08-29 by an orphan CSS class, `smallAvatarImg`.** The tab exists (`ModalHost.svelte`, `id="nav-notes"`) and so does its gate, which was fixed the same day — `admin-notes-password` used to alert `Wrong password!` whatever was typed and the comparison is real now. What is missing is everything BEHIND the gate: `{#if !canManageNotes}` has **no `{:else}`**, so clearing the password reveals an empty panel. Upstream, read whole rather than guessed from neighbouring consts: `pTe` is the false branch this room has; `mTe` @ 2,065,140 is the true one — const 93, a `col` scrolling at `max-height:300px`, an `@for` over the notes, an `<hr>`, and an " Add Note " button (const 95 + `fa-plus-circle` 96) calling `addNote()`. Each row is `fTe` @ 2,064,959: `<img class="smallAvatarImg" [src] [alt]>` with `src = e.pic || "https://secure.gravatar.com/avatar/" + e.emailHash + "?d=mm&s=80"` and `alt = user.nick` — note the avatar is the NOTE AUTHOR's while the alt text is the SUBJECT's, which is upstream's own inconsistency and is recorded rather than silently corrected — then `" [" (e.date | date:"short") "] " e.name ": " e.note " "`, then a `fa-minus-circle` button (99) calling `deleteNode(note, $index)`. **So a note carries `{pic, emailHash, name, date, note}` and is addressed BY ORDINAL for deletion**, which is the same parent-plus-ordinal addressing the Q&A thread uses and the same reason: these rows have no id of their own. **What this needs that does not exist here:** a durable per-user notes store, an add and a delete command, and a read — the read is row 9's `userInfo` frame, whose payload carries `notesArr` beside `privData` and `badges`. The two rows are one piece of work and should be built together | **MEDIUM — a gate that now works correctly opens onto nothing** |
