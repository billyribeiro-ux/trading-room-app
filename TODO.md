...
| 10 | **RE-READ FIRST-HAND 2026-08-26, and the row conflated a receiver with a sender.** All four callback receivers sit together at bytes 2596540-2597340 and were read end to end — they are already transcribed in `dialogs.svelte.ts:63-70`, and **the machinery is BUILT**: `alertThen(message, ondismiss)` exists, is tested, and `forceReload` already uses it. The four: (1) **`hardReset`** — `subscribe("hardReset", () => { disconnectAll(), bootbox.alert("The room is being reset by an administrator. Click OK to continue...", () => location.reload()) })`; its sender frame is `case"hardReset": emit("hardReset"), this.disconnect()` at 1013595 — **emit THEN disconnect**. (2) **`openSession`** — `bootbox.alert("The session is now open, click here to reload the page and enter", () => location.reload())`; sender `case"openSession": this.openSession()` at 1013476. (3) `forceReload` — BUILT. (4) **`permsChangeReload`** — `bootbox.alert("An admin has changed your room permissions, you need to reload this page to continue", () => location.href = `${apiROOT}/sessions/v2/reAuthSessionTok?sessionID=…&tok=…&r=1`)`; its sender is `case"changeUserPerms": this.disconnect(), emit("permsChangeReload")` at 1025048 — **disconnect THEN emit**, the opposite order to `hardReset`, and that asymmetry is upstream's. **THE ROW'S SENTENCE "`hardReset` … the sender is `case"changeUserPerms"`" WAS WRONG** — `changeUserPerms` sends `permsChangeReload`, not `hardReset`. **What actually blocks the two buildable ones is NOT the receiver:** our room's `session-hard-reset` and `session-open` write PREFERENCES (`savePreference`) and broadcast nothing, so connected clients are never told. The missing piece is a SENDER, not a receiver | two broadcast senders so the existing `alertThen` receivers have something to receive. `permsChangeReload` stays blocked: `reAuthSessionTok` is confirmed absent from the bundle |
