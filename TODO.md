...
| **R** | **Screenshare quality and the MP4 question — rows 6, 8 and 10 of `apps/room/docs/streaming-choices.md`.** Rows 2 and 4 shipped: the recorder picks VP9 at 8 Mbps (`recording-codec.ts`) and `contentHint = 'detail'` is set on the captured screen track. **STILL OPEN:** row 6 raising the 1920 cap for Retina (every member pays the bandwidth) and row 8 an explicit `maxBitrate` (a floor is exactly what hurts the member on the worst connection). Both need the same measurement. **The reason it has not been taken was RE-MEASURED on 2026-09-01 and the recorded one was wrong:** the row said `getDisplayMedia` needs an OS screen-picker dialog automation cannot click. Chromium ships `--auto-select-desktop-capture-source` for exactly that, and it works — driven under `xvfb-run` with headed Chromium 1194 the picker never appears and the call never returns `NotAllowedError`. It returns `NotReadableError: Could not start video source` on six attempts, because this container's Xvfb display has no surface the X11 capturer can open (`preferCurrentTab`, `--enable-usermedia-screen-capturing`, `--use-gl=swiftshader` and `+extension COMPOSITE +extension DAMAGE +extension RANDR` all tried). **So this is blocked on a CAPTURABLE DISPLAY, not on a PERSON** — an environment fixed once rather than a per-run human cost, which is a very different price. The synthetic-gradient claim is about `--use-fake-device-for-media-stream`, a different flag for `getUserMedia` cameras that was not used in any attempt. The human procedure in `apps/room/docs/MEASURE-SHARE-QUALITY.md` (~5 minutes) stays correct and stays the fastest way to get the number today, and that doc now carries the same correction at its own "Why it cannot be automated" heading. **Row 10 is re-scoped 2026-08-31, measured rather than inherited:** it does NOT need the same thing as X and AC. Server-side recording was RUN — MediaMTX v1.20.1 with `record: yes` wrote an `.mp4` and its playback server listed it — so what row 10 waits on is a decision to turn that on (T5-16), because there is nothing to remux until something records. The fmp4 recorder skipped the VP8 track and kept only Opus, which is the finding row 10 has to answer first: a screenshare recorded server-side is audio-only unless the publisher sends a codec the recorder keeps. See `apps/room/docs/MEDIA-PLANE-MEASURED.md`. **`streaming-choices.md`'s TWO STALE LINES ARE CORRECTED, 2026-08-29** — the header no longer claims nothing but row 1 is implemented, and the "What is already true" table records `contentHint` as `'detail'` rather than unset. **This row's own citation for it was ALSO stale and is fixed here:** it named `media-transport.svelte.ts`, where `contentHint` has zero occurrences. The line is in `local-capture.svelte.ts`'s `startScreenSharing`, which took the produce paths when it was extracted — a third stale pointer of the same kind as the 44 in `missing-commands-triage.md`, found by verifying the row before acting on it | rows 6 and 8: a capturable display — NOT a human; re-measured 2026-09-01. Row 10: the T5-16 recording decision |
...
**AC — `stopRecMsg`.** Every measurement in the row was exact and was confirmed again by reading the
bundle: three occurrences, the emitter 35 bytes after its own `case` label at 1,014,265, the
subscriber at 2,505,283. The receiver is now in `events.svelte.ts` with the capture's case-sensitive
`indexOf("Stopped")` severity switch and its browser notification. Two things differ and both are
stated at the code: the toast goes through `RoomToasts` (`enableHtml: false` — a match, since
ngx-toastr's default is the same, but one this room must make deliberately for text off a socket),
...
receiver in `events.svelte.ts` the whole of `app-rec-preview` is transcribable, and this room's server
never sending the command is not a gap: the capture's own arming test fails, the card stays dark, and
that is what a reference room does in the same state. `recPreviewWindow` now has three readers.
`rec-preview-contract.test.ts` mounts the card and drives every subscription.

**One decision unblocks R's row 10, and it is not "build server-side recording."** The
