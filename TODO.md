...
| R | **Recording downloads in MP4, and the member-side screenshare quality — both were built in the OTHER folder/repo and must be re-established here.** Raised by the owner 2026-08-10. Two separate pieces: (1) **recordings download as MP4** rather than whatever the recorder emits — the reference's Recordings tab offers a download, and `hideRecs` / "Disable download button for Recordings for users" are already in the settings schema, so the surface exists here even though the pipeline does not; (2) **the resolution a MEMBER receives from a presenter's screenshare** was tuned upward there, which is the quality complaint that matters most to a paying viewer. **Nothing has been measured here yet** — this row is a placeholder for the review, not a description of a defect, and the first task is to find what that repo actually did rather than re-derive it. Relevant knowns on our side: `docs/streaming-choices.md` measured VP9 screen share at **3841 kbps** on realistic chart content; `setPreferredLayers` IS implemented in `services/media` (deliberately beyond the capture) so per-consumer layer choice exists; `useH264`/`useVP9`/`useHQVideo` are in the settings schema and all `wired: false`. | owner-raised; affects every viewer | **not yet written up — start by diffing the other repo's recorder and media settings** |
