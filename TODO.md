...
| 9 | **DONE 2026-08-23 — the 79 duplicates are out of the tree**, moved to `/tmp/trading-room-dup-quarantine` on the owner's grant rather than deleted, so today's action is reversible. `eslint .` over `apps/room` now reports **0 errors across every file**, down from 28. **The root cause is NOT fixed and is what to watch:** something duplicates files in this working tree, and `apps/room/scripts/` holds work that is untracked and absent from a fresh checkout (row E), so a conflict landing on the wrong copy could still lose real work | find what is duplicating them |
