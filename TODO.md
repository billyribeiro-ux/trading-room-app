...
| # | open item | needs |
| --- | --- | --- |
| 1 | **EXTRACT SESSION CONTROL OUT OF `RoomUserActions` — the seam is measured and this is the next action.** The suite is RED: `user-actions.svelte.ts` 747 against 730, `create-room.svelte.ts` 1088 against 1086, so the `forceReload` fix does not ship until it is under. Two ceilings were briefly raised on the inference that an earlier approval carried over; reverted, because the contract's own failure message states the decision — *"extract a slice into a module or component rather than raising this number."* **The seam is proven by the file's own `why:` string**, "everything that can be done TO a user": ten of its branches act on the SESSION, not on a user — `session-reload-config` (15 lines), `session-send-video` (20), `session-soft-reset` (9), `session-hard-reset` (9), `session-lock` (7), `session-refresh-roster` (7), `session-open` (6), `session-unlock` (6), `session-save-close` (6), `session-save-close-message` (5) = **90 lines against the 17 needed**. **The one coupling that must move with it:** `user-action-disposition-contract.test.ts:82` reads `user-actions.svelte.ts` alone to learn which actions are handled, so it must read BOTH files or every moved action reads as "dispatched into the void" | the extraction; scope and coupling both identified |
