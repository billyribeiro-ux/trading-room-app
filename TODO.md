...
| 2 | **CONFIRMED DIVERGENCE — our `forceReload` reloads the member's page with NO WARNING; the reference asks first.** Checked 2026-08-23 rather than left as a question, and it is worse than the "does it disconnect" I originally wrote down. The reference, byte 995901: `case"forceReload":e.disconnect(),e.appEventBus.emit("forceReload")`. Its SUBSCRIBER, byte 2597102: `appEventBus.subscribe("forceReload", () => bootbox.alert("You need to reload this page to continue", () => window.location.reload()))`. So upstream **disconnects, warns, and reloads only when the member dismisses the dialog**. Ours (`events.svelte.ts:679-681`) calls `location.reload()` immediately — a member mid-sentence loses it with no notice. **What it needs:** `RoomDialogs` has no alert-with-dismiss-callback (`alert` is a plain string; only `confirm` carries a handler), so the captured `bootbox.alert(msg, cb)` shape needs adding to the primitive first. Using `confirm` instead would put a Cancel button on a dialog the reference does not give one — a second divergence to paper over the first. `source.close()` is separately available at the receiver, which is already inside the closure that owns the `EventSource` **MEASURED BLOCKER, 2026-08-23:** all three files this fix touches are EXACTLY at their ceilings — `events.svelte.ts` 903/903, `RoomOverlays.svelte` 769/769, `dialogs.svelte.ts` 115/115 — so it cannot be written without an extraction first. `events.svelte.ts` was checked for slack and has none: zero unused imports, zero consecutive blank lines. This is the ratchet working as designed rather than a fault; the next change to this area has to pay down size debt before it can add behaviour, and neither raising a number nor shortening a comment is available **THE SEAM IS MEASURED.** `events.svelte.ts` dispatches six channels — `cmds` 314 lines, `chat` 150, `roster` **93**, `privChat` 43, `privCmds` 34, `cmdsAdmin` 7. The `roster` block (line 577, 93 lines) is self-contained and needs five collaborators: `isPresenter`, `prefs`, `roster`, `session`, `toasts`. 93 lines against the ~8 this fix needs | extract the roster channel, then the alert-with-callback primitive, then the receiver |
