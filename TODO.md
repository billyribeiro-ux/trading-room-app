...
| # | open item | needs |
| --- | --- | --- |
| 1 | **`kick-duplicates` reports a hardcoded negative.** `kickDuplicates()` is read in full at byte 2078708 of the v4 bundle and is entirely client-side: match `globals.roster` on `emailHash` with a different `_id`, send one `kickUser` each, count, and alert ``"Kicked "+n+" duplicate(s) of "+nick`` or ``"No duplicates found for "+nick``. **`kickUser` now EXISTS** (built 2026-08-23), so only one thing blocks it | `emailHash` and `_id` on the roster this class can see — `RosterAuthority` is `{id, isP?}` |
| 2 | **`admin-notes-password` alerts `"Wrong password!"` unconditionally.** The comparison target is `sessData.deleteAlertPW`, a `.trim()` compare with its own wrong-password branch, read at bytes 2048693 / 2051139 / 2167386. It appears **nowhere in `apps/room/src`** | `deleteAlertPW` delivered to the room in its session config |
| 3 | **`session-refresh-roster` and `session-soft-reset` alert a captured string and send nothing.** Both server behaviours ARE captured — the reference states them on screen: refresh roster *"clears the user list and forces all 'stale' connections out"*, soft reset *"Resets the media state of the room, Makes all users reconnect to the media servers gently"*. The method is `softReset()`, not `softResetSession()` | two server commands, gated like `kickUser` |
...
| 6 | **`kick` sends, but two halves remain.** Built 2026-08-23. Still missing: a durable ban store, which is why `kick-ban` is inert rather than aliased; and upstream's `app-kicked-page` (`currPage="kicked"`), which this room has no equivalent of — the member is told why and left disconnected | a ban store; a kicked page |
