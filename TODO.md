...
| # | open item | needs |
| --- | --- | --- |
| 1 | **`askQuestion` has no mute gate — and NEITHER DOES THE REFERENCE, so this is a MATCH, not a gap.** The row said there was "no evidence either way" and asked for a capture script. The evidence was in `docs/source/components/app-alert-qa-modal.full.js` the whole time. Its send path is `sendMessage(){ if(!this.canPost) return void bootbox.alert('Sorry, you cannot post to this channel'); const e=yi('#textAreaQATxt').val().toString().trim(); if(!e) return !1; this.appService.sendAlertQAReply(this.qaMsg._id,e), yi('#textAreaQATxt').val(''), this.scrollToBottomQA() }` — so the only gate is `canPost`, **and `canPost` is set to `!0` at construction (line 187) and never reassigned anywhere in the component.** The sibling `canPostImages` IS computed (line 203, from `isPresenter \|\| sessData.userUploads`), which is what proves the constant is deliberate rather than an oversight: one flag is derived, the one beside it is not. A chat mute therefore does not silence Q&A upstream, and ours behaving the same way is faithful | NOTHING. Delete this row once someone confirms the reading |
| 2 | **`kick-duplicates` reports a hardcoded negative — and the row's own excuse was FALSE.** It said the positive arm "needs a kick this room cannot perform". It does not. `kickDuplicates()` is read in full at byte 2078708 of `docs/source-v4-2026-08-15/main.d1d09071be31f1ba.js` and is **entirely client-side**: it takes `user.emailHash`, `user.nick||user.name` and `user._id`, refuses if there is no `emailHash`, seeds a `bootbox.prompt` (`inputType:"textarea"`) titled ``"Kick all other duplicates of "+nick+" with the following message:"`` from `getPreference("kickMsg")` defaulting to `"You have been kicked from the room by an administrator"`, then walks `globals.roster` and for every entry with `emailHash===` the target's and `_id!==` the target's sends `sendServerAdminCommand("kickUser",{user:c,msg:a,ban:!1,kickAllInstances:!1})`, counting them. It closes the modal and alerts ``"Kicked "+n+" duplicate(s) of "+nick`` or ``"No duplicates found for "+nick``. **Every string, both arms and the whole loop are captured.** **I THEN WROTE "NOTHING. Build it" AND THAT WAS WRONG** — corrected the same day by checking instead of assuming: the loop calls `sendServerAdminCommand("kickUser", …)`, and **this room has no kick command of any kind.** `UserActionCommands` exposes `presenter`, `editUsername`, `unmuteChat`, `forceReload` and `savePermissions` — no kick — and the only `kickUser`/`kickPage` text anywhere in `apps/room/src` is inside a COMMENT in `events.svelte.ts:662`. So the row's original wording, "needs a kick this room cannot perform", was right in substance even though its reasoning was thin. **Also blocking: `RosterAuthority` is `{id, isP?}` only**, with no `emailHash` and no `_id`, so the duplicate match itself has nothing to match on | a `kickUser` command (server-gated), and an `emailHash` on the roster the class can see |
| 3 | **`admin-notes-password` — the comparison target EXISTS and is named.** The row asked for "a comparison target that exists"; it is `appService.globals.sessData.deleteAlertPW`, read at bytes 2048693, 2051139 and 2167386 of the v4 bundle. The pattern is `bootbox.prompt({title:"Please enter the password for this action:",value:"",callback:i=>{i&&(i.trim()===...deleteAlertPW ? <do it> : bootbox.alert("Wrong password!"))}})` — a `.trim()` comparison, with the WRONG-password branch its own alert. **It gates several actions, not one**, including the media-server hard reset. Note what it is NOT: user notes are `addUserNote`/`delUserNote` via `invokeServerCommand` and carry no password at all. **"NOTHING. Wire it" was ALSO wrong**, corrected the same day: `deleteAlertPW` appears **nowhere in `apps/room/src`**, so the field the comparison needs does not reach this room. The comparison target exists in the REFERENCE, which is what the row asked for; the value still has to arrive here | `deleteAlertPW` on the room's session config, from the controller |
| 4 | **`session-refresh-roster` and `session-soft-reset` promise a server command and only refetch locally.** Now in `session-control.svelte.ts:74-79` and `:82-89`. **The wire is CAPTURED and the names are exact** (read 2026-08-23): byte 2169139 is `refreshRoster(){sendServerAdminCommand("refreshRoster", null), bootbox.alert("Command send OK. Please allow 1/2 minute for old entries to get deleted from the list")}` and byte 2167060 is `sendServerAdminCommand("softResetSession", {}), this.done(), bootbox.alert("Soft reset request sent...")`. Our alerts are those strings verbatim — the wording is faithful and the FACT is not, because nothing is sent. **"THE SERVER HALF IS UNCAPTURED" WAS FALSE, and it was false because ONE occurrence was read instead of all three.** `refreshRoster` appears at bytes 2148196, 2169139 and 2169195 of the v4 bundle. 2169139 is the sender the row quotes. **2148196 is the TEMPLATE, and the reference describes its own server behaviour on screen**: the button reads `" Refresh Roster & Count (User List) "` under `<h5>' This clears the user list and forces all "stale" connections out. '</h5>` and `<strong>"It will take up to 1/2 minute for changes to take effect"</strong>`. The soft reset is the same story and the method is **`softReset()`**, not `softResetSession()`: its button is `" Soft Reset Session "` under `" Use this before a hard reset. Resets the media state of the room, Makes all users reconnect to the media servers gently. Works Well... Swap to your backup media servers if the primary are not working, and vice versa. "`. So the behaviour IS captured, by the reference's own copy, and no owner decision is needed. The same pane also yields `reloadSession()`, `hardReset()` → `sendServerAdminCommand("hardResetSession",{revoke:!1})`, `hardResetAndRevoke()` → `{revoke:!0}`, and `hardResetMediaServerOnServer` → `resetMediaServer`, all password-gated by `deleteAlertPW` (row 3) | NOTHING. Two server commands whose effect is now written down |
| 5 | **Eleven inert controls — and NINE of them now have their wire, read 2026-08-23. "No wire captured" was false for every one of the six media controls.** All six are `remotePresCommand` sub-commands whose call sites sit together at bytes 2063708-2064410 of the v4 bundle: `"mutemic"` (` Mute Audio `), `"mutecam"` (` Mute Camera `), `"mutescreens"` (` Stop Screens `), `"restartScreen"` (` Restart Screens ` — singular in the command, plural in the label), `"startRec"` (` Start Rec `), `"stopRec"` (` Stop Rec `). Both ends captured: `sendServerAdminCommand("remotePresCommand",{user,cmd})` at 2080529, received as `case"remotePresCommand": emit("remotePresCommand", xe.subCmd)` at 996380 — **the key changes from `cmd` to `subCmd` across the hop, so the server re-wraps, which is where the authority check belongs.** `start-recording`/`stop-recording` were filed as "blocked on a MediaMTX host"; they are not — the room half is a peer command like the other four. `debug-log` is a three-hop exchange with a captured receiving UI (`textarea#debugLogModalTxt rows="1000"` in `#debug-log-modal`). `upload-profile-picture` has `adminUploadProfilePic` → `case"updateProfilePic"`. **`disable-private-chat` is a MATCH**: upstream renders that button with an icon, a label and no `x("click")` binding at all while every neighbour has one. **`test-follow-sound` is BUILT** (2026-08-23): the method is `testFollowChatSound` — `Chat` in the middle of the name is why three sessions of searching missed it — and the sound is `pling`, already in `SOUND_EFFECT_SOURCES` with both files on disk. **`get-my-token` is now EVIDENCED and still inert**: byte 2255348 is a `"Session Information"` bootbox showing `globals.sessionID` and `globals.sesionToken` in two readonly inputs with Copy buttons. **Ten inert controls remain, not eleven.** Reasons rewritten in `user-action-intent.ts`; disposition contract 9/9 | a SERVER-side presenter check in `services/**` for the six media controls — not evidence. They stay inert until the server enforces authority, which is the 2026-08-07 rule, not a missing capture. `get-my-token` needs only building |
| 6 | **BOTH HALVES DONE — and half (b)'s premise was false, exactly like half (a)'s.** This row said *"the room has no `@playwright/test`, no config and no specs"*. Wrong. `apps/room/scripts/verify-{tooltip-placements,screen-volume,viewer-only-layout,mobile-layout}.mjs` launch **real Chromium** through `@playwright/test`'s `chromium`, resolved from `apps/controller` deliberately — the scripts say so at their top, so that adding a browser gate is not a dependency change. They are not smoke tests: they measure tooltip arrow boxes and the 6px gap `k_([0, 6])` specifies, the volume icon per level, the navbar dropdown's full child order, split layouts and mobile widths, and exit non-zero on a mismatch. jsdom reports every rect as zero, so no vitest run can make those claims. **Measured 2026-08-23 locally: 6/6 placements, 8/8 volume states, 4/4 viewer-only layouts, 4/4 mobile widths — 22 assertions, all green.** Two keep an honest NOTE rather than overclaiming: viewer-only measures the reference's CSS taking effect on our elements rather than a pixel diff (no capture of this product in that mode exists), and mobile renders the ARRANGEMENT rather than the Svelte component (the room needs a controller). **THEY CANNOT RUN IN CI, AND I PROVED THAT BY TRYING.** I added them to the `room` leg, dispatched, and the runner failed with `MODULE_NOT_FOUND` on `verify-tooltip-placements.mjs`: `.gitignore:176` ignores `/apps/room/scripts/` **entirely — zero files tracked**. The rule is deliberate and documented at `.gitignore:160-175`: collectors that reach the REFERENCE application live there and are not published, while the repository's own gate lives in `apps/room/gate/` and is. The step was reverted. So the room's browser coverage is real, green and **local-only by design** | an OWNER DECISION, and it is genuinely one: publishing `/apps/room/scripts/` would put reference-reaching collectors in a public repository, which is the exact thing `.gitignore:160-175` was written to prevent. The alternative is a published gate under `apps/room/gate/` that re-implements the measurements against tracked inputs only |
| 7 | **`kick` NOW SENDS — built 2026-08-23. `kick-ban` deliberately does not, and is now INERT rather than lying.** The defect: `kick` and `kick-ban` shared one branch that opened a prompt, closed the modal and alerted *"User kicked OK"* while sending nothing, because this room had no kick command at all. Worse than the inert controls, which stay silent — a presenter clicked Kick, was told the person was gone, and the person stayed. **Built:** `kickUser` in `presenter-commands.remote.ts`, presenter-gated by `presenterRoom()` exactly like `forceReload`, addressed with `publishToUsers` on `privCmds`, payload `{cmd:'kickUser', targetUserId, msg}` with the message bounded `min(1).max(500)` after trim. Receiver in `events.svelte.ts` emits **then** disconnects — upstream's asymmetry against `forceReload`'s disconnect-then-emit, reproduced not normalised. Contract test added with its **negative control seen RED** (`expected [] to deeply equal [{targetUserId: 5, …}]`). **`kick-ban` is NOT aliased onto it**: a ban must outlive the frame and this room has nowhere durable to record it, so reusing the plain kick would drop the ban silently — the same defect just repaired. It moved from a lying `handled` branch into `INERT_ACTIONS` with that reason. **Still open:** the ban store, and upstream's `app-kicked-page` (`currPage="kicked"`), which this room has no equivalent of — the member is told why and left disconnected. **The gate hole stands:** `user-action-disposition-contract.test.ts` sorts into handled/alerted/inert and says there is "no fourth option"; a branch that dialogs and does not act counts as `handled`, which is how this hid | a ban store; a kicked page; and a contract that stops counting "has a branch" as handled |
| 8 | **FIVE MORE CONTROLS SAY IT WORKED AND DO NOTHING — the same class as `kick`, found by asking the gate's question by hand.** Every one verified by reading its branch, 2026-08-23: `kick-duplicates` alerts ``"No duplicates found for "+nick`` **hardcoded**, never looking at a roster; `admin-notes-password` alerts `"Wrong password!"` **hardcoded**, never comparing anything; `session-save-close-message` alerts `"Message Saved"` and writes nothing; `session-send-sales-image` and `session-send-users-url` both fall through to `closeModal(); alert('Command send OK.')` with no command — note their sibling `session-send-video` in the SAME branch is genuine, writing the video list to `localStorage`, which is what makes the other two easy to miss. **All five sit in the `handled` bucket** of `user-action-disposition-contract.test.ts`, counted as done because a branch exists. **The gate hole is the finding**: that contract says there is "no fourth option" beyond handled/alerted/inert, and a branch that dialogs without acting is exactly a fourth. A probe over the dispatch body flagged 6 branches with no acting marker; 3 were false positives of my own marker list (`copied-to-clipboard` and `invalid-restream-link` are notices about something the caller already did, `mute-all-non-admins` calls a real method), which is why each was read before being written down here | a contract assertion that a `handled` branch must call a command or write state, or be declared local-only with a reason — then these five stop being invisible |
| 9 | **A guest's pre-hydration email is RARELY discarded on `/session/[code]`, and the mechanism is not known.** Seen once in nine full e2e runs on 2026-08-23, never in eight targeted repeats of the same spec. What the call log shows, and it is specific: the passing runs take 3.2s and the failing one took 15.2s; `#login-nickname-new` kept its typed value while `#login-email` in the same form did not; and Playwright resolved that field twice — first as `<input value="" …>`, the SSR node, then as an `<input>` with no `value` attribute — so **the node was replaced rather than hydrated**. That is the failure `hydration-contract.spec.ts` exists to catch, and it is the ONLY candidate application defect this work surfaced; the other six findings were all stale specs. Mitigated but NOT fixed: `playwright.config.ts` retries twice in CI, which reports it as flaky rather than hiding it | a reproduction — the load correlation suggests throttling the CPU or the script release in `openBeforeHydration` would make it deterministic |
| 10 | **The `privCmdsIn` switch carries ELEVEN cases, not the eight this row implied — and BOTH halves of every one are captured.** The row said "SIX commands this room does not handle" and that "what each SHOULD do beyond the emit is not in the room bundle". Both were the result of stopping at byte 996700, mid-switch. Read to its end (995532→998000, v4 bundle) the cases are: `forceReload`, `remoteRestartAudio`, `getDebugLog` (the member REPLIES — `e.send("debugLogResp",{requestor:xe.requestor,log:V1})`), `debugLogResp`, `kickUser` (`emit("kickPage",xe.msg)` **then** `disconnect()`), `muteChat`, `unmuteChat`, `remotePresCommand`, `userInfo`, **`updateProfilePic`** and **`getRoster`** — the last three never recorded anywhere. Channel: `/sess/${sessionID}/privCmdsIn/${userID}-${globals.user.id}/`, subscribed `{waitForAuth:!0,priority:501}`; and on any `/cmds/` subscribe the client transmits `cmd:"getMyState"`. **`getRoster` carries a precise sort** — presenters first, then ascending `loggedIn` — and merges `privData` onto each row by `_id`. **`userInfo` guards** `!user&&!userXref`→skip, sets `user.notes=userXref.notesArr`, and on `privData` also sets `user.badges=userXref.badges||[]`. The SENDERS are all in the user-info modal at 2078600-2080700: `getDebugLog()` sends with no alert; `remoteRestartAudio()` alerts `"Audio restart request sent OK"`; `forceReload()` alerts `"Reload request sent OK"`; `muteChat(t)` sends `{user,time:t}` for `t>=0` and `unmuteChat` otherwise, alerting `"user chat muted"` / `"user chat unmuted"`, driven by three buttons — `"Mute Chat for 24hrs"` (`"24"`), `"Mute Chat indefinately"` (`"0"`, the reference's own misspelling) and `"Unmute Chat"` (`"-1"`). The presenter menu is `Force Reload / Restart Audio / Get Debug Log / Kick / Kick & Ban / Kick Duplicates / Mute-Unmute Chat / Disable Private Chat / Upload Profile Picture`. The debug-log RECEIVER UI is `app-debug-log-modal`: modal `#debug-log-modal`, title `Debug Log`, one readonly `textarea#debugLogModalTxt rows="1000"` | NOTHING. Every command has both halves |
| 11 | **Three more `bootbox.alert(msg, callback)` receivers, and the SENDER this row asked for is found.** Read at 2596300-2597700 of the v4 bundle: `hardReset` → **`this.appService.disconnectAll()` FIRST**, then *"The room is being reset by an administrator. Click OK to continue..."* → `window.location.reload()`; `openSession` → *"The session is now open, click here to reload the page and enter"* → reload; `permsChangeReload` → *"An admin has changed your room permissions, you need to reload this page to continue"* → `window.location.href = \`${apiROOT}/sessions/v2/reAuthSessionTok?sessionID=${sessionID}&tok=${sesionToken}&r=1\``. **The disconnect-before-alert on `hardReset` was never recorded** and is the same ordering `forceReload` uses. Note `sesionToken` — the reference's own one-`s` typo in the global name. **The sender is `case"changeUserPerms"` on the `/cmds/` channel: `this.disconnect(), this.appEventBus.emit("permsChangeReload")`** (byte 1025131) — disconnect first again. That same switch also carries `updateUsername`, `giveMicScreen`, `presenterColorsChanged` (sets `sessData.presenterSettings` then emits `redrawChatAndAlerts`), `updateStreamStatus` → `refreshStreaming`, and `updateProfilePic`. Sibling pages: `kickPage` sets `currPage="kicked"` with the message, `closedPage` sets `currPage="closed"` | NOTHING for two of them. Only the `reAuthSessionTok` endpoint is genuinely absent here |
