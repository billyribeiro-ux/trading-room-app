...
`room-settings-schema.ts` declares **269** settings and marks **179** of them `wired: false` —
...
**35 of the 179 are read by the reference's OWN room client**, as `sessData.<name>`.

**Twenty-three have already left the list, on the day it was written.** It opened at 202 unwired and
58 questions; both numbers have moved eighteen times since.
...
`src/lib/setting-coverage-contract.test.ts` pins the 35 by NAME and asserts separately that the seven
