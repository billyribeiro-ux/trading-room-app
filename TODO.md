...
`room-settings-schema.ts` declares **269** settings and marks **178** of them `wired: false` —
...
**34 of the 178 are read by the reference's OWN room client**, as `sessData.<name>`.

**Twenty-four have already left the list, on the day it was written.** It opened at 202 unwired and
58 questions; both numbers have moved nineteen times since.
...
`src/lib/setting-coverage-contract.test.ts` pins the 34 by NAME and asserts separately that the seven
