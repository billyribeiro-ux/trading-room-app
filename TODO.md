...
| 5 | **RESOLVED by taking the second option, and confirmed 2026-08-31.** The row offered two ways out: publish the collectors, or re-implement the measurements under `apps/room/gate/`. The second was taken. `apps/room/gate/` now holds `audit-feature-coverage.mjs`, `audit-setting-coverage.mjs`, `evidence-bound-tests.mjs`, `verify-privacy-boundary.mjs`, `verify-postgres-schema-artifacts.mjs` and `privacy-utils.mjs`, all committed; **no `package.json` entry names `scripts/` any more** — the thirty that did are recorded in `apps/room/docs/UNPUBLISHED-SCRIPTS.md` rather than deleted silently. `.gitignore:176` still excludes `/apps/room/scripts/` whole, which is the part of the decision that stands: those collectors reach the reference application and this repository is public. And the room now drives a real browser in CI — the `room-e2e` job in `.github/workflows/quality.yml:386`, 25 assertions across `room-renders.spec.ts` and `room-config-seam.spec.ts`. | nothing |
...
| 8 | **ASSERTED 2026-08-31, and not reproducible under the condition the row named.** The row said *"the load correlation suggests throttling the CPU or the script release"*. Both were tried: twelve repeats on a machine at load average 12, then ten more with `Emulation.setCPUThrottlingRate` at **8x** through CDP — **twenty-two attempts, zero reproductions**, against the row's own eight. Not a proof of absence, and the test does not claim one. What changed is who finds it next: **the field was asserted nowhere**, so the only record was this sentence. `room-renders.spec.ts` now drives a guest handoff, reads `#login-email` as the live DOM property (an attribute assertion would fail on a correctly hydrated field, since Svelte sets the property client-side and does not re-emit the attribute), and re-asserts it AFTER the mount-time effect strips `jwtSite` from the address bar — both halves of the same page's known behaviour, together, because both turn on that effect. **A defect was found on the way in:** `e2e/handoff.ts` minted every token with `id: '1'`, and `handoff-token.ts:149` refuses `type === 'guest' && id !== ''` as `bad-claims` — so the guest door was unreachable from the suite and answered 403 with an error page. The helper now derives the default from the type, so the rule is honoured by construction and the refusal itself stays testable. **One consequence to read correctly:** `playwright.config.ts` retries twice in CI, which was the mitigation while nothing tested this; a recurrence now surfaces as **flaky** rather than red, so a flaky mark on that spec is the reproduction this row asked for and is a finding, not noise. | nothing; it fails loudly now if it recurs |
...
| PC | **BUILT, and this row was stale — found 2026-08-31 by trying to build it.** Everything the row asked for exists: `CompactMessageRow` emits `id="pcm-{message._id}"`, `RoomPrivateChat.#loadMoreAnchorId` records the top row BEFORE the request at `private-chat.svelte.ts:641`, and `restoreAfterLoadMore` in `room/private-chat-scroll.ts` scrolls to it after the render that inserted the rows and backs off `LOAD_MORE_OVERSCROLL_PX = 20`. `private-chat-strip-contract.test.ts`'s **G14** pins all of it — the anchor string, the order (`await this.loadLog` before `restoreAfterLoadMore`), and the constant. Two docblocks were still describing the absent version and are corrected: `chat-paging.ts`'s "deliberately absent" paragraph and `load-more-paging.test.ts`'s "which is a gap and not an omission". The three-field shape they pin is still right — the anchor is a DOM id with a one-call life, not paging state — so the assertion survives its own explanation. | nothing; recorded here until the next tracker pass removes the row |
| 9 | **CLOSED 2026-08-31, and the row's own prescription was wrong.** It said the remaining half of `userInfo` *"asks the MEMBER'S OWN BROWSER for live telemetry … a round trip to a peer, not a database read, and it is the same shape as `getDebugLog`/`debugLogResp`: a request frame addressed to one member and a reply frame back"*, and called it **"Blocked on nothing; not built."** Read at verified boundaries instead: `a ? o.socketService.getUserInfo(s,r,a,l,c) : o.getUserInfoDB(s,r)` (byte 1,159,275), whose socket arm is `this.socket.invoke("invokeCmd",{cmd:"userInfo",uid,rid,socketID,serverID,liveOnly})` (byte 1,026,474) — an invoke naming a live SOCKET, answered by the SERVER holding it. `case "userInfo"` in `privCmdsIn` (byte 996,456) reads `xe.user`, `xe.userXref` and `xe.privData` and emits: **it is the answer arriving at the asker, not a request arriving at the target.** The member's browser is never involved and could not be — a page cannot learn its own public address. Building the prescribed peer round trip would have been inventing a protocol the reference does not have. **What it actually delivers is five cells, and none of them had a producer:** `ModalHost.svelte` renders `targetUser.ip`, `.userAgent`, `.appVersion`, `.streamServer`, `.serverId`, each declared once on `ModalTargetUser`, each consumed once, each assigned nowhere — `n/a` for everybody, always, exactly as `loggedIn` and `email` were before `server/user-detail.ts`. **TWO are now built:** `ip` and `userAgent` come from the request that opened the SSE stream, recorded on the hub's `ListenerContext` and returned by the same presenter-only, room-scoped `user-detail` endpoint, so `RoomUserDetail.decorate` fills the cells with no client change at all. `connection-facts-contract.test.ts` pins them, including that neither ever reaches the roster wire — asserted against a PRESENTER recipient, because the first version of that assertion was answered by `publishRosterToRoom`'s existing `locStr` redaction and its control came back green. **THREE are not, each with the measurement:** `appVersion` is upstream's `data.cver`, which only the client knows, and a member whose browser is misbehaving can report any string — which is the case the cell exists for; `streamServer` and `serverId` are the media plane, blocked on the same `STREAM_SERVER_MTX` host as rows X, AC and R. `location` (`privData.locStr`, byte 2,061,069) is a sixth and needs a geo-IP service — an owner decision about an external dependency. | nothing buildable; three cells blocked and named |
