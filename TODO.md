...
| S   | **The login page — ASK WILL BEFORE TOUCHING IT.** Placeholder recorded 2026-08-11 at the owner's request. **The flow is now CONFIRMED FROM EVIDENCE, 2026-08-14, so the conversation can start from facts.** The Launch button is captured verbatim at `evidence-dumps/TIER1-fetched/views/page.manageSession.html:11` (and again on `page.welcome.html:379`): `<a ng-href="/session?id={{sess.uuid}}&jwtSite={{tokSite}}" target="_blank" …>Launch</a>`. **`jwtSite` appears NOWHERE in the room's 2.9 MB bundle**, so `/session` is the MAIN site's route: it validates the site JWT and hands off to the room. The room then ALWAYS renders `app-session-login` — `doLoginCheck()` has exactly FOUR callers and every one is a click or submit binding (`app-session-login.full.js:408,451,908,948`), so **nothing auto-submits** and the button reads `Login`. What is prefilled depends on the handoff: with a `tok`, name comes from `savedNick` or the token's `name`, email from the token's `email`, and `this.email && e && (this.readOnlyEmail = !0)` makes the email READ-ONLY — so the member does not retype those, but does press Login. **A PASSWORD is only demanded when** `showPresenter || 'pw' == authMode || 'webinarRoom' === authMode` (`:362`, `:1013`), where `showPresenter = sessData.showPasswordField` — the manage-page checkbox "Show password field?", help text "Show password field on the login page". Default `authMode` is `"reg"`. The room also accepts credentials straight off the URL — `pw`, `email`, `name`, and `dlf=1` which sets `disableLoginForm` (`main.d6d3c112b59b7d0d.js` byte 2595200ff). **HONEST GAP:** what `/session` does internally is server-side on protradingroom.com and is in no capture here, so whether it ALWAYS mints a token cannot be proven from what we hold. | **owner decision** — the facts are gathered; the question is what WE should do | this row |
