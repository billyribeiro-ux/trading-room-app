...
| Y | **`schema:verify` is a DEAD GATE and the settings schema is stale. FIVE causes found; four fixed in the generator and decoder, one localised and remaining.** `pnpm test` runs `schema:verify` FIRST and at HEAD it throws `ENOENT: evidence-dumps/login-page/manage` — never present here — so the chain fails at step one. Copying it in makes the generator run and `schema:verify` **pass**. Settings side-by-side, measured at each step: **16 baseline → 1094 → 65 → 63 → 62.** **(1) FIXED** — a helper taken from inside a following `<div ng-show=…>`; `pairSecretKey` absorbed the sample-link label, offsetting every node after it. 1094 → 65. **(2) FIXED** — the schema recorded help TEXT but not SHAPE. The reference writes helpers **four** ways, counted: 136 `<br><label class="muted">`, 37 `<br><label>`, 5 bare `<label>`, 13 a plain TEXT NODE with no element. `helpShape` is now generated. **(3) FIXED** — `outline.mjs` capped every text node at **160 characters**, so four helpers shipped cut off mid-sentence. Raised to 1000; the longest real helper is 203. **(4) FIXED** — `settingHelp()` consulted three hand-maintained tables (`BARE`, `CLASSLESS`, `NO_BR`, 16 names) beside a generated file of 269. It had already drifted: it held 3 of the 13 text-node helpers, so 10 rendered with a `muted` class and a `<br>` the reference has none of. **Verified before replacing:** all 9 `CLASSLESS` generate `plain` and all 4 `NO_BR` generate `bare`, so the derived shape reproduces the hand-written one exactly where they overlap. **(5) REMAINING, localised** — 62 differing, all one insertion at `doNotAutoSoftReset`: our render emits its helper (`label` + text) twice where the reference emits it once. Not a duplicate schema row (269 names, zero duplicates), not the two settings loops (disjoint slices — `apiSecret` at 130, this at 206), and not `helpCopy` (renders once). The reference's own capture contains that string once. **Nothing half-applied:** the evidence file, regenerated schema, `settingHelp` rewire and renderer change were all reverted, so the suite is green and the gate is dead exactly as at HEAD. Only the generator and decoder fixes are committed; they change nothing until the schema is regenerated, which must be the same commit that resolves (5). | MEDIUM — a gate that cannot run has been protecting nothing | this row; `CHANGELOG.md` 2026-08-12 |
