...
defect already found and fixed for screens — `focus-on-screen-contract.test.ts:12` says in as many
...
`ModalHost.svelte:2376-2381`) is the same. So is the archive pair: the `Reload Log List` button at
...
table, which is the 24-hour path. Distinct from the viewer's own mute at `+page.server.ts:628-640`,
...
| **AE** | **The `+page.svelte` decomposition — the plan and its record now live in `apps/room/docs/PHASE-5-DECOMPOSITION.md`, and this row is the pointer, not the narrative.** Phases 1–4 are complete and Phase 5 has run slices 0–27. **THIS ROW DELIBERATELY STATES NO LINE COUNTS, AND THAT IS THE CORRECTION RATHER THAN AN OMISSION.** The previous version carried `9,605 / 8,627 / 978` and was wrong by 3.3× within a day; a draft of this rewrite carried today's figures and four of them were wrong within the hour, because another session is decomposing these files continuously. A number that is re-measured every time the suite runs does not belong in prose that is edited by hand once a week. **`apps/room/src/lib/source-size-contract.test.ts` is the authority** — 37 ceiling entries over 181 cases, ceilings that only ever go DOWN, plus a staleness check so a ceiling cannot sit far above the real count and silently license growing back. Read a ceiling from there; do not read a line count from here. **WHAT IS ACTUALLY LEFT, and it is the only part of this row that is work:** (1) **no automated browser check runs in CI** — `.github/workflows/` holds `backend-quality.yml`, `quality.yml` and `smoke.yml`, and none drives a browser, so a regression is caught by a person; (2) **component-render coverage was MIS-STATED here and is corrected 2026-08-17.** This row used to say "five of the six extracted panes have no mount test… `PrivateChatPanel.test.ts` is still the only one". Measured: **thirteen** test files render a component — four with client `mount` (`PrivateChatPanel`, `FilesPane`, `PresenterMuteRows`, plus the clsx fixture) and **nine with `render` from `svelte/server` parsed via `parse5`** (`RoomMessage`, `NotesPane`, `NoteEditor`, `NoteVersionHistory`, `PostAlert`, `SpeechRecoOverlay`, `StreamTabs`, `SwingAlerts`, `DayTradeAlerts`). An established SSR-render pattern existed the whole time and this row did not mention it, which sent at least one session hunting a gap that was half-filled. **The two forms are not interchangeable, and that was tested rather than assumed:** SSR emits the structure but NOT anything an `{@attach}` sets, because attachments are client-only — rendering `PresenterMuteRows` server-side yields the row ids and no `checked`. So `label.control`, `input.checked` and `click()` need `mount`; structure and attributes do not. **What is genuinely still open:** `AlertChatArea`, `RoomSidebar`, `PresentationArea` and `RoomNavbar` have neither form. `AlertChatArea`'s headline behaviour is scroll-follow, which reads `scrollHeight`/`offsetHeight` — both `0` in jsdom — so a test there would assert something that cannot fail honestly, and that is a real limit rather than a backlog item. (was:) **five of the six extracted panes have no mount test.** Phase 2 named this its real deliverable — "a pane can be MOUNTED… each remaining component gets the same treatment" — and `PrivateChatPanel.test.ts` is still the only one, so `RoomSidebar`, `RoomNavbar`, `AlertChatArea`, `PresentationArea` and `FilesPane` are asserted on as source text. jsdom has no layout, so even those cannot prove a panel is draggable or positioned where the capture puts it. **THE RULE THE WHOLE PHASE EARNED, which outlives the line counts: migrate the tests with the code, and re-point every `not.toContain` at the file that now owns the thing.** A positive assertion fails loudly when a region moves; a negative one starts passing for the wrong reason. That has now happened four times in this repository — `exactAlerts`, `chat-mode-contract.test.ts`, and twice in `screen-volume-contract.test.ts` | **MEDIUM — the standard is being met; the verification is not** |
...
| **AH** | **THE SWEEP IS DONE, 2026-08-17 — 253 markers across 52 test files, THREE dead guards found, all three fixed. What remains is making it impossible to recur.** Each one sliced a source file for a marker that had moved during the decomposition, so `indexOf` returned -1 and every `not.toContain` built on it passed against the empty string: `screen-volume-contract.test.ts:466` (`<div class="room-sound-options">`, moved to `RoomNavbar.svelte` in `7023893`); `focus-on-screen-contract.test.ts` (`function selectScreenTabOfId(`, now a method on `RoomMediaTransport`) — the file whose own subject is a control that said "Bring everyone here" and brought nobody; and `private-chat-remote-contract.test.ts` (`async function loadPrivateChatLog(`, now `RoomPrivateChat.loadLog`). **All three are re-pointed at the file that owns the behaviour and assert the index is `> -1` BEFORE asserting anything about the slice**, and each was negative-controlled: mutating the guarded behaviour turns them red, where before the fix the same mutation left the suite entirely green. **THE METHOD MATTERS MORE THAN THE COUNT, because it is repeatable and an eye is not:** for every `X.indexOf('literal')` in a test, check the literal against EVERY file that test reads; absent from all of them means no binding could rescue it. **RULING OUT THE INSTRUMENT WAS MOST OF THE WORK — the first version of the scan reported six and four were its own fault.** It mapped each `const` to one file and so missed local re-binding (`APPLIED` is one CSS file at module scope and a different one inside a test body), it read markers quoted in PROSE as live calls (`chat-mode-contract.test.ts` explains its own deleted slice in a comment), and it assumed every `.indexOf` runs on a file when `speech-reco-overlay-render.test.ts` runs one on rendered DOM. **Running the tests is what exposed it:** three of the four assert `toBeGreaterThan(-1)` and were green, which is impossible if the marker is absent. A scan that had been trusted would have produced four fictitious defects and sent somebody to read working tests. **THE STANDING GATE NOW EXISTS — `apps/room/src/lib/slice-anchor-contract.test.ts`, 2026-08-17 (S9).** This row asked for exactly it: per-SLICE rather than per-file, catalog-driven over every `src/lib/*.test.ts`, with the false-positive classes encoded so it does not cry wolf. It walks the TypeScript AST — the question is whether a VALUE flows from an `indexOf` into a `slice` bound, and no pattern can answer that — and requires each such local to be asserted found. **It found TWELVE live instances beyond the three this row fixed**, nine of them first-order; the sharpest was `chat-gif-muted-contract.test.ts:138`, slicing `pageCode` from `indexOf('const messageChrome')` with no check. All twelve now carry an anchor assertion and **all twelve pass**, so the pattern was unguarded but not yet broken — stated plainly, because twelve new assertions reads like twelve bugs and it was not. **Two rounds of my own false positives were caught before reporting:** the first scanner reported ZERO repo-wide because it detected `.slice(` via `node.expression.getText()`, which for `code.slice(at)` yields `code.slice` and has no parenthesis — "0 problems" read exactly like success; the second reported nineteen, seven of which were correct code guarding with `return open === -1 ? null : …`, so the conditional form is now recognised rather than legislated against. A vacuity guard on the guard is included: if the scanner ever stops finding the pattern, that test fails instead of passing. Negative-controlled — anchor removed from `webcam-contract.test.ts` → red naming that line; reverted → green, with the removal verified to have LANDED first. **Honest gap, counted not hidden:** 142 sites inline their `indexOf` into the slice bound, where there is no local to name in a failure message; pinned as a ceiling that may only go down | **CLOSED — the gate exists, is catalog-driven, and found twelve more than the sweep did** |
