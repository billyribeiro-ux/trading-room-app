...
| Y | **`schema:verify` is a DEAD GATE, and making it live reveals a two-capture conflict.** Found 2026-08-11 while wiring item X. `pnpm test` runs `schema:verify` FIRST, and at HEAD it throws `ENOENT: evidence-dumps/login-page/manage` — that file has never existed in this repository, so the whole `pnpm test` chain has been failing at step 1 and the schema has been unverifiable. Copying it in from `new-room-control/evidence-dumps/login-page/manage` makes the generator RUN — it reports `269 settings, wired: 44` — but the regenerated schema differs from the committed one by **45 lines**, every difference being help text the committed copy has as `null` (e.g. `usernameInstructions` gains "Instructions how user can edit his username"). Regenerating then moves the Settings side-by-side from its baseline of **16 to 1094**, because `manage-sections-sbs.test.ts` compares against a DIFFERENT capture — `/Users/billyribeiro/Desktop/new-room/mising/file2`. **Two captures of the same page disagree, and which one wins is not mine to guess.** Nothing was changed: the evidence file was removed again and the schema left exactly as committed, so this is recorded rather than absorbed. **What it blocks:** marking `hasAppPairLink` and `pairSecretKey` as `wired: true` now that item X consumes them — the flag is inventory metadata and does not gate the endpoint, which works either way. | MEDIUM — a gate that cannot run has been protecting nothing | this row |
