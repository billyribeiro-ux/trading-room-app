...
| AJ | **TWO REFERENCE SURFACES FOUND BY ARITHMETIC, 2026-08-29 — `app.css` styles 29 classes nothing wears.** Measured across 200 class selectors in our own `app.css`, with every `.svelte` and non-test `.ts` in `src/` as the corpus of possible wearers. The 29 split cleanly and the split is what makes them actionable: **14 are CAPTURED** (the reference's own sheet has them, so the rule is waiting for markup) and **15 are OURS** (invented pre-decomposition panel names that nothing will ever wear). Two findings fall out of the captured half. **(a) TWO OF THE FOUR WERE ONE CONTROL, and it is BUILT — 2026-08-29.** `edit-user-avatar-options` was read at template `K2e` (bundle byte 2,058,852) with its consts parsed from the `app-user-info-modal` table at 2,087,748, and **const 23 is `remove-profile-picture-btn`** — the button that shipped earlier the same day as a floating control on the avatar, presenter-gated, under a note admitting its handler had not been located. It belongs INSIDE this dropdown, its click is `clearProfilePic()`, and the menu's gate is `O(6, o.user.userXrefID === o.appService.globals.user.userXrefID ? 6 : -1)` — **no role term at all**. Both guesses about the button were wrong, and the correction widened the server: `roomForAvatarChange` in `profile-picture.remote.ts` now answers two authorities, and the self path is STRICTER than the presenter one because it names nobody — the id must equal the one the server read from the session. The menu is TWO-STATE (`O(4, prefs.profilePic ? 5 : 4)`), not four items at once. Four negative controls seen red, three of them also caught by a new `every scoping helper is real` assertion that stops an indirection from becoming a hole in `remote-command-scope-contract.test.ts`. **TWO REMAIN and neither is markup:** `chat-stars`, a per-member rating nothing here reads or writes, and `tagline`, which needs a column. The original finding follows. **(a) `#user-modal` was missing four affordances.** `edit-user-avatar-options`, `remove-profile-picture-btn`, `chat-stars` and `tagline` all carry the Angular component id `ng-c1441935951`, which IS the user-info modal this room renders — so the modal is built and four of its controls are not. `remove-profile-picture-btn` is the other half of `upload-profile-picture` (built 2026-08-29): a presenter can set an avatar and cannot clear one. `chat-stars` is a per-member rating nothing here reads or writes; `tagline` needs a column this room does not have. **(b) WAS NOT A SURFACE AT ALL — THE GATE WAS BLIND, corrected 2026-08-29.** This row said *"a microphone-test surface with FIVE states … with no counterpart here at all"*. **It is fully built.** `ModalHost.svelte` declares `type MicStatus = 'idle' | 'testing' | 'success' | 'no-audio' | 'error'`, assigns every one of the five, and renders `class="mic-status mic-status-{micStatus} mb-3"` — so all five classes are worn by an INTERPOLATION, which `orphan-style-contract.test.ts`'s literal matcher could not see. A matcher that answers "no" for the wrong reason files working code as absent, which is the mirror of the hollow-coverage failure that same file records about comments and the more expensive direction, because the answer looks like work. `isWornByInterpolation` now sees it — requiring BOTH an interpolated prefix and the suffix as a string literal, so `mic-status-{x}` cannot vouch for `mic-status-anything` — and the five entries are gone from the catalog, which is that file's own declaration that something is done. Three negative controls seen red, including removing the interpolation site alone to prove a half-match does not vouch. **The catalog is 13 entries to 8.** **How this was found is the point:** `.debug-area` sat in `app.css` with two rules and no wearer for the entire port, and it was the captured class for the Debug Log textarea — the CSS was the only trace that the feature was missing. That was luck. `orphan-style-contract.test.ts` makes it arithmetic: all 29 are catalogued with a reason and a measured `captured` flag, a THIRTIETH fails the build, and an entry that gains markup fails until it is deleted — the same declaration `INERT_ACTIONS` treats as done | (a) and (b) are unbuilt reference surfaces, each now named with its component id. **The 15 invented rules are GONE, 2026-08-29** — 26 rules and 185 lines, deleted in their own commit for the reason the measuring one recorded. Not one rule was partial: every selector in every one of the 26 was dead, so `postcss` could remove whole rules and refuse by construction to touch a rule mixing live and dead selectors. Every custom property they referenced still has other consumers, and the browser suite is green on the deletion. **`remove-profile-picture-btn` is also gone from the catalog** — built the same day, which turned the gate red and is the declaration that it is done. Thirteen entries remain and every one is now a FEATURE rather than dead code |
