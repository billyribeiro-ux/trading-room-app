...
| --- | --- | --- | --- |
| ~~M~~ | ~~**Neither app has a check that boots the built server and fetches a page.**~~ — **CLOSED 2026-08-10 04:48.** `pnpm smoke` (`scripts/smoke.mjs`) probes all three tiers in about a second and exits non-zero on failure: the controller's six public routes with a content check, the room's invalid-token refusal (403, not 500 — the exact Kit 3 signature), and the SFU's health payload plus its 404 contract. Verified both ways — 0 against production, `6 of 9 checks FAILED` and exit 1 when pointed at a host that answers differently. Documented in `DEPLOYMENT.md` as a step after every deploy. **Not yet automatic**: it is a command, not a CI gate or a post-deploy hook, so it still depends on someone running it. Wiring it into the deploy path is the remaining half. | — | `CHANGELOG.md` 2026-08-10 04:48 |
| ~~M2~~ | ~~**`pnpm smoke` is not wired into anything.**~~ — **CLOSED 2026-08-10 04:52.** `.github/workflows/smoke.yml` runs it on every push to `main` (which is when the controller auto-deploys), waits for the home page to answer 200 first so it does not race the build, and is also `workflow_dispatch`-able by hand — which is what a manual room deploy needs, since no push corresponds to one. Deliberately NOT on a schedule: a 15-minute canary would be 96 billed runs a day to re-prove something that only changes on deploy, and an uptime canary belongs in a monitoring service rather than in CI. The job installs nothing, because `scripts/smoke.mjs` uses only `fetch`. | — | `CHANGELOG.md` 2026-08-10 04:52 |
| M2-orig | **`pnpm smoke` is not wired into anything.** It exists and works; nothing runs it automatically. The controller auto-deploys on push to `main`, so a Vercel deploy hook or a GitHub Action running `pnpm smoke` against the fresh deployment would make the gap structurally closed rather than procedurally closed. Until then a tired operator skipping one command reopens the hole that caused two outages. | **HIGH — the half that makes it a gate** | `CHANGELOG.md` 2026-08-10 04:48 |
| M-orig | **Neither app has a check that boots the built server and fetches a page.** Two total outages in one day got past `svelte-check`, 566 and 524 passing tests, and clean builds — because every one of those inspects source or bundles, and neither starts the artefact that ships. The room's Kit 3 env bug (`$env/dynamic/private` empty at runtime) and the controller's gsap externalisation (`Cannot use import statement outside a module`) are both invisible until a built server runs against a real environment. The room now has a one-line smoke test (`/session?…&jwtSite=bogus` → 403, documented in `DEPLOYMENT.md`); the controller has none, and `/` returned 500 for hours while every gate stayed green. A post-deploy probe of `/` and `/login` would have caught it in seconds. | **HIGH — this is how both outages reached production** | `CHANGELOG.md` 2026-08-10 04:24 and 04:35 |
