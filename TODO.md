...
| Y | **`schema:verify` is a DEAD GATE, and the settings schema is genuinely stale. Two real defects found; one fixed, one specified.** `pnpm test` runs `schema:verify` FIRST and at HEAD it throws `ENOENT: evidence-dumps/login-page/manage` — a file that has never existed in this repository — so the chain has been failing at step one and the generated schema has been unverifiable. Copying it in from `new-room-control` makes the generator run and `schema:verify` **pass** (`268 extracted + 1 reviewed = 269; 42 wired`). **My first diagnosis was wrong and is withdrawn:** the two captures do NOT disagree about help text — `mising/file2`, `evidence-dumps/login-page/manage` and the uncompiled partial all carry exactly **141** `<label class="muted">`, and all three contain the strings in question. The committed schema is simply out of date. **DEFECT 1, FIXED:** the generator attributed a label from inside a following `<div ng-show=…>` to the setting above it — `pairSecretKey` took "Sample link you would need to use to add each user…", which labels the sample-link input, not the setting. That single mis-attribution offset every node after it and drove the Settings side-by-side from 16 differing elements to **1094**. `extract-manage-schema.mjs` now breaks on a conditional container; regenerating gives `pairSecretKey: help: null` and the score drops to **65**. **DEFECT 2, SPECIFIED NOT FIXED:** the remaining 65 are one shape difference. The reference writes helpers TWO ways — `<br><label class="muted">` (141 of them) and a bare `<label>` with no `<br>` — and the schema records the help TEXT but not which shape, so our renderer emits the muted form for all of them. Closing it means adding a shape to the generated schema and honouring it in the manage page. **Nothing was left half-applied:** the evidence file and the regenerated schema were both reverted so the suite is green, and only the generator fix is committed — it changes nothing until the schema is next regenerated, which is the same commit that must carry the renderer change. | MEDIUM — a gate that cannot run has been protecting nothing | this row; `CHANGELOG.md` 2026-08-11 17:40 |
