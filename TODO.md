...
| --- | --- | --- | --- |
| P | **The SFU liveness fix is committed but NOT deployed** (`a11883c`). The box still runs the 2026-08-09 build, in which the peer loop has no timer, so an abandoned socket holds its `max_peers` slot, its per-identity slot (4 max), and the room's router forever. Measured 2026-08-10: two sockets counted as peers for **2h 07m** and **1h 49m** with **zero application bytes since connect**, both belonging to the same user. Deploying is the documented rebuild in `DEPLOYMENT.md` — `cd /opt/sfu-build && docker build --build-arg CARGO_BUILD_JOBS=1 -f media/Dockerfile -t tradingroom-media:local . && systemctl restart tradingroom-media` — about **15 minutes**, and the restart drops every connected socket (they reconnect). Two optional new variables may be set in `/etc/tradingroom-media/media.env`: `MEDIA_PEER_PING_SECONDS` (20) and `MEDIA_PEER_SILENCE_SECONDS` (60); both default correctly if absent. **Also outstanding: `services/**` is a mirror, so this change must be promoted to `new-room-control` and re-sealed** — see `apps/room/TODO.md` entry 2, a divergence that has already happened twice. | **HIGH — resource exhaustion, and it is written but not running** | `CHANGELOG.md` 2026-08-10 05:42 |
