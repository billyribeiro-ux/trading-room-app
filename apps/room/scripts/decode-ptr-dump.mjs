...
for (const [k, v] of Object.entries(meta)) {
  if (['capturedAt', 'url', 'ua', 'role', 'viewport', 'errors', 'styleSheets'].includes(k))
    continue;
...
if (meta.resources?.length) {
  w('10-resources.txt', [
    ...
    ...meta.resources.map(
      (r) =>
        `${String(r.size ?? 0).padStart(9)} B  ${String(r.type).padEnd(12)} ${r.ms}ms  ${r.name}`
    )
  ]);
}
...
const ALL_KEYS = (() => {
  ...
  for (const cap of caps)
    for (const n of nodesOf(cap) ?? []) for (const k of Object.keys(n.style ?? {})) s.add(k);
  return [...s];
})();
...
function fmtNode(n, common, idx) {
  ...
  if (n.before !== undefined && n.before !== null)
    L.push(`  ::before: ${JSON.stringify(n.before)}`);
...
function defaultsFile(label, nodes, common) {
  const L = [];
  L.push(
    `COMMON (most-frequent) computed-style values for capture "${label}" — ${nodes.length} nodes.`
  );
  ...
  if (!isFullDiff) {
    ...
    nodes.forEach((n, k) => {
      ...
      for (const d of ['x', 'y', 'w', 'h'])
        if (r[d] !== br[d]) L.push(`  rect.${d}: ${br[d]} -> ${r[d]}`);
      ...
      if (n.text !== b.text)
        L.push(`  text: ${JSON.stringify(b.text)} -> ${JSON.stringify(n.text)}`);
      ...
      if (L.length === 0) identical.push(n.path);
        ...
        blocks.push(
          `#${k} path=${n.path} <${n.tag}> — ${L.length} difference(s) vs baseline\n` + L.join('\n')
        );
    });
    const removed = baselineNodes
      .filter((b) => !seen.has(b.path))
      .map((b) => `${b.path} <${b.tag}>`);
