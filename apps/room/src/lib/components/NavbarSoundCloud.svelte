...
{:else}
  <!--
    ...
    **Const 176 carries the `id` attribute TWICE** — `cssSoundCloudIcon` and then `soundcloudDropdown`
    — and a browser keeps the first, so `cssSoundCloudIcon` is the id this element actually has.
    Written out rather than transcribed as a pair: the second is inert in the reference too, and the
    name it repeats belongs to the presenter's dropdown above, which is the one element in the room
  ...
  <li title="Music is playing from SoundCloud for all" class="nav-item">
    <!--
      `a11y_consider_explicit_label` is ignored rather than answered: the reference's anchor holds
      two icons and no text (`T(2,"i",166)(3,"img",169)`), and the label a screen reader needs is on
      the `li` above, where const 97 puts it.
    -->
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <!-- svelte-ignore a11y_consider_explicit_label -->
    <a
      id="cssSoundCloudIcon"
