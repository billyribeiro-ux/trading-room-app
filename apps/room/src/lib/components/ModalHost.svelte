<script lang="ts">
  ...
  */
  ...
  onMount(() => {
    ...
    /*
      This used to `return () => cleanupMicTest()`. The mic test moved to `ConnectivityModal.svelte`
      on 2026-09-01 and its teardown went with it: that component's `$effect` releases the stream,
      the `AudioContext`, the recorder and both timers whenever the modal closes OR the component
      unmounts, which is strictly more than this line covered. Deleted rather than forwarded — a
      second teardown for state this file no longer holds is a call that can only ever be wrong.
    */
  });
            ...
            <div class="row">
              ...
              <div class="col">
                ...
                {#if privateMessageHistoryEnabled}
                  <!--
                    Consts 90 and 52 both carry `data-bs-toggle="modal"` and
                    `data-bs-target="#all-user-pm-modal"`, and both are transcribed here.

                    They are INERT in this application and cannot fight the state machine beside
                    them: Bootstrap's JavaScript is not loaded — `app.html` and `package.json` name
                    it nowhere — so nothing in the page reads a `data-bs-*` attribute. The modal is
                    opened by `modals.open('all-private')` in the handler, which is this room's
                    equivalent of the data API and the half that actually works.

                    Carried anyway because the decision is to match the dump, and because the
                    repository already transcribes the same family elsewhere (`data-bs-dismiss` on
                    every modal's Close, `data-bs-toggle="tab"` on both tab strips).
                  -->
                  <button
                    type="button"
                    data-bs-toggle="modal"
                    data-bs-target="#all-user-pm-modal"
                    class="btn btn-block btn-outline-light"
