...
const TAG_STYLE_RULES: Readonly<Record<string, Readonly<Record<string, RegExp>>>> = {
  ...
  div: {
    ...
    display: /^flex$/,
    /*
      A SLIDE WITHOUT A LINK IS A `div`, AND IT WAS LOSING THIS — found 2026-09-01.

      `renderHTML` emits a slide as `['a', …]` when it has a link and `['div', …]` when it does not,
      and both carry the same `wrapperStyle`: `width:…%;height:100%;flex-shrink:0;display:block;
      overflow:hidden;`. The `a` rules above admit `flex-shrink`; these did not, so every UNLINKED
      slide had it stripped.

      What that costs is the whole carousel, not a detail. The track is `display:flex` and each slide
      is `100/count` percent of a track that is `count * 100` percent wide — so a slide is exactly one
      viewport. Without `flex-shrink: 0` the flex algorithm is free to shrink items to fit the
      container, so all of them compress into one screen width and `translateX(-n * (100/count))`
      lands nowhere near slide `n`.

      Not a CSSOM finding, unlike the three above: this was a plain gap between two rule sets that
      describe the same element in two shapes. It surfaced only because this file put a browser's own
      output through the sanitiser and read what came back.
    */
    'flex-shrink': /^0$/,
