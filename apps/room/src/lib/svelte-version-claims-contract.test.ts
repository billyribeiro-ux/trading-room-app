 ...
 *
 * `declaration-tag-contract.test.ts` — in this app and in the controller, the same sentence twice — said
 ...
 *
 * `docs/` is out of scope for the same reason, one level up: the controller's
 * `SVELTE-CONFORMANCE-AUDIT.md` is a dated audit record and quoting the versions it was run against
 * is correct. Its header carries a version correction instead, which is a thing a human writes and
 * not a thing a gate can.
 *
...
describe('the sweep is measuring something', () => {
  it('reads a corpus and knows this app s own pins', () => {
    ...
    expect(PINS.svelte, 'svelte must be pinned for any of this to mean anything').toMatch(
      /^\d+\.\d+\.\d+/
    );
  ...
  it('names each claim that does, with the version it states', () => {
    ...
    */
    ...
    for (const file of SOURCES) {
      ...
      for (const match of readFileSync(file, 'utf8').matchAll(PIN_CLAIM))
        claims.push(`${file} states ${match[1]}`);
    }
    expect(
      claims,
