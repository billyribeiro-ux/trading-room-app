...
describe('no $effect exists only to synchronise state', () => {
  it('found the effects it is meant to police', () => {
    /*
      A floor, because every assertion below is vacuous at zero. The corpus held 63 effects across 23
      files on 2026-08-28; 20 is comfortably below that and far above any plausible collapse of the
      scanner or the glob.
    */
    expect(allEffects.length).toBeGreaterThan(20);
  });

  it('reads .svelte script blocks and not only .ts modules', () => {
    // Nearly every effect in this app is in a component. A scanner that silently covered only the
    // rune modules would still clear the floor above and would be measuring almost nothing.
  ...
  it('has no effect whose body is assignments and nothing else', () => {
    ...
    expect(
      offenders.map(
        (effect) => `${effect.file}:${effect.line} assigns ${effect.targets.join(', ')}`
      ),
...
describe('the detector leaves the legitimate shapes alone', () => {
  ...
  it('does not flag a transition tracker', () => {
    ...
    expect(
      classifyOne('if (open && !wasOpen) beginOpenState();\nwasOpen = open;').assignsOnly
    ).toBe(false);
  });
  ...
  it('does not flag a guarded reset that seeds an editable form', () => {
    ...
    */
    expect(
      classifyOne("if (name !== 'settings') return;\nchatStyle = { ...initialChatStyle };")
        .assignsOnly
    ).toBe(false);
  });
