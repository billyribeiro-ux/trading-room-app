...
describe('XCP-07 and XCP-09 — the gaps this column cannot close from its own file', () => {
  it('names both where the component points, so neither can go quiet', () => {
    ...
    */
    /*
      EMPTY since 2026-09-02, and the empty list is the assertion rather than the absence of one:
      every `XCP-` row this surface documents is answered. `XCP-07` was refused on a rule that
      governs INVENTED classes rather than transcribed ones; `XCP-09` was blocked on a re-capture of
      a file that was never the only source — the rules were in the pinned bundle the whole time.

      The rows that left are still DOCUMENTED, which is what the loop below and the three lines after
      it check: the measurement outlives the gap, and a blocker that named the wrong missing thing is
      the failure worth being able to recognise again.
    */
    expect([...EXTRA_CHAT_MEASURED_GAPS]).toEqual([]);
    ...
    expect(SURFACE).toContain('XCP-09');
  });
  ...
  it('XCP-09 — the component ships 5,807 bytes of styles, and both facts that closed it', () => {
    /*
      THE SECOND HALF USED TO BE THE BLOCKER AND IS NOW ONLY CONTEXT. It said: neither the generated
      sheet nor the capture it is generated FROM has ever seen this component, so a re-capture was
      required. Both statements are still true and are still asserted below; the conclusion drawn
      from them was wrong, because a captured DOCUMENT is not the only place a component's rules
      exist. The compiled COMPONENT carries them, and it is the first half of this test.

      The rules reach the room through `src/lib/styles/captured-extra-chat.css`, generated by
      `gate/sync-extra-chat-styles.mjs` from the bundle and from sections 34 and 35 of the sheet
      below. `extra-chat-styles-contract.test.ts` owns that artifact; what stays HERE is the
      measurement of the surface — that these bytes exist, where, and that the two sheets this file
      already reads are not where they came from.
    */
    ...
    /* And the artifact that closed it exists, is generated, and is reached from `app.css`. */
    const sheet = read('./styles/captured-extra-chat.css');
    expect(sheet).toContain('do not edit by hand; run: pnpm css:sync-extra-chat');
    expect(sheet.replace(/\s+/g, ' ')).toContain('app-extra-chat .chatTabs');
    expect(APP_CSS).toContain("@import './lib/styles/captured-extra-chat.css';");
  });
});
