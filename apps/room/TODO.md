...
| 22  | ~~A role change does not renegotiate media~~ **RESOLVED 2026-08-11 for the TAKE direction; the GIVE direction is blocked on a decision.** The recipient now closes its media session and builds a new one 3s later, matching the capture's `disconnectAll()` + re-init. Taking mic/screen away therefore works completely — every producer that peer held is closed, immediately and server-side. **Giving still does not grant produce rights, and that is architectural rather than a missing line:** the SFU decides who may produce from the GRANT's role, and `/api/media/grant` mints it from the CONTROLLER's membership (`joinsMediaAsProducer(isPresenter \|\| hasCam \|\| hasMic \|\| hasScreen)`), while `isLimitedPresenter` is runtime state that never touches the membership — so a rebuilt session re-mints the same `member` grant and the SFU answers `forbidden`. **The decision, which is the owner's:** either `giveMicScreen` writes `hasMic`/`hasScreen` onto the membership (durable, works, diverges from the capture's explicitly transient model), or the grant learns to carry a runtime elevation (which means the client asserting its own authority). Neither was invented. | give-direction blocked on a design decision | in-code at the `giveMicScreen` handler |
