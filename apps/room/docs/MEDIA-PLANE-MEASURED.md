...
| **X** `recPreviewWindow` | "a MediaMTX cluster"                        | unchanged — `recPreviewLocation` is set by the REFERENCE's server on the cmds channel, and that server is not in the capture. A local MediaMTX does not produce it |
| **AC** `stopRecMsg`      | "a MediaMTX cluster"                        | unchanged, same reason: the payload is server-GENERATED text                                                                                                       |
