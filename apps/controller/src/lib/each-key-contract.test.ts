...
const ALLOWED_INDEX_KEYS: readonly { file: string; why: string }[] = [
  /*
    EMPTY, and that is the measurement rather than an omission.

    Running the room's rule here on 2026-09-01 found THREE index-keyed blocks — two in `DeskMock`
    (the candle chart and the chat replay) and one in `VoicesSection` (the testimonial rails). All
    three iterate a `const` built once at init and never replaced or reordered, so the honest outcome
    was the first of the two this rule offers: **no key at all**. Index-keyed and unkeyed reuse the
    DOM identically, which makes `(i)` a promise with nothing behind it.

    The room's copy has two entries — `PollPanel` and `RoomMessage` — where position genuinely IS the
    identity. Nothing here has that shape yet, and an empty table is what says so.
  */
];
...
describe('every {#each} key says something true', () => {
  ...
  it('every allow-list entry still has an index-keyed block to justify', () => {
    ...
    */
    const indexKeyedFiles = new Set(blocks.filter((block) => block.indexKeyed).map((block) => block.file));
    const stale = ALLOWED_INDEX_KEYS.filter((entry) => !indexKeyedFiles.has(entry.file)).map((entry) => entry.file);
  ...
  it('the segment lists carry no key at all, which is the honest form', () => {
    /*
      ...
      be satisfied by somebody deleting the `{#each}` entirely. These three are the blocks the
      2026-09-01 audit found index-keyed and corrected, and they must stay unkeyed rather than
      acquire a new invented key.
    */
    const unkeyedAt = (file: string, count: number) => {
      const inFile = blocks.filter((block) => block.file === file && !block.keyed);
      expect(inFile.length, `${file} should hold ${count} unkeyed each block(s)`).toBeGreaterThanOrEqual(count);
    };
    /* The candle chart and the chat replay, both `const` lists built once at init. */
    unkeyedAt('src/lib/components/home/DeskMock.svelte', 2);
    /*
      The testimonial rails. Its INNER loop keeps its key on purpose — `{#each [0, 1] as pass (pass)}`
      is keyed by the ITEM, not by a position, which is the form this rule asks for.
    */
    unkeyedAt('src/lib/components/home/VoicesSection.svelte', 1);
  });
});
