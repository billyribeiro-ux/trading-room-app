 ...
 *
 * Produces dumps in the SAME shape as evidence-dumps/NEXT-STEP/ptr1.json / prt2.json, so
...
`
  ...
  function walk(root) {
    ...
    (function rec(el, path) {
      const n = { path, tag: el.tagName.toLowerCase(), attrs: attrsOf(el), rect: rectOf(el), style: styleOf(el) };
      const t = ownText(el);
  ...
  async function focusStates() {
    ...
    const els = Array.from(document.querySelectorAll('input,textarea,select,button,a[href],[tabindex]')).filter(
  ...
  async function interactions() {
    ...
    try {
      ...
      for (const menu of menus) {
        ...
        caps.push(
          captureSubtree(`dropdown:${menu.className.trim().split(/\s+/).join('.')}`, menu, {
            ...
            triggerText: (chain[chain.length - 1]?.querySelector('.dropdown-toggle,button,a')?.textContent || '')
              .replace(/\s+/g, ' ')
              .trim(),
      ...
      for (const pane of panes) {
        ...
        await settle();
        caps.push(captureSubtree(`tabpane:${pane.id || cssPath(pane)}`, pane, { activatedByHarness: true }));
      ...
      for (const col of Array.from(document.querySelectorAll('.collapse:not(.in)'))) {
        ...
        await settle();
        caps.push(captureSubtree(`collapse:${col.id || cssPath(col)}`, col, { expandedByHarness: true }));
  ...
  function chunk(caps, meta, base) {
    ...
    download(`${base}-part0-META.json`, { part: 0, totalParts: parts.length + 1, caps: [{ label: '__meta__', meta }] });

    parts.forEach((p, i) => {
      download(`${base}-part${i + 1}of${parts.length}.json`, { part: i + 1, totalParts: parts.length, caps: p });
    });
  ...
  async function run() {
    const t0 = Date.now();
    const label =
      CFG.LABEL || (location.hash || location.pathname).replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-|-$/g, '') || 'page';
    ...
    log(`stylesheets: ${sheets.length} (${recovered.length} recovered by fetch, ${blocked.length} still blocked)`);
    ...
    const meta = {
      ...
      bodyAttrs: Object.fromEntries(Array.from(document.body.attributes).map((a) => [a.name, a.value])),
      ...
      resources: performance
        ...
        .map((r) => ({ name: r.name, type: r.initiatorType, size: r.transferSize, ms: +r.duration.toFixed(1) })),
      fonts: (() => {
        try {
    ...
    console.table(caps.map((c) => ({ label: c.label, nodes: (c.fullDom || c.subtree || {}).count ?? '-' })));
    return { caps: caps.length, nodes: totalNodes, files, errors };
  }

  window.__PTR_CAPTURE = { run, CFG, captureDom, interactions, stylesheets, stateRules, mediaQueries };
