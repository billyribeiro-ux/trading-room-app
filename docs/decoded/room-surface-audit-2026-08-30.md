...
byte offsets make the second reading the tempting one. **The running total of appended rows lives in
this paragraph and nowhere else, so update it here rather than adding a sentence beside it.** It
stands at **21**, and each appended row carries the sentence this one opens with, which is what
`apps/room/src/lib/room-surface-audit-counts.test.ts` counts. Twenty were appended on 2026-08-31 in
one batch, covering `PrivateChatComposer.svelte` (PCC-01…09), `GiphyPicker.svelte` (GIF-01…06) and
`SpeechRecoOverlay.svelte` (SRO-01…05) — three surfaces that had no section here — and that batch
says only what it read: it decoded four Giphy templates, six `onKey` implementations and one
292-entry const table by value, and it did not re-read any surface already in the tables above.

## Where the work stands

**0 open · 244 closed · 244 rows.**
