...
**TWENTY-TWO MORE arrived on 2026-08-31**, and they are twenty-two rows on two surfaces this
document had no section for at all: nine `XCP-` rows on `ExtraChatPane.svelte` and thirteen `QAM-`
rows on `AlertQaModal.svelte`. Twenty-three rows have therefore been appended since this document
was committed — RM-25 and those twenty-two — and none of them is folded into the totals above, which
describe the two-verifier pass and should keep describing it. Each of the twenty-two says so in its
own body, in the fixed sentence `room-surface-audit-counts.test.ts` counts, and the surfaces table
above is deliberately left at eighteen for the same reason.
