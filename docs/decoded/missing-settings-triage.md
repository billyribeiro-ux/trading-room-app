...
| `allRoomsWelcomeMatPW`     | 3     | `setWelcomeMatNoteTab` with `allRooms: true`, byte 1,474,192. The password is also POSTED back to the server as `pw`, so the server _may_ re-check it; the client check is still client-side.         |
...
reading _"a default that seeds a preference that styles nothing is not a feature to reproduce"_, and
...
- _"It runs during component INITIALISATION, not from a click, and the options string it passes is
  exactly the popup shape browsers block without a user gesture. The reference does not check
  `window.open`'s `null` return, so a blocked open is silent."_
- _"Because the statement is in both chat components, a member with the extra chat column enabled
  gets it **twice**."_
...
The third sentence — _"building it 'safely', as a link the member can click, would be inventing a
control the capture does not have"_ — was right and is honoured: there is no link. The window opens,
...
**This section opened with twelve rows and is now empty**, and two of the rows that left were
CORRECTIONS rather than builds — see the section below. The pattern they shared is worth carrying
...
`recsInRoom` left on 2026-09-03 with the first-party authenticated recording archive it gates.
...
**`isNewIndicatorOn` was filed as WIRE and correctly blocked until it had authoritative data.** The
blocker closed on 2026-09-03: the controller defines a membership as new for its first 30 complete
days and projects that decision through the signed room configuration. All three badge consumers
remain presenter-only and the setting now has both a producer and consumers.
...
asked, was told nothing, and watched the thread go on saying _"There are no questions."_

`alert-log.ts` had argued in writing that a `room_short_code` column _"would denormalise a fact this
schema already derives"_. It does not derive it for every row, and those rows had **no room anchor at
...
## FEATURE — repository-buildable rows complete

This section opened with four rows. Each repository-buildable feature is now implemented; provider
registration and production credentials are deployment inputs, not missing application paths.

**`hasAlertScheduler` LEFT THIS SECTION ON 2026-08-28, and its blocker named the wrong process.** The
row read _"A scheduler process in `services/api`, and the crate's TEST targets cannot be built
here."_ Both halves are true of that crate and neither is a reason to put the scheduler in it. The
...
**At that checkpoint, `enableDiscord` was the only unbuilt setting left.** It subsequently closed
with the OAuth implementation recorded below; provider registration is deployment configuration,
not a missing repository path. Every row of this enumeration is now built or has a recorded reason.
...
`enableDiscord` closed on 2026-09-03 with presenter-only status/connect/unlink UI, single-use hashed
OAuth state, callback exchange, identity persistence and unlink. Provider access tokens are never
stored. A production Discord application id, secret and redirect URI are deployment configuration.
...
**What this section used to end with, kept because the judgement was right:** _"Not started,
deliberately. A partial channel model is worse than none: a tab that renders and cannot be posted to
is the dead scaffolding this repository forbids."_ Nothing partial shipped — the type reaches the
...
## BLOCKED — empty for repository-buildable settings as of 2026-09-03

| setting | blocked on |
| ------- | ---------- |

- `isNewIndicatorOn`, `linkedRoomAlerts`, `recsInRoom`, `backupClusterID` and `recordChat` all left
  this table with end-to-end implementations on 2026-09-03.
- `authMode` and `description` are controller-login concerns and were never room-runtime gaps.
- Linked targets remain controller-only; the room receives a presenter-only availability bit and
  carries `dontCrossPost` through immediate and scheduled durable delivery.

---

**`isLocked` LEFT THIS TABLE ON 2026-09-02, and its blocker was a SHAPE rather than evidence.** It
read: _"Needs a lock the SERVER owns; `room_state` has no column for it, and a client-side lock is
not a lock."_ Both halves are true and the conclusion did not follow. The lock is a room SETTING on
...
because the first two — _wire it_ and _do not wire it_ — could not describe what happened here.
...
1,958,716 and 2,532,633): _"`openTranscript` passes the room name as a `&name=` query parameter to
the transcript popup. Blocked with the transcript window itself, which this room does not open."_
...
said _"it was two"_ ever since. **This document did not follow**, which is the failure it warns about
in its own header: two places recording one thing, and the stale one is always the summary.

The row was also wrong about what the feature needed. It said _"needs the private-message unread
signal, not the title"_; the reference's gate is
